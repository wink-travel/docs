...
Webhook ļauj jūsu sistēmām saņemt reāllaika paziņojumus, kad jūsu Wink kontā notiek notikumi — jaunas rezervācijas, atcelšanas, maksājumu atjauninājumi un citi. Šī rokasgrāmata ved jūs cauri iestatīšanai un labākajām praksēm.
...
8. Noklikšķiniet uz `Save` — atbilde parāda jūsu **parakstīšanas slepeno atslēgu vienreiz**; saglabājiet to tagad.
</Steps>

## Notikumu veidi

Wink šodien publicē 70 webhook notikumu veidus rezervācijās, īpašumos, kontos (pārvaldības vienībās) un inventārā (numuru veidi, tarifu plāni, galvenie tarifi, papildinājumi, ērtības, pārdošanas kanāli, akcijas). Biežākie:
...
Pilns, ģenerētais saraksts — ar aprakstu, kam tas tiek nosūtīts un saiti uz katra notikuma atsauces lapu — ir **Webhook notikumu katalogs**. Atsauces lapa katram notikumam (JSON saturs, galvenes, atkārtotas piegādes politika) atrodas [Webhooks API](/lv/api/webhooks/).
...
- `type` — notikuma veida atslēga (tiek nosūtīta arī kā `Wink-Event-Type` galvene). Izmantojiet `type` un `schemaVersion`, lai parsētu `object`.
...
Katras piegādes shēma ir dokumentēta katram notikumam [Webhooks API](/lv/api/webhooks/) atsaucē.
...
Katram webhook ir **parakstīšanas slepenā atslēga** (`whsec_…`), ko Wink parāda **vienreiz**, kad izveido webhook vai nomaina slepeno atslēgu. Saglabājiet to kā paroli. Katrs piegādes pieprasījums satur

```
Wink-Signature: t=1755250200,v1=5d41402abc4b2a76b9719d911017c592…
```

kur `t` ir Unix laika zīmogs (sekundes) un `v1` ir mazajiem burtiem hex formātā **HMAC-SHA256** no virknes `t + "." + rawBody`, parakstīts ar jūsu slepeno atslēgu, un `rawBody` ir precīzs pieprasījuma ķermeņa baitu saturs, kāds tas tika saņemts — nepārrakstiet JSON pirms pārbaudes. 24 stundas pēc slepenās atslēgas nomaiņas galvene satur otru `v1=` vērtību, kas parakstīta ar iepriekšējo slepeno atslēgu; pieņemiet piegādi, ja **kāds** no `v1` atbilst.
...
Nomainiet slepeno atslēgu portālā vai ar `POST /api/managing-entity/{id}/webhook/{webhookId}/rotate-secret`; atbilde parāda jauno slepeno atslēgu vienreiz, un vecā atslēga turpina darboties 24 stundas, kamēr to izplatāt.

## Mēģinājumi un atkārtota piegāde

- Atbildiet ar jebkuru `2xx` 10 sekunžu laikā, lai apstiprinātu. Veiciet smago apstrādi asinhroni.
...
Nosūtiet sev sintētisku `webhook.test` notikumu no portāla vai ar `POST /api/managing-entity/{id}/webhook/{webhookId}/test`. Tas ir parakstīts un piegādāts tieši kā īsts notikums, tāpēc varat pārbaudīt savu galapunktu, paraksta pārbaudi un idempotences apstrādi pirms abonējat dzīvos notikumus.

## Labākās prakses

- **Izmantojiet HTTPS** — Wink nosūta datus tikai uz HTTPS galapunktiem.
- **Atbildiet ātri** — Atgrieziet `200 OK` tiklīdz saņemat datus. Veiciet smago apstrādi asinhroni.
- **Idempotence** — Jūsu apstrādātājam jābūt idempotentam; deduplikējiet pēc `Wink-Event-Id`. Wink mēģina atkārtot, ja nesaņem `2xx` atbildi.
- **Avota validācija** — Pārbaudiet `Wink-Signature` galveni (skatīt [Parakstu pārbaude](#parakstu-pārbaude)) pirms apstrādes; noraidiet visu, kas neiztur pārbaudi.
...
- [Izstrādātāji > API](/lv/api/overview/) — Pilna API dokumentācija.
