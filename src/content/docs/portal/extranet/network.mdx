...
To manage your Sales channels, click on `Distribution > Sales channels` from the main nav bar.
...
To find new affiliates on Wink, click on `Distribution > Explore network` from the main nav bar.
...
Developers who want to manage their `Network` can head over to [Developers > APIs > Distribution](/api/overview/).
