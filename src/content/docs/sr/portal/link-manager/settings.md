---
title: Podešavanja
description: U ovom članku objašnjavamo kako možete prilagoditi vašu WinkLinks stranicu koristeći funkcije dostupne u podešavanjima.
sidebar:
  order: 4
---

Da biste upravljali podešavanjima za WinkLinks, idite na `WinkLinks` u glavnom navigacionom meniju i kliknite na karticu `Design`.

Evo na koje načine možete konfigurisati ponašanje WinkLinks pomoću `Design`:

- **Intelligent** Omogućavanjem inteligencije pokušavamo da preuzmemo `rich` sadržaj dostupan sa sajtova kao što su `Wink`, `Spotify`, `Instagram`, `Facebook`, `YouTube`, `X (formerly Twitter)` i drugih. *npr. Ako podelite IG objavu na WinkLinks, objava će biti prikazana kao što je vidite na IG, sa lajkovima, komentarima i mogućnošću interakcije sa objavom.*
- **Initial display** Kontrolišite kako vaša publika prvo komunicira sa vašim sajtom i vidi vaš sadržaj. Na raspolaganju su vam tri opcije:
    - `Column-based` dizajn. Ovo je naš preporučeni dizajn i ako ne prikazujete previše sadržaja, izgledaće odlično na vašoj stranici. Međutim, može postati `heavy` ako delite mnogo rich sadržaja.
    - `Row-based` dizajn. Ovo je jedan od načina da vaš sadržaj bude malo manje zahtevan za pretraživače, ali preporučujemo da vaš sadržaj ima sličnu količinu teksta kako bi se kartice bolje poravnale.
    - `List-based` dizajn. Većina drugih menadžera linkova prikazuje jednostavnu listu sa URL-om i minimalnim sadržajem. Izaberite ovaj način prikaza ako imate mnogo sadržaja i želite da ga prikažete što brže.
- **Profile picture** Na vrhu svake WinkLinks stranice nalazi se profilna slika vašeg naloga zajedno sa opisom naloga. Profilnu sliku možete prikazati na dva načina:
    - `Circle` Preporučujemo kružni stil ako je vaša slika vas ili bilo šta drugo što se uklapa u okrugli okvir.
    - `Rectangle` Preporučujemo pravougaoni stil ako je logo vaše kompanije vaša profilna slika, jer su većina logotipa pravougaoni.
- **Customization** Personalizujte izgled i doživljaj vaše WinkLinks stranice. Možete prilagoditi paletu boja koliko god želite da savršeno odgovara vašem brendu. Ovo podešavanje takođe utiče na prikaz Wink inventar kartica, mapa i mreža.  
  
Dodatne kontrole uključuju:  

- Ponašanje prikaza objava (Banner Mode, Smart Banner/Interactive, Layout-Aware automatsko prilagođavanje)  
- Prilagođeni QR kodovi sa brendom (oblik, boje, tačke, uglovi)  
- Organizacija sadržaja pomoću tagova (kategorizacija linkova kao što su "Hoteli", "Putovanja", "Prodaje")  
- Potpuna kontrola nad rasporedom linkova, uploadom fajlova i blokovima sadržaja

## API

Programeri koji žele da upravljaju `WinkLinks` mogu otići na [Developers > API > WinkLinks](/sr/api/overview/).

## Dalje čitanje

- Pročitajte više o [Customizations](/sr/portal/settings/branding).
