...
:::note
Testiranje uvek treba da se odvija u našem [staging okruženju](/sr/getting-started/environments). Tamo ima mnogo manje podataka, ali slobodno možete testirati koliko god želite bez uticaja na produkciono okruženje.
...
<Steps>
1. [Registrujte se](/sr/account/register) kao korisnik na Wink-u.
...
3. [Kreirajte Aplikaciju](/sr/portal/settings/applications) i povežite je sa affiliate nalogom koji ste napravili u Koraku 2. Za server-to-server API integracije, izaberite **MACHINE_2_MACHINE** kao tip klijenta.
...
13. Podesite polje `Scope` na razmake odvojene opsege koje vaša integracija zahteva (npr. `inventory.read inventory.write inventory.remove`). Pogledajte [referencu opsega](/sr/developers/authentication#scopes) za kompletnu listu.
