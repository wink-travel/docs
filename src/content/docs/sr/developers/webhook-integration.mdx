...
| Zaglavlje | Značenje |
|---|---|
| `Wink-Version` | Verzija protokola, `2.0`. |
...
gde je `t` Unix vremenska oznaka (sekunde), a `v1` je mala heksadecimalna vrednost **HMAC-SHA256** stringa
...
```js
// Node.js (Express stil; obavezno imate RAW telo, ne parsirani objekat)
...
## Pokušaji ponovnog slanja i ponovna isporuka

- Odgovorite sa bilo kojim `2xx` u roku od 10 sekundi da potvrdite prijem. Teže operacije radite asinhrono.
...
- Svaki drugi `4xx` se tretira kao "odbili ste ovu isporuku" i **ne** pokušava se ponovo.
- Svaki događaj, isporuka i pokušaj (status, deo odgovora) vidljivi su pod **Applications > Webhooks**
  i preko API-ja (`…/webhook/event/grid`, `…/webhook/delivery/grid`). Možete **ponovo isporučiti** bilo koju isporuku
  (`POST …/webhook/delivery/{deliveryId}/redeliver`, što pokreće novu seriju pokušaja), ponovo isporučiti sve mrtve
...
- **Idempotentnost** — Vaš handler treba da bude idempotentan; deduplicirajte po `Wink-Event-Id`. Wink ponovo pokušava ako ne dobije `2xx` odgovor.
...
- [Webhooks API referenca](/sr/api/webhooks/) — Šeme podataka po događaju, zaglavlja i endpointi za upravljanje pretplatama/isporukama.
...
- [Developers > APIs](/sr/api/overview/) — Kompletna API dokumentacija.
