---
title: Integracija Sojern Pixel-a
...
sidebar:
  label: Integracija Sojern Pixel-a
---
...
1. **Preuzmite vaš GTM Container ID od Sojern-a**
...
<Aside type="note">
Ne morate postavljati nikakav kod piksela na vašem sajtu za ovu integraciju. Wink automatski učitava vaš Sojern kontejner na svim stranicama gde je vaša nekretnina aktivna.
...
- **Polje za Sojern GTM Container ID** dostupno je u kartici Customizations u vašem Wink portalu. Kada unesete vaš Container ID, Wink proverava format i sigurno ga čuva povezano sa vašom nekretninom.
...
Svaki događaj prati standardnu GA4 Enhanced Ecommerce strukturu: string `event` na vrhu i objekat `ecommerce`. Pre svakog slanja, Wink briše prethodni ecommerce sadržaj da spreči zastarele podatke da utiču na nove okidače.
...
| `coupon` | string | — | Primenjeni kupon kod na stavku |
...
| `cancellation_policy` | string | Jedna od vrednosti: `Refundable`, `Non-Refundable`, `Refundable-With-Restrictions`, `Non-Refundable-With-Restrictions` |
| `rate_plan_id` | string | Identifikator cenovnog plana |
...
GA4 ignoriše prilagođene parametre vezane za stavku u izveštajima dok ih eksplicitno ne registrujete kao dimenzije vezane za stavku. Potrebno je registrovati samo one parametre koje planirate da koristite u izveštajima — nije potrebno registrovati svih 18.
...
Ovaj događaj ne sadrži `ecommerce` objekat.
