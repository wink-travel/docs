...
- [Payment Web Component](/sr/integrations/web-components/#payment).
- [Payment API](/sr/api). *Pročitajte odeljak o `Create payable contracts`*

Evo koraka za uspešno korišćenje TripPay-a za plaćanje rezervacije:

<Steps>
1. Uverite se da su korisnici kojima se isplaćuje novac u ugovoru o rezervaciji mapirani.
...
- Koristite `traceId` da grupišete više rezervacija kod više dobavljača. Na ovaj način možete otkazati grupnu rezervaciju.
...
- `identifier` je važeći UUID koji generišete.
- Dobavljača od kojeg želite da rezervišete inventar.
...
- Svaki unos u `contractItemList` sadrži:
    ...
    - **Plaćanje** Kada treba naplatiti putnika. *Trenutno podržavamo samo opcije za trenutno plaćanje.*
...
Kada TripPay odgovori sa payable contract-om, primetićete da je ubacio svog korisnika sa 4,0%.
:::
