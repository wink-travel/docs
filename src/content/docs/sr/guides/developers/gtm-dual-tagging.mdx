...
Ako vodite oglasne kampanje preko Sojern-a, možete povezati vaš Sojern nalog sa Wink-om unošenjem vašeg Sojern GTM Container ID u Wink portal. Kada je povezano, Sojern-ov tracking pixel se automatski aktivira za vašu nekretninu — pokrivajući ceo put gosta od pretrage do završene rezervacije.
...
2. **Unesite ga u Wink portal**
   ...
   Prijavite se na [app.wink.travel](https://app.wink.travel), izaberite vaš nalog i idite na karticu **Customizations**. Nalepite vaš Sojern GTM Container ID u polje za Sojern i sačuvajte. Vaša integracija je sada aktivna.

3. **Potvrdite sa vašim Sojern menadžerom naloga**

   Obavestite vašeg Sojern menadžera naloga da je integracija aktivna. Oni mogu sa svoje strane proveriti da li se pixel ispravno aktivira na `wink.travel` za vašu nekretninu i da li podaci stižu u vaš Sojern nalog.

</Steps>

<Aside type="note">
Ne morate postavljati nikakav pixel kod na vašem sajtu za ovu integraciju. Wink automatski učitava vaš Sojern container na svim stranicama gde je vaša nekretnina aktivna.
...
- **Polje za Sojern GTM Container ID** dostupno je u kartici Customizations vašeg Wink portala. Kada unesete Container ID, Wink proverava format i sigurno ga čuva povezano sa vašom nekretninom.
- **Na svakoj stranici gde je vaša nekretnina aktivna** na Wink-u, vaš Sojern GTM Container ID se ubacuje u stranicu i Wink-ov sistem za upravljanje tagovima dinamički učitava vaš Sojern container zajedno sa svojim. Ovo se dešava automatski — nisu potrebni ručni koraci od Wink tima po nekretnini.
- **Vaš Sojern container se učitava asinhrono**, što znači da ne utiče na performanse učitavanja stranice za vaše goste.
- **Wink-ovo sopstveno praćenje i vaš Sojern pixel rade paralelno**, čitajući iz istih zajedničkih podataka o događajima. Svaki događaj rezervacije koji Wink prati je istovremeno dostupan vašem Sojern containeru.
- **Kada vaša nekretnina nije pregledana** — na primer, kada gost pretražuje drugi hotel na Wink-u — vaš Sojern container se ne učitava. Ovo osigurava da Sojern prima samo podatke relevantne za vašu nekretninu.

---

## Referenca DataLayer-a

Wink šalje GA4-kompatibilne događaje u `dataLayer` tokom celog puta gosta. Vaš GTM container čita iz istog zajedničkog `dataLayer`-a, tako da svaki okidač ili promenljiva koju konfigurišete ima pristup svim svojstvima opisanima ispod.

Svaki događaj prati standardnu GA4 Enhanced Ecommerce strukturu: string `event` na vrhu i objekat `ecommerce`. Pre svakog slanja, Wink briše prethodni ecommerce payload da spreči zastarele podatke da utiču na nove okidače.
...
`transaction_id` odgovara događaju `purchase` za istu rezervaciju. `coupon`, `shipping` i `tax` su prisutni samo kada su primenjivi.
