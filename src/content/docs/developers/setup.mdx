...
<Steps>
1. [Register](/getting-started/sign-up) your user account on Wink.
2. [Create an affiliate account](/portal/create-account). You'll need this to bind your Application to. See [what is Studio](/portal/studio/what-is-studio) for more context.
3. [Create an Application](/account/applications) and associate it with the affiliate account you created in Step 2. For server-to-server API integrations, choose **MACHINE_2_MACHINE** as the client type.
4. Once you've created your `Application`, copy and save the client-id and secret key immediately — the secret key is only shown once and cannot be retrieved again.
</Steps>
...
Head on over to our [APIs](/api/overview/) section and choose the API you want to work with.
...
<Steps>
1. Use the [Lookup API](https://developer.wink.travel/lookup) to retrieve destinations or hotels.
...
2. Click on `Download OpenAPI spec` from the top of the [Lookup API](https://developer.wink.travel/lookup) page.
...
13. Set the field `Scope` to the space-delimited scopes your integration needs (e.g. `inventory.read inventory.write inventory.remove`). See the [scope reference](/developers/authentication#scopes) for the full list.
