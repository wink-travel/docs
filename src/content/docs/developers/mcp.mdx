...
For the complete traveler booking flow, connect **Wink Booking Engine** at `https://api.wink.travel/mcp/booking-engine` and a compatible **payment wallet**. Adding the Wink MCP opens browser sign-in and permission selection; your client then handles authentication.

Booking Engine includes destination lookup, hotel and room search, `agentic_booking_quote`, `agentic_booking_pay`, `agentic_booking_book`, booking retrieval and receipts, when agentic booking and payment are enabled for that environment. See [Agentic Payments](/developers/agentic-payments/) for connection setup, the permissions to select and the steps from search to confirmation.
...
| **Booking a room with an AI agent** | `/mcp/booking-engine` + a compatible payment wallet — see [Agentic Payments](/developers/agentic-payments/) |
| **Building a booking experience** | `/mcp/booking-engine`; optionally `/mcp/reference` for additional lookups |

**Booking Engine includes the core booking workflow.** It provides account and customization discovery, destination lookup, hotel availability and room-rate details, shopping carts, checkout, and traveller booking management. Agentic booking and payment tools are also included when enabled for that environment.

For a city-based hotel search, use `destination_lookup_search_suggestions` to resolve the destination, then `inventory_search_city` to find available hotels and rates. Both tools are in `/mcp/booking-engine`, so this flow does not require a Reference connection.

Add `/mcp/reference` when you need supporting lookups such as countries, currencies and exchange rates, supported languages, timezones, broader geographic searches, OTA classifications, or public property knowledge. These are optional additions to the booking flow.
