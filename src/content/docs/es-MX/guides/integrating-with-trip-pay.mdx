...
- [Payment Web Component](/es-MX/integrations/web-components/#payment).
...
### Solicitud
Aquí hay un ejemplo de solicitud JSON que incluye todo lo que necesitas para crear tu primer contrato pagable.
...
El arreglo `contractList` contiene el/los artículo(s) que el viajero quiere comprar de varios proveedores. Cada artículo contiene:
...
- Cada entrada bajo `contractItemList` contiene:
    ...
    - **Beneficiarios** Incluye qué cuenta(s) de TripPay deben recibir qué monto(s) del precio del artículo. *Se soportan montos fijos / porcentajes.*
