---
title: Servidor MCP
description: Conecta rápidamente agentes de IA a la documentación de Wink y a especificaciones OpenAPI en vivo a través del servidor MCP alojado.
sidebar:
  order: 10
---
...
Configuración: Agrega servidor remoto vía configuración MCP.
```json
{
  "mcpServers": {
    "wink-docs": {
...
|-------------|-----------|------------------------------|
| `api_search` | Encontrar la API o operación en vivo para integrar | `crear afiliado`, `recibo de pago`, `consultar inventario`, `invitar gerente` |
| `docs_search` | Encontrar documentación conceptual / guías | `autenticación`, `webhooks`, `wordPress` |
...
1. Ejecuta `api_search` con tu intención (por ejemplo *"crear afiliado"* o *"recibo de pago"*).
