...
4. Ingresa la URL del servidor Wink Inventory:
   ```
...
:::note
El soporte MCP en Windsurf requiere la versión **1.4** o superior. Verifícalo en **Ayuda → Acerca de Windsurf** y actualiza desde el sitio web de Windsurf si es necesario.
:::
...
Cuando apruebas la pantalla de consentimiento verás un desglose de lo que el agente está solicitando. Los clientes MCP usan el mismo vocabulario de permisos que el resto de la plataforma — consulta la referencia completa de
[alcances en la página de Autenticación](/es-MX/developers/authentication#scopes).
...
Has iniciado sesión en una cuenta Wink que no tiene entidad gestora. Ve a [Wink Portal](https://app.wink.travel) y asegúrate de que tu cuenta esté configurada antes de usar el servidor MCP.
