...
consulta los ejemplos de [`curl`](#staging) arriba y el [recorrido en Postman](/es-MX/developers/setup#postman).
...
| **MCP** | `mcp.read` `mcp.write` `mcp.remove` | Abre el [transporte MCP](/es-MX/developers/mcp) (`/mcp`) para agentes de IA | Bajo–Medio |

:::note
La columna **Riesgo** refleja la sensibilidad: `read` es el nivel más bajo y `write`/`remove` el más alto.
**Payment** y **Accounting** protegen el movimiento de dinero, por lo que tienen un riesgo elevado.
:::

Algunas cosas para tener en cuenta:

- **Solicita lo mínimo.** Pide solo los scopes que tu integración realmente usa — un token emitido
  con acceso más amplio del necesario tiene un mayor impacto si se filtra.
- **Los scopes desconocidos son rechazados.** Solicitar un scope para el que tu aplicación no está registrada, o uno
  que no existe, falla en la emisión del token.
- **`mcp.*` es solo para clientes agentes de IA.** Lo necesitas para abrir el transporte MCP; no es requerido
  para llamadas regulares a la API REST. Consulta [Model Context Protocol](/es-MX/developers/mcp).

:::tip
Puedes probar todas nuestras APIs sin escribir una sola línea de código antes de comenzar.

Obtén un 'Client ID' y 'Secret Key' de una de tus aplicaciones y navega a cualquiera de nuestras [APIs](/es-MX/api/overview/).
:::
