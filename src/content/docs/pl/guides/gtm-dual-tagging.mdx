...
- **Pole identyfikatora kontenera GTM Sojern** jest dostępne w zakładce Dostosowania w portalu Wink. Po wprowadzeniu identyfikatora kontenera, Wink weryfikuje jego format i bezpiecznie przechowuje go powiązanego z Twoją nieruchomością.
...
- **Gdy Twoja nieruchomość nie jest wyświetlana** — na przykład, gdy gość przegląda inny hotel na Wink — Twój kontener Sojern nie jest ładowany. Zapewnia to, że Sojern otrzymuje tylko dane istotne dla Twojej nieruchomości.

---

## Odwołanie do DataLayer

Wink wysyła zdarzenia kompatybilne z GA4 do `dataLayer` przez całą podróż gościa. Twój kontener GTM odczytuje z tego samego współdzielonego `dataLayer`, więc każdy wyzwalacz lub zmienna, którą skonfigurujesz, ma dostęp do wszystkich właściwości opisanych poniżej.
...
| `num_perks` | number | Liczba dodatków wliczonych w stawkę |
...
GA4 ignoruje niestandardowe parametry przedmiotów w raportach, dopóki nie zarejestrujesz ich wyraźnie jako niestandardowe wymiary na poziomie przedmiotu. Musisz zarejestrować tylko te parametry, które zamierzasz używać w raportach — nie musisz rejestrować wszystkich 18.
...
GA4 pozwala na maksymalnie 50 niestandardowych wymiarów na poziomie przedmiotu na standardową właściwość (właściwości 360 mają wyższy limit).
</Aside>

Użyj poniższej tabeli jako odniesienia podczas tworzenia niestandardowych wymiarów. Wartość **Parametru zdarzenia** musi być dokładnie zgodna — GA4 rozróżnia wielkość liter.
