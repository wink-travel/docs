...
Ten przewodnik jest przeznaczony dla programistów integrujących Wink z systemami zewnętrznymi, takimi jak systemy zarządzania nieruchomościami (PMS), channel managerowie, CRM-y lub niestandardowe pulpity nawigacyjne.
...
3. Twój serwer przetwarza ładunek i odpowiada `200 OK`.
...
Pełna, generowana lista — z opisem, odbiorcą i linkiem do strony referencyjnej każdego zdarzenia — to **Katalog Zdarzeń Webhooków**. Strona referencyjna dla każdego zdarzenia (ciało JSON, nagłówki, polityka ponawiania) znajduje się w [Webhooks API](/pl/api/webhooks/).
...
- `id` — identyfikator zdarzenia; identyczny dla każdego punktu końcowego Twojego konta, który otrzymuje to zdarzenie oraz dla każdej próby ponownej wysyłki. Używaj go jako **klucza idempotencji**.
...
Każdy schemat ładunku jest dokumentowany dla zdarzenia w referencji [Webhooks API](/pl/api/webhooks/).
...
| `Wink-Delivery-Attempt` | Numer próby dostarczenia, zaczynając od 1. |
...
gdzie `t` to znacznik czasu Unix (sekundy), a `v1` to małe litery szesnastkowe **HMAC-SHA256** ciągu
`t + "." + rawBody`, kluczowane Twoim sekretem, a `rawBody` to dokładne bajty ciała żądania, jakie otrzymałeś — nie serializuj ponownie JSON przed weryfikacją. Przez 24 godziny po rotacji sekretu nagłówek zawiera drugi podpisany `v1=` z poprzednim sekretem; zaakceptuj dostawę, jeśli **którykolwiek** `v1` pasuje.
...
Obróć sekret z poziomu portalu lub za pomocą `POST /api/managing-entity/{id}/webhook/{webhookId}/rotate-secret`; odpowiedź pokaże nowy sekret raz, a stary będzie ważny przez 24 godziny, podczas gdy go wdrażasz.
...
- `5xx`, timeout, `408` lub `429` są ponawiane z opóźnieniem: po 1 minucie, 5 minutach, 30 minutach, 2 godzinach, 6 godzinach, 12 godzinach, potem codziennie — 10 prób przez około 3 dni — po czym dostawa jest oznaczana jako **martwa**.
...
`POST /api/managing-entity/{id}/webhook/{webhookId}/test`. Jest podpisane i dostarczane dokładnie jak prawdziwe zdarzenie, więc możesz zweryfikować swój punkt końcowy, sprawdzenie podpisu i obsługę idempotencji przed subskrypcją na zdarzenia na żywo.

## Najlepsze praktyki

- **Używaj HTTPS** — Wink wysyła ładunki tylko do punktów końcowych HTTPS.
- **Odpowiadaj szybko** — Zwróć `200 OK` zaraz po otrzymaniu ładunku. Ciężkie przetwarzanie wykonuj asynchronicznie.
...
- **Logowanie** — Loguj każdy otrzymany ładunek webhooka. Ułatwia to debugowanie problemów z integracją.
...
- [Katalog Zdarzeń Webhooków](/pl/developers/webhook-events) — Wszystkie typy zdarzeń, generowane z katalogu platformy.
- [Referencja Webhooks API](/pl/api/webhooks/) — Schematy ładunków, nagłówki oraz punkty końcowe zarządzania subskrypcjami i dostawami.
...
- [Developers > APIs](/pl/api/overview/) — Pełna dokumentacja API.
