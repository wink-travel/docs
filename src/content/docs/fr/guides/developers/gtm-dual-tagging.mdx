...
description: Comment connecter votre compte Sojern à Wink afin que Sojern puisse suivre le parcours de réservation de vos clients et alimenter vos campagnes de reciblage et de publicité.
sidebar:
...
1. **Obtenez votre ID de conteneur GTM auprès de Sojern**
   ...
   Contactez votre gestionnaire de compte Sojern et demandez votre **ID de conteneur GTM**. Il suit le format `GTM-XXXXXXX`. Ceci est différent de votre ID de pixel Sojern — assurez-vous de demander spécifiquement l’ID de conteneur GTM.

2. **Saisissez-le dans le portail Wink**

   Connectez-vous à [app.wink.travel](https://app.wink.travel), sélectionnez votre compte, puis allez dans l’onglet **Personnalisations**. Collez votre ID de conteneur GTM Sojern dans le champ Sojern et enregistrez. Votre intégration est maintenant active.
...
Cette configuration fonctionne avec n’importe quel ID de conteneur GTM personnalisé. Nous avons simplement utilisé Sojern comme exemple car c’est une intégration très courante.
</Aside>

---

## Ce que fait Wink de son côté
...
Chaque événement suit la structure standard GA4 Enhanced Ecommerce : une chaîne `event` au niveau supérieur et un objet `ecommerce`. Avant chaque push, Wink vide la charge utile ecommerce précédente pour éviter que des données obsolètes ne contaminent les nouveaux déclencheurs.
...
| 5 | `add_payment_info` | Le client sélectionne un mode de paiement |
...
Se déclenche lorsque le client sélectionne ou confirme un mode de paiement.
