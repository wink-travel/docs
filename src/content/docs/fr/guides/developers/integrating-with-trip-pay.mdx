...
- [Composant Web de Paiement](/fr/integrations/web-components/#payment).
- [API de Paiement](/fr/api). *Lisez la section sur `Créer des contrats payables`*
...
- Lignes : **2 - 7** correspond à l'utilisateur responsable de la réservation. Il existe deux types de réservateurs :
    - Voyageur.
    - Agent de voyage.
- Lignes : **8 - 9** correspond au compte TripPay facilitant la réservation. Il peut s'agir de :
...
- `redirectUrl` informe TripPay de la page vers laquelle rediriger après la fin du paiement.
...
- Chaque entrée sous `contractItemList` contient :
    ...
    - **Bénéficiaires** Inclure quels comptes TripPay doivent recevoir quelle(s) part(s) du prix de l'article. *Les montants fixes / en pourcentage sont supportés.*

:::note
Dans l'exemple JSON ci-dessus, nous avons 3 bénéficiaires :

- 1,5 % ont été alloués à la plateforme qui a facilité la réservation. *ex. Wink*
- 9,85 % ont été alloués à l'affilié en tant que commission. *ex. Kim Kardashian*
- 88,65 % ont été alloués au fournisseur pour la réservation de la chambre. *ex. Hilton*
