...
La majeure partie de ce montant revient au prestataire de paiement (Visa, MasterCard, etc.). Une réservation moyenne nous coûte `2,95 %` à acquérir ; parfois jusqu’à `3,6 %`. Les remboursements partiels nous coûtent encore plus cher car le prestataire de paiement considère cela comme une nouvelle transaction.
Nous souhaitons être totalement transparents sur la tarification dès maintenant pour faire gagner du temps à tout le monde avant d’essayer de négocier des remises supplémentaires sans avoir utilisé Wink. Notre modèle tarifaire est plus que juste et nous devons aussi gagner notre vie.
...
- Sur l’API partenaire, un **hôtel-jour** correspond à un hôtel tarifé pour une nuitée — *pas* un appel API. Une recherche qui retourne 20 hôtels pour un séjour de 3 nuits correspond à 60 hôtel-jours pour une seule requête. Les points de terminaison de recherche et d’autocomplétion sont gratuits et jamais mesurés.
...
Enfin, à mesure que nous continuons de grandir en taille et en nombre de réservations, nous souhaitons pouvoir partager certains effets de plateforme avec vous. Plus de réservations apportent des opportunités de remises sur volume auprès de notre prestataire de paiement, ce qui nous permettrait de réduire nos frais de traitement pour tout le monde.
