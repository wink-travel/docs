...
:::note
Les tests doivent toujours être effectués dans notre [environnement de staging](/fr/getting-started/environments). Il y a beaucoup moins de données, mais vous êtes libre de tester autant que vous le souhaitez sans impacter l’environnement de production.
...
<Steps>
1. [Enregistrez-vous](/fr/my-account/register) en tant qu’utilisateur sur Wink.
2. [Créez un compte affilié](/fr/my-account/create-account). Vous en aurez besoin pour lier votre Application. Consultez [qu’est-ce que Studio](/fr/studio/what-is-studio) pour plus de contexte.
3. [Créez une Application](/fr/my-account/applications) et associez-la au compte affilié que vous avez créé à l’étape 2. Pour les intégrations API serveur-à-serveur, choisissez **MACHINE_2_MACHINE** comme type de client.
...
Rendez-vous dans notre section [APIs](/fr/developers/apis) et choisissez l’API avec laquelle vous souhaitez travailler.
...
:::tip[Astuce pro]
Pour faciliter l’utilisation, nous recommandons aux intégrateurs d’utiliser les points de terminaison que nous désignons comme `OAuth2`. Ces points de terminaison savent qu’ils doivent s’attendre à des appels API et connaissent déjà le compte sur lequel vous êtes, vous n’avez donc pas à spécifier le `companyIdentifier` dans vos URLs.
:::
...
:::tip[Astuce pro]
Il est recommandé de transformer le domaine du serveur, le client ID et le client secret en variables d’environnement. Ainsi, vous pouvez facilement basculer entre les environnements de staging et de production et réutiliser toutes les opérations que vous avez créées.
:::
