...
<Steps>
1. [Enregistrez](/fr/account/register) votre compte utilisateur sur Wink.
2. [Créez un compte affilié](/fr/portal/create-account). Vous en aurez besoin pour lier votre Application. Voir [qu’est-ce que Studio](/fr/portal/studio/what-is-studio) pour plus de contexte.
3. [Créez une Application](/fr/portal/settings/applications) et associez-la au compte affilié que vous avez créé à l’étape 2. Pour les intégrations API serveur à serveur, choisissez **MACHINE_2_MACHINE** comme type de client.
...
Rendez-vous dans notre section [APIs](/fr/api) et choisissez l’API avec laquelle vous souhaitez travailler.
...
:::tip[Astuce pro]
Pour faciliter l’utilisation, nous recommandons aux intégrateurs d’utiliser les endpoints que nous désignons comme `OAuth2`. Ces endpoints savent qu’ils s’adressent à des appelants API et connaissent déjà le compte sur lequel vous êtes, vous n’avez donc pas à spécifier le `companyIdentifier` dans vos URLs.
:::
...
13. Réglez le champ `Scope` sur les scopes délimités par des espaces dont votre intégration a besoin (ex. `inventory.read inventory.write inventory.remove`). Voir la [référence des scopes](/fr/developers/authentication#scopes) pour la liste complète.
...
:::tip[Astuce pro]
Il est recommandé de transformer le domaine du serveur, le client ID et le secret client en variables d’environnement. Ainsi, vous pouvez facilement basculer entre les environnements de préproduction et de production et réutiliser toutes les opérations que vous avez créées.
:::
