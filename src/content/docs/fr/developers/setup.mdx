...
<Steps>
1. [Inscrivez-vous](/fr/getting-started/sign-up) pour créer votre compte utilisateur sur Wink.
2. [Créez un compte affilié](/fr/portal/create-account). Vous en aurez besoin pour lier votre Application. Consultez [qu’est-ce que Studio](/fr/portal/studio/what-is-studio) pour plus de contexte.
3. [Créez une Application](/fr/account/applications) et associez-la au compte affilié que vous avez créé à l’étape 2. Pour les intégrations API serveur à serveur, choisissez **MACHINE_2_MACHINE** comme type de client.
4. Une fois votre `Application` créée, copiez et sauvegardez immédiatement le client-id et la clé secrète — la clé secrète n’est affichée qu’une seule fois et ne peut pas être récupérée ultérieurement.
</Steps>
...
Rendez-vous dans notre section [APIs](/fr/api/overview/) et choisissez l’API avec laquelle vous souhaitez travailler.
...
:::tip[Astuce pro]
Pour plus de simplicité, nous recommandons aux intégrateurs d’utiliser les points de terminaison que nous désignons comme `OAuth2`. Ces points de terminaison savent qu’ils s’adressent à des appelants API et connaissent déjà le compte sur lequel vous êtes, vous n’avez donc pas à spécifier le `companyIdentifier` dans vos URLs.
:::
...
3. Saisissez votre client-id et votre clé secrète, puis cliquez sur le bouton `Get TOKEN`.
...
<CloudinaryImage
      ...
      resize="scale"
      quality="best"
      alt="Point de terminaison API Lookup"
...
8. Donnez un nom au champ `Token Name` pour nommer votre jeton sauvegardé dans Postman, par exemple Wink Staging.
...
12. Réglez le champ `Client Secret` sur la clé secrète générée par votre application.
13. Réglez le champ `Scope` sur les scopes délimités par des espaces dont votre intégration a besoin (par exemple `inventory.read inventory.write inventory.remove`). Consultez la [référence des scopes](/fr/developers/authentication#scopes) pour la liste complète.
14. Réglez le champ `Client Authentication` sur `Send as Basic Auth header`.
15. Cliquez sur le bouton `Get New Access Token` pour récupérer votre jeton et le sauvegarder.
16. Pour tous les appels API sous la collection Wink API, assurez-vous que l’onglet d’authentification indique `Inherit auth from parent` afin d’inclure votre jeton à chaque appel.
</Steps>

:::tip[Astuce pro]
Il est recommandé de transformer le domaine du serveur, le client ID et la clé secrète en variables d’environnement. Ainsi, vous pouvez facilement basculer entre les environnements de préproduction et de production et réutiliser toutes les opérations que vous avez créées.
:::
