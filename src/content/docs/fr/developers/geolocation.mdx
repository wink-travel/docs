...
Une coordonnée géographique unique (latitude, longitude). Utilisée pour marquer un emplacement spécifique — par exemple, l'adresse d'une propriété ou la localisation actuelle d'un utilisateur.
...
| Recherche dans la zone visible de la carte | Rectangle | Propriétés visibles dans la vue actuelle de la carte |
| Recherche par quartier | Polygone | Hôtels dans un arrondissement spécifique |
| Localisation d'une propriété | Point | Emplacement exact d'une annonce immobilière |

## Pour les développeurs

Les paramètres de requête spatiale sont acceptés par les points d'accès Lookup API et Inventory API. Passez la forme GeoJSON appropriée dans votre requête de recherche pour filtrer les résultats par localisation.

Consultez la [documentation des APIs](/fr/api/overview/) pour les détails spécifiques à chaque point d'accès.

## Lectures complémentaires

- [Recherche (Booking Engine)](/fr/booking-engine/search) — Comment les voyageurs recherchent l'inventaire.
- [Cartes (Studio)](/fr/portal/studio/maps) — Découverte d'inventaire basée sur la carte.
- [APIs](/fr/api/overview/) — Référence complète des API.
