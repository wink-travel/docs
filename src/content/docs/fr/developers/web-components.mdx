...
Des exemples sont disponibles pour les [cartes](/studio/cards), [grilles](/studio/grids) et [cartes géographiques](/studio/maps).
...
:::note
Pour apprendre à générer l’`id` du contrat de réservation, lisez le guide : [Intégration avec TripPay](/guides/integrating-with-trip-pay).
:::
...
L’inventaire peut ne plus être disponible. Allez sur [Wink Studio](/studio/cards) et vérifiez si le statut de la « carte » est disponible.
...
L’inventaire peut avoir été retiré. Allez sur [Wink Studio](/studio/cards) et vérifiez si le statut de la « carte » est disponible.
S’il ne l’est pas, il apparaîtra en rouge. Dans ce cas, vous pouvez attendre et peut-être que le fournisseur le rende à nouveau disponible ou le retirer de votre liste.
...
Ceci est uniquement pour les développeurs web. Si vous avez accidentellement supprimé l’[Application](/developer/applications) ; rendant l’ID client indisponible, veuillez créer une nouvelle application et utiliser le nouvel ID client pour intégrer nos Composants Web.
