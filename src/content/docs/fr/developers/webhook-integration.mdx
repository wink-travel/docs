...
2. Lorsqu’un événement survient (par exemple, une nouvelle réservation), Wink envoie un HTTP POST à votre URL.
...
- `id` — identifiant de l’événement ; identique pour chaque point de terminaison de votre compte recevant cet événement et pour chaque réessai. Utilisez-le comme votre **clé d’empotence**.
- `type` — clé du type d’événement (également envoyée dans l’en-tête `Wink-Event-Type`). Branchez-vous sur `type` et `schemaVersion` pour analyser `object`.
...
| `Wink-Event-Id` | Identique à `id` dans le corps — votre clé d’empotence. |
...
Vérifiez en quatre étapes : analysez `t` et chaque `v1` ; recalculer le HMAC sur `t.rawBody` avec votre secret ; comparez avec une comparaison en temps constant ; rejetez si `|maintenant − t|` dépasse votre tolérance (5 minutes recommandées).

```js
// Node.js (style Express ; assurez-vous d’avoir le corps BRUT, pas un objet analysé)
...
- **Empotence** — Votre gestionnaire doit être idempotent ; dédupliquez sur `Wink-Event-Id`. Wink réessaie si un code `2xx` n’est pas reçu.
- **Validez la source** — Vérifiez l’en-tête `Wink-Signature` (voir [Vérification des signatures](#verifying-signatures)) avant de traiter ; rejetez toute requête échouant à la vérification.
...
Vous pouvez **désactiver** un webhook sans le supprimer. Cela suspend la livraison pour vous permettre de résoudre les problèmes sans perdre votre configuration. Lorsque vous êtes prêt, réactivez-le.
...
- [Référence API Webhooks](/fr/api/webhooks/) — Schémas de charge utile par événement, en-têtes, et points de terminaison de gestion des abonnements/livraisons.
...
- [Développeurs > APIs](/fr/api/overview/) — Documentation complète de l’API.
