...
- `read` — consulter les ressources (requêtes sûres `GET`)
- `write` — créer et mettre à jour les ressources (`POST` / `PUT` / `PATCH`)
- `remove` — supprimer les ressources (`DELETE`)
...
- **Demandez le minimum.** Ne demandez que les scopes que votre intégration utilise réellement — un jeton émis avec un accès plus large que nécessaire présente un plus grand risque en cas de fuite.
...
Récupérez un 'Client ID' et une 'Secret Key' depuis l’une de vos applications et naviguez vers n’importe laquelle de nos [APIs](/fr/api/overview/).
:::
