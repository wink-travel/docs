...
2. **Bunu Wink Portalına Girin**
   ...
   [app.wink.travel](https://app.wink.travel) adresine giriş yapın, hesabınızı seçin ve **Özelleştirmeler** sekmesine gidin. Sojern alanına Sojern GTM Container ID'nizi yapıştırın ve kaydedin. Entegrasyonunuz artık aktif.

3. **Sojern Hesap Yöneticinizle Onaylayın**

   Entegrasyonun aktif olduğunu Sojern hesap yöneticinize bildirin. Onlar, mülkünüz için `wink.travel` üzerinde pikselin doğru şekilde tetiklendiğini ve verilerin Sojern hesabınıza aktığını doğrulayabilirler.

</Steps>

<Aside type="note">
Bu entegrasyon için kendi web sitenize herhangi bir piksel kodu yerleştirmeniz gerekmez. Wink, mülkünüzün aktif olduğu tüm sayfalarda Sojern konteynerinizi otomatik olarak yükler.
...
- **Sojern konteyneriniz asenkron olarak yüklenir**, yani misafirlerinizin sayfa yükleme performansını etkilemez.
...
Misafir rezervasyon hunisinde ilerledikçe bu etkinlikler otomatik tetiklenir. GTM tetikleyicilerinizi aşağıdaki etkinlik isimleriyle eşleşen **Özel Etkinlik** tetikleyicileri olarak yapılandırın.
...
|----------|------|----------|-------------|
| `item_id` | string | Evet | Oda veya ürün için benzersiz kimlik |
| `item_name` | string | — | Oda veya ürün adı |
...
Bu parametreler Wink'e özgü rezervasyon bağlamını taşır. GA4 bunları otomatik olarak raporlarda göstermez — görünmeleri için her birini GA4 mülkünüzde **ürün kapsamlı özel boyut** olarak kaydetmeniz gerekir. Kurulum için aşağıdaki [GA4'te Özel Boyutlar](#custom-dimensions-in-ga4) bölümüne bakın.

| Özellik | Tür | Açıklama |
|----------|------|-------------|
| `property_id` | string | Wink mülk kimliği |
| `property_name` | string | Mülk görüntü adı |
...
| `userId` | Kimlik doğrulanmış kullanıcı kimliği (giriş yapılmadıysa yok) |
