---
title: Fiyatlandırma
description: Wink'in tüm hizmetlerini gönlünüzce ücretsiz kullanın. Sadece yapılan her rezervasyon için küçük bir ücret ödersiniz.
sidebar:
  order: 4
---

Wink'in tüm hizmetleri herkesin ücretsiz kullanımına açıktır. Rezervasyon yapıldığında, Wink platformu sürdürmek için küçük bir ücret alır.

Wink hem satıcı hem de acente modellerini destekler.

## Model 1

Rezervasyon gerçekleştiğinde Wink kayıtlı satıcıdır. Wink, fonlardan ve seyahat acentesi işletmek için gerekli tüm lisanslardan sorumludur.  
Bu model tüm rezervasyonların %95'ine uygulanır.

Wink platformu sürdürmek için `rezervasyon başına %5,5` ücret alır.  
Bunun çoğu ödeme geçidi (Visa, MasterCard vb.) ücretlerine gider. Ortalama bir rezervasyonun edinme maliyeti bize ` %2,95` civarındadır; bazen ` %3,6` kadar yüksek olabilir. Kısmi iadeler bizi daha da zorlar çünkü ödeme geçidi bunu yeni bir ücret olarak görür.  
Fiyatlandırma konusunda tamamen şeffaf olmak istiyoruz, böylece Wink'i kullanmadan önce daha fazla indirim pazarlığı yapmaya çalışırken herkesin zamanını kurtarırız. Fiyatlandırma modelimiz oldukça adildir ve bizim de geçimimizi sağlamamız gerekiyor.

### Dağılım

:::note[İşlem ücreti]
Wink, rezervasyon başına %1,5 işlem ücreti alır. Bu, tüm hizmetlerimizi ücretsiz sunmamızı sağlar ve platform bakımını karşılar.
:::

:::note[Ödeme ücreti]
TripPay, rezervasyon başına %4,0 ödeme ücreti alır. Bu, yolcudan ödemenin tahsil edilmesini kapsar.
:::

:::note[Fon dağıtımı]
Hesabınıza para gönderirken ücretler vardır. Bu, seçtiğiniz ödeme yöntemine bağlıdır. Şu anda desteklediğimiz yöntemler:

- **Banka havalesi** Ücret, bulunduğunuz ülkeye ve fonların gönderildiği yere bağlıdır. Hesabınızda kullanılabilir fon olduğunda kullanabileceğiniz bir teklif hesaplayıcı sunuyoruz.

Başka bir ödeme yöntemini desteklememizi isterseniz, bize e-posta gönderin.
:::

## Model 2

Bu model yalnızca kendi bölgesinde seyahat acentesi lisansına sahip ve kayıtlı satıcı olmak isteyen seyahat acentelerine açıktır. Bazı kayıtlı seyahat acentelerimiz, ödemeyi yönetmek ve fonları otellere dağıtmakla sorumlu olmak ister. Bu modelde, fonlardan sorumludurlar ve ülkelerinde faaliyet göstermek için gerekli lisanslara sahiptirler.

### Dağılım

:::note[İşlem ücreti]
Wink, rezervasyon başına %1,5 işlem ücreti alır. Bu, tüm hizmetlerimizi ücretsiz sunmamızı sağlar ve platform bakımını karşılar.
:::

Bu modeli kullanan seyahat acenteleri sadece Wink'in işlem ücretini öder ve Wink seyahat acentesine aylık fatura keser.

## Abonelikler

Mülklerin satış ve pazarlama ekiplerini güçlendirmelerine yardımcı olmak için isteğe bağlı bir abonelik hizmeti sunuyoruz (bkz. [Social](/tr/portal/social/what-is-social)).

## Platform etkisi

Son olarak, hem büyüklük hem de rezervasyon sayısı arttıkça, platform etkilerinden bir kısmını sizinle paylaşmak istiyoruz. Daha fazla rezervasyon, ödeme geçidimizden toplu indirim fırsatları getirir ve bu da işlem ücretimizi herkes için düşürmemize olanak sağlar.

Bugün Wink'e katılın ve konaklama sektöründe yeni, karlı bir iş yapma yolunu keşfedin!
