...
Wink bugün rezervasyonlar, mülkler, hesaplar (yönetim birimleri) ve envanter (oda tipleri, fiyat planları, ana fiyatlar, ek hizmetler, tesisler, satış kanalları, promosyonlar) genelinde 70 webhook olay türü yayınlamaktadır. Yaygın olanlar:
...
Tam, oluşturulmuş liste — açıklama, alıcı bilgisi ve her olayın referans sayfasına bağlantı ile — **Webhook Olayları Kataloğu**dur. Her olayın referans sayfası (JSON gövdesi, başlıklar, yeniden deneme politikası) [Webhooks API](/tr/api/webhooks/) adresinde bulunur.
...
Her teslimat, webhook URL'nize `Content-Type: application/json` ile bir HTTP `POST` ve şu zarf yapısıyla gelir:
...
- `object` — olayla ilgili kaynağın özetlenmiş hali (tanımlayıcılar, durum, işlem yaptığınız alanlar) ve `links.self`, tedarikçi tarafındaki tam kaynağın canonical REST URL'si. Özetden fazlasına ihtiyaç duyduğunuzda kendi API kimlik bilgilerinizle alın; olayı bir bayi veya seyahat acentesi olarak alıyorsanız, aynı tanımlayıcı için kendi API yüzeyinizdeki ilgili kaynak uç noktasını kullanın.
...
| Başlık | Anlamı |
|---|---|
| `Wink-Version` | Wire sözleşme versiyonu, `2.0`. |
| `Wink-Event-Id` | Gövdedeki `id` ile aynı — idempotency anahtarınız. |
| `Wink-Delivery-Id` | Olay başına uç nokta bazında benzersiz; yalnızca yeniden teslim ederseniz değişir. |
...
Burada `t` Unix zaman damgası (saniye), `v1` ise gizli anahtarınızla anahtarlanmış, küçük harfli hex formatında **HMAC-SHA256** imzasıdır. İmza, `t + "." + rawBody` stringinin HMAC'sidir ve `rawBody` tam olarak alınan istek gövdesi baytlarıdır — doğrulamadan önce JSON'u yeniden serileştirmeyin. Gizli anahtar döndürme işleminden sonraki 24 saat boyunca başlık, önceki gizli anahtarla imzalanmış ikinci bir `v1=` değeri taşır; **herhangi bir** `v1` eşleşirse teslimatı kabul edin.

Dört adımda doğrulayın: `t` ve her `v1`'i ayrıştırın; gizli anahtarınızla `t.rawBody` üzerinde HMAC'i yeniden hesaplayın; sabit zamanlı karşılaştırma yapın; `|şimdi − t|` toleransınızı aşarsa (5 dakika önerilir) reddedin.
...
- Teslimatı onaylamak için 10 saniye içinde herhangi bir `2xx` ile yanıt verin. Ağır işleri asenkron yapın.
...
- **Idempotency** — İşleyiciniz idempotent olmalıdır; `Wink-Event-Id` ile çoğaltmaları önleyin. Wink `2xx` yanıtı alamazsa yeniden dener.
...
- [Webhook Olayları Kataloğu](/tr/developers/webhook-events) — Platform kataloğundan oluşturulmuş tüm olay türleri.
...
- [Geliştiriciler > API'ler](/tr/api/overview/) — Tam API dokümantasyonu.
