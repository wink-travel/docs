...
description: Þessi grein inniheldur fullkomið dæmi um hvernig á að samþætta og láta TripPay sjá um greiðsluna fyrir þig.
sidebar:
  order: 4
---
...
- [Payment Web Component](/is/integrations/web-components/#payment).
- [Payment API](/is/api). *Lestu kaflann um `Create payable contracts`*
...
Við gerum ráð fyrir að þú hafir þegar lokið `Skrefi 1` og `Skrefi 2` og sért tilbúinn að láta TripPay vita af bókuninni.

## Búa til greiðanlegan samning

:::note
Farðu á Payment API til að læra meira um kröfurnar til að búa til greiðanlegan samning.
:::

### Fyrirspurn
Hér er JSON dæmi um fyrirspurn sem inniheldur allt sem þú þarft til að búa til þinn fyrsta greiðanlega samning.
...
Fylkið `contractList` inniheldur þann hlut eða hluti sem ferðalangurinn vill kaupa frá mörgum birgjum. Hver hlutur inniheldur:

- `identifier` er gilt UUID sem þú býrð til.
- Birginn sem þú vilt bóka birgðir frá.
- Birgðirnar sem þú vilt bóka frá þeim birgi. Þau eru öll tilgreind í innra fylkinu `contractItemList`.
- Hver færsla undir `contractItemList` inniheldur:
    ...
    - **Greiðanlegt** Hvenær ferðalangurinn á að greiða. *Við styðjum núna aðeins tafarlausar greiðslumöguleika.*
...
Þegar TripPay svarar með greiðanlegum samningi, munt þú taka eftir að það bætir við sínum eigin þiggjanda með 4,0%.
:::
...
Varðandi samþættingu, þá er þetta allt sem þú þarft að gera. Allt þungavigtarvinnan er unnin af TripPay héðan í frá.
