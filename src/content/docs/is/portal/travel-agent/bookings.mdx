---
title: Bókanir
description: Þessi grein sýnir þér hvernig á að stjórna bókunum sem þú býrð til sem ferðaskrifstofumaður á Wink.
sidebar:
  order: 2
---

import { Steps } from '@astrojs/starlight/components';
import CloudinaryImage from '/src/components/cloudinary-image.astro';

Þegar bókun er gerð á Wink gerist eftirfarandi:

<Steps>
1. Þinn rásastjóri, CRS eða PMS fær tilkynningu.
2. Þú færð staðfestingar tölvupóst *(samband við bókunarborð)* með tengli til að stjórna nýrri bókun þinni.
3. Ferðamaður fær staðfestingar tölvupóst sem inniheldur einnig upplýsingar um hvernig á að hafa samband við bókunarborðið þitt.
4. Ferðaskrifstofumaðurinn fær sömu bókunarstaðfestingu og ferðamaðurinn.
4. Forritarar geta hlustað á `booking.created` webhook atburðinn og fengið ítarlegar bókunarupplýsingar frá okkur í rauntíma.
</Steps>

Fylgdu annað hvort tenglinum í tölvupóstinum eða farðu í `Bookings` úr aðalvalmyndinni til að byrja.

## Bókunargrind

Bókunargrindin sýnir þér allar fyrri og komandi bókanir þínar, raðaðar eftir bókunardegi í hækkandi röð.

### Síur

Þú getur beitt síum til að finna þær bókanir sem þú ert að leita að.

- Bókunardagur [bil]
- Komudagur [bil]
- Brottfarardagur [bil]
- Bókunarkóði
- Eftirnafn ferðamanns
- Fornafn ferðamanns
- Nafn á master-verði
- Nafn eignar

### Útflutningur

Þú getur flutt út bókanir sem uppfylla núverandi síuskilyrði þín í CSV.

Til að flytja út bókanir þínar:

<Steps>
1. Gakktu úr skugga um að viðeigandi síur séu virkar.
2. Bíddu eftir að bókunargrindin uppfærist.
2. Smelltu á 🗂️ (*Skráarútflutningur*) táknhnappinn neðst í hægra horninu á grindinni.
</Steps>

## Bókun

Veldu bókunina sem þú vilt vinna með úr bókunargrindinni og þú verður vísað á `Booking` síðuna.
Þessi síða inniheldur allar þær upplýsingar sem þú þarft til að taka vel á móti gestinum þínum á komudegi.

Síðan inniheldur einnig nokkrar mikilvægar aðgerðir sem þú þarft til að þjónusta gesti þína rétt. Þú finnur allar tiltækar aðgerðir í `Actions` fellivalmyndinni efst á síðunni.

### Endursenda tölvupóst

Til að endursenda staðfestingar tölvupóst til ferðamannsins, smelltu á `Actions > Resend confirmation email`.

### Hætta við bókun

Til að hætta við bókunina sem eign, smelltu á `Actions > Cancel booking`.

Glugginn sem opnast leyfir þér að:

<Steps>
1. Velja tegund hættingar. *t.d. Rangir dagsetningar*
2. Slá inn ástæðu fyrir hættingu.
</Steps>

*Þessi hnappur fylgir reglum þíns hættingarstefnu og verður því ekki alltaf tiltækur.*

## API

Forritarar sem vilja stjórna `Bookings` sem ferðaskrifstofumaður geta farið á [Developers > API > Travel Agent](/is/api/overview/).
