---
title: Netverk
description: Þessi grein sýnir þér hvernig á að kanna og stjórna neti þínu af sölurásum á Wink.
sidebar:
  order: 16
---

import CloudinaryImage from '/src/components/cloudinary-image.astro';

Þessi grein sýnir þér hvernig á að tengjast áhugasömum samstarfsaðilum á Wink og stjórna þeim sem fyrir eru.

## Sölurásir

Til að stjórna sölurásum þínum, smelltu á `Distribution > Sales channels` í aðalvalmyndinni.

<figure>
<CloudinaryImage
      width="350"
      height="402"
      resize="scale"
      quality="best"
      alt="Sölurás"
      src="academy/sales-channel"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*Inngangur í sölurás*</figcaption>
</figure>

Sölurásir eru núverandi samstarfssambönd þín, skilgreind með birgðum, tilboðum og afsláttum sem þær hafa aðgang að.

Til að uppfæra núverandi sölurás, smelltu á `Actions` hlekkinn undir þeirri sölurás sem þú vilt uppfæra.

- **Aðildarafsláttur** Aðildarverðið er gefið ferðalöngum sem bóka í gegnum þessa sölurás.
- **Þóknun** Þóknunin er úthlutað sölurásinni fyrir hverja vel heppnaða bókun.
- **Einstök kynningartilboð** Bættu við þeim kynningartilboðum sem þú vilt að þessi sölurás hafi.
- **Pakkaðar kynningartilboð** Bættu við þeim pakkaða kynningartilboðum sem þú vilt að þessi sölurás hafi.
- **Virkjað** Kveiktu eða slökktu á rofanum til að virkja eða óvirkja sölurásina alveg.
- **Svartlistað** Að merkja sölurás sem svartlistaða gerir það ómögulegt að bóka í gegnum þessa sölurás.

Þegar þú ert ánægður með stillingarnar þínar, smelltu á `Save` hnappinn til að halda áfram.

:::tip
Með því að svartlista sölurás gerir þú það ómögulegt fyrir sölurásina að fá neinar verðskrár frá þér; jafnvel ekki Wink Network-verðskrár þínar.
:::

## Kanna net

Til að finna nýja samstarfsaðila á Wink, smelltu á `Distribution > Explore network` í aðalvalmyndinni.

Í þessum hluta geturðu séð beiðnir sem bíða samþykkis frá samstarfsaðilum efst á síðunni og lista yfir samstarfsaðila til að tengjast neðst.

### Bregðast við beiðni

Þú getur valið að hafna beiðni samstarfsaðila. Ef þú vilt halda áfram með beiðnina, verður þú fluttur á `affiliate details` síðuna þar sem þú getur fengið frekari upplýsingar um samstarfsaðilann auk fyrri árangursmælinga á Wink.

### Leita að samstarfsaðilum

Í samstarfsaðilagrindinni geturðu leitað að samstarfsaðilum eftir nafni, tegund og staðsetningu. Þegar þú sérð einhvern sem þér líkar við, smelltu á hann og þú verður fluttur á `affiliate details` síðuna.

### Tengjast samstarfsaðila

Ef þú velur að stofna beint samband við samstarfsaðila, verður þú beðinn um að skilgreina sambandið á sama hátt og þú myndir stjórna sölurás hér að ofan.

Þegar þú hefur vistað nýju sölurásina, verður þú fluttur aftur á listann þinn yfir sölurásir.

## API

Forritarar sem vilja stjórna `Network` geta farið á [Developers > APIs > Distribution](/is/developers/apis/#distribution-api).
