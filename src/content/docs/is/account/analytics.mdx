---
title: Greiningar
description: Þessi grein sýnir þér hvernig á að stjórna greiningum þínum á Wink Travel Platform.
sidebar:
  order: 14
---

import CloudinaryImage from '/src/components/cloudinary-image.astro';
import { Steps, Badge } from '@astrojs/starlight/components';

Þekking er lífsblóð Wink. Hún segir þér hvernig allir og allt eru að standa sig á Wink, niður í klukkustund. Áður en Wink kom til, gerðu hótel samninga við þriðja aðila og greiddu þeim mikið fé til að fá aðgang að því sem tekjustjórnunarteymið kallar `the comp set`; sem þýðir: `Hvernig stendur eign mín sig miðað við aðra eign?`.

Á Wink hefur þú aðgang að greiningum á vettvangsstigi án endurgjalds. Þetta gerir þér kleift að fylgjast með frammistöðu hvers tengils, ferðaskrifstofu eða eignar á hvaða mælikvarða sem þú vilt... eins mikið og þú vilt. Þú getur einnig fengið aðgang að víðtækari þróun eftir svæðum til að sjá hvaða tegund af vöru er seld hvar og hvenær. Í réttum höndum getur þessi innsýn bætt söluhæfileika hvers Wink notanda.

## Aðgangur að myndritum

Þú getur fengið aðgang að myndritum þínum frá hvaða vefsíðu okkar sem er. Til að fá aðgang að greiningarmyndritum þínum, fylgdu þessum skrefum:

<Steps>
1. Þú verður að vera skráður inn á einni af vefsíðum okkar.
2. Smelltu á prófílmyndina þína, efst í hægra horninu, og valmynd birtist.
3. Smelltu á `Analytics` hlekkinn.
4. Þú verður vísað á myndritin þín.
</Steps>

:::note
Sjálfgefið búum við til einföld myndrit fyrir hverja eign, ferðaskrifstofu og tengilreikning sem þú stofnar svo þú getir auðveldlega fylgst með frammistöðu eigin reikninga.
:::

## Búa til myndrit

Þegar þú vilt byrja að búa til eigin myndrit, farðu á myndritin þín eins og lýst er að ofan og smelltu á `Create chart` hnappinn.

Hér að neðan eru einföld skref til að búa til greiningarmyndrit, fylgt eftir með ítarlegri leiðbeiningum.

<Steps>
1. **Tímaraðir** Veldu tímaraðir fyrir myndritið þitt. *Dæmi: Daglegt*.
2. **Tegund eftirfylgni** Veldu hvort þú viljir fylgjast með föstu tímabili eða samfellt. *Dæmi: `Continuous`*.
    - Samfellt: Veldu tímareiningar. *Dæmi: 7 dagar*
    - Fast: Sláðu inn dagsetningu / tímabil *Dæmi: 22. sept. 2023 16:00 - 29. sept. 2023 16:00*.
3. **Sýna gjaldmiðil** Ef þú ert að vinna með upphæðir í myndritinu, veldu hvaða gjaldmiðla þú vilt sjá þær í. *Dæmi: USD*.
4. **Fylgjast með eign** Fylgstu með ákveðinni eign. *Valfrjálst*
5. **Fylgjast með sölurás** Fylgstu með staðsetningu sölurásar eftir heimsálfu, landi eða borg. *Valfrjálst*
6. **Sía eftir tegund sölurásar** Síðu eftir ákveðinni tegund sölurásar. *Dæmi: Áhrifavaldur*. *Valfrjálst*
7. **Fylgjast með ákveðinni sölurás** Fylgstu með ákveðinni sölurás. *Valfrjálst*
8. **Fylgjast með ferðamanni** Fylgstu með staðsetningu ferðamanns eftir heimsálfu, landi eða borg. *Valfrjálst*
9. **Velja gagnapunkta** Veldu þá gagnapunkta sem þú vilt fylgjast með. *Dæmi: Bókanir & Hætt við*
10. **Hópa gögn** Í stað þess að sía eftir eignum eða sölurásum getur þú líka hópað til að safna saman gagnapunktum. *Dæmi: Hópa eftir sölurásum*
11. **Nafn myndrits** Gefðu myndritinu lýsandi nafn. *Dæmi: Travelbug bókanir á klukkustundargrunni*
12. Smelltu á `Save` hnappinn.
</Steps>

## Hvernig það virkar

Myndrit eru unnin í 4 skrefum. Þessi skref eru útskýrð hér að neðan.

### Tegund tímaraðar

Með því að velja fast tímabil eða samfellt tímabil byrjarðu að einbeita þér að HVENÆR þú vilt sjá gögn frá.

### Eftirfylgni / Síun

Með því að velja eign, sölurás o.s.frv., byrjarðu að einbeita þér að HVEM þú vilt sjá gögn frá. Þú ert í raun að sía út allt sem þú vilt ekki sjá.

### Gagnapunktar

Með því að velja gagnapunkta byrjarðu að einbeita þér að HVAÐ þú vilt sjá gögn um.

:::note
Gagnapunktarnir ættu að vera 'samverkandi' þannig að þeir passi saman á sama myndriti. Til dæmis myndir þú ekki hafa 'Síðuskoðanir' og 'Heildar hreinar tekjur' á sama myndriti.
:::

### Hópun <Badge text="Nýtt" variant="note" />

Hópun er öflug leið til að safna saman og einbeita sér að gögnum.

:::tip
Til dæmis, ef þú ert eign og vilt fylgjast með hversu margar bókanir hver sölurás þín gerði, myndir þú sía eftir eign þinni (að ofan), velja að fylgjast með 'Bókunum' (að ofan) og þú myndir hópa eftir 'Sölurás' í hópunarhlutanum.
:::

<figure>
<CloudinaryImage
      width="700"
      height="219"
      resize="scale"
      quality="best"
      alt="Greiningarmyndrit"
      src="academy/analytics"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*Myndrit með hópun eftir sölurás*</figcaption>
</figure>

Þegar þú hefur vistað, verður þú vísað aftur á myndritin þín þar sem þú getur séð nýja myndritið í notkun.

## Fjarlægja myndrit

Til að fjarlægja myndrit, fylgdu þessum skrefum:

<Steps>
1. Smelltu á `Actions` hlekkinn sem er undir hverju myndriti.
2. Smelltu á `Remove` hnappinn.
</Steps>

Myndritið þitt hefur verið eytt.

## API

Forritarar sem vilja stjórna greiningarmyndritum sínum geta farið á [Developers > API > Analytics](/is/api).

## Frekari lestur

- Kynntu þér leiðbeininguna okkar um [Insight for Dummies](/is/guides/general/insight-for-dummies).
