...
2. **Enter It in the Wink Portal**
   ...
   Log into [app.wink.travel](https://app.wink.travel), select your account, and open your customization settings. In the **Analytics & integrations** section, paste your GTM Container ID into the **GTM Container ID** field and save. Your integration is now live.

3. **Confirm with Your Provider**
...
- **A GTM Container ID field** is available in the **Analytics & integrations** section of your customization settings. When you enter your Container ID, Wink validates the format (`GTM-XXXXXXX`) and stores it securely against your property.
...
- **The completed booking (`purchase`) fires in the browser on the thank-you page.** A GTM Container ID is a black box — Wink can't assume what tags are inside it — so Wink never calls Google on your behalf from its servers. Instead, whatever GA4 or conversion tag lives in your container receives the `purchase` client-side, exactly like every other booking event. Because the payment provider redirects the guest's browser away and back, Wink reconnects the analytics session that the redirect interrupted **before** firing `purchase`, so the sale is attributed to the guest's original visit rather than counted as a new session.
