...
:::note
Testaus tulisi aina suorittaa meidän [testausympäristössämme](/fi/getting-started/environments). Siellä on paljon vähemmän dataa, mutta voit testata niin paljon kuin haluat ilman, että kuormitat tuotantoympäristöä.
...
<Steps>
1. [Rekisteröidy](/fi/my-account/register) käyttäjätilillesi Winkissä.
2. [Luo affiliate-tili](/fi/my-account/create-account). Tarvitset tätä sitomaan sovelluksesi. Katso lisätietoja kohdasta [mitä Studio on](/fi/studio/what-is-studio).
3. [Luo sovellus](/fi/my-account/applications) ja liitä se affiliate-tiliin, jonka loit vaiheessa 2. Palvelin-palvelin API-integraatioissa valitse **MACHINE_2_MACHINE** asiakastyypiksi.
...
Siirry [API-rajapintoihimme](/fi/developers/apis) ja valitse haluamasi API.
...
:::tip[Pro Vinkki]
Käytön helpottamiseksi suosittelemme, että integraattorit käyttävät päätepisteitä, jotka määrittelemme `OAuth2`:ksi. Nämä päätepisteet tunnistavat API-kutsujat ja tietävät jo, millä tilillä olet, joten sinun ei tarvitse määrittää `companyIdentifier`-arvoa URL-osoitteissasi.
:::
