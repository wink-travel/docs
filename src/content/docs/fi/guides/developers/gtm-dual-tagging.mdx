...
Jos suoritat mainoskampanjoita Sojernin kautta, voit yhdistää Sojern-tilisi Winkiin syöttämällä Sojern GTM Container ID -tunnuksesi Wink-portaaliin. Kun yhteys on muodostettu, Sojernin seurantapikseli aktivoituu automaattisesti kiinteistölläsi — kattaen koko vieraan matkan hausta aina varauksen vahvistamiseen asti.

<Aside type="tip">
Tämä edellyttää, että sinulla on jo aktiivinen Sojern-tili ja olet saanut GTM Container ID -tunnuksesi Sojern-tilisi yhteyshenkilöltä. Jos sinulla ei vielä ole Sojern-tiliä, käy osoitteessa [sojern.com/contact](https://www.sojern.com/contact) aloittaaksesi.
</Aside>
...
2. **Syötä se Wink-portaaliin**
   ...
   Kirjaudu sisään osoitteessa [app.wink.travel](https://app.wink.travel), valitse tilisi ja siirry **Customizations**-välilehdelle. Liitä Sojern GTM Container ID Sojern-kenttään ja tallenna. Integraatiosi on nyt käytössä.
...
- **Sojern-kontainerisi ladataan asynkronisesti**, eli se ei vaikuta sivun latausnopeuteen vieraille.
...
- **Kun kiinteistöäsi ei katsota** — esimerkiksi kun vieras selaa toista hotellia Winkissä — Sojern-kontaineria ei ladata. Tämä varmistaa, että Sojern saa vain kiinteistöäsi koskevat tiedot.
...
Jokainen tapahtuma noudattaa standardia GA4 Enhanced Ecommerce -rakennetta: ylin `event`-merkkijono ja `ecommerce`-objekti. Ennen jokaista työntöä Wink tyhjentää edellisen ecommerce-payloadin estääkseen vanhojen tietojen vuotamisen uusiin triggereihin.
...
Älä luota siihen, että ecommerce-ominaisuudet säilyvät tapahtumien välillä. Lue ne aina triggerin aikana samassa tapahtumassa.
</Aside>
...
Nämä tapahtumat aktivoituvat automaattisesti, kun vieras etenee varausprosessissa. Määritä GTM-triggerisi **Custom Event** -tyyppisiksi ja käytä alla olevia tapahtumanimiä.
...
Aktivoituu, kun hotellin varausnäkymä tai lista renderöidään.
...
`payment_type` täytetään maksun vastaanottajan tyypillä. `coupon` on mukana vain, jos alennuskoodi on käytössä.
...
Nämä parametrit sisältävät Wink-spesifin varauskontekstin. GA4 ei automaattisesti näytä niitä raporteissa — sinun tulee rekisteröidä jokainen niistä **tuotekohtaiseksi mukautetuksi ulottuvuudeksi** GA4-tilissäsi ennen kuin ne näkyvät tutkimuksissa tai raporteissa. Katso [Mukautetut ulottuvuudet GA4:ssa](#custom-dimensions-in-ga4) alla asennusohjeet.

| Ominaisuus | Tyyppi | Kuvaus |
|------------|--------|--------|
| `property_id` | string | Wink-kiinteistön tunniste |
| `property_name` | string | Kiinteistön näyttönimi |
| `property_brand` | string | Kiinteistön tuotemerkki |
| `property_chain` | string | Kiinteistön ketju |
...
| `room_rate_name` | string | Huonekohtaisen hinnan näyttönimi |
...
GA4 ei huomioi mukautettuja tuoteparametreja raporteissa, ennen kuin rekisteröit ne nimenomaisesti tuotekohtaisiksi mukautetuiksi ulottuvuuksiksi. Sinun tarvitsee rekisteröidä vain ne parametrit, joita aiot käyttää raporteissa — kaikkia 20 ei tarvitse rekisteröidä.

<Steps>

1. **Avaa Mukautetut määritelmät**

   GA4:ssa siirry kohtaan **Admin** (vasen alakulman rataskuvake) → valitse oma property → **Data display** → **Custom definitions**.
...
GA4 sallii enintään 50 tuotekohtaista mukautettua ulottuvuutta per property standardeissa tileissä (360-tilit sallivat enemmän).
</Aside>

Käytä alla olevaa taulukkoa viitteenä mukautettuja ulottuvuuksia luodessasi. **Event parameter** -arvon on oltava täsmälleen sama — GA4 on kirjainkoolla tarkka.
...
| Huonekohtaisen hinnan tunniste | `room_rate_id` | Teksti |
| Huonekohtaisen hinnan nimi | `room_rate_name` | Teksti |
...
| `userId` | Tunnistautuneen käyttäjän tunniste (poistettu, jos ei kirjautunut sisään) |
| `startDate` | Saapumispäivä nykyisestä hakusessiosta |
| `endDate` | Lähtöpäivä nykyisestä hakusessiosta |
| `guests` | Vieraiden määrä nykyisessä hakusessiossa |
| `rooms` | Huoneiden määrä nykyisessä hakusessiossa |
