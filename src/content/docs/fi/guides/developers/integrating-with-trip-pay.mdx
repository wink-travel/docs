...
- [Maksu Web-komponentti](/fi/integrations/web-components/#payment).
- [Maksu API](/fi/api). *Lue osio `Luo maksettavat sopimukset`*
...
- Rivit: **8 - 9** ovat TripPay-tili, joka mahdollistaa varauksen. Se voi olla:
    - Sinun TripPay-integraattoritilisi.
...
- `redirectUrl` kertoo TripPaylle, mille sivulle ohjata maksun jälkeen.
- `sourceUrl` on sivusto/sovellus, jossa varaus tehtiin.
...
- Varasto, jonka haluat varata kyseiseltä toimittajalta. Nämä on määritelty sisäkkäisessä taulukossa `contractItemList`.
- Jokainen `contractItemList`-kohde sisältää:
    - **Vieraskäyttäjä** Tämä on käyttäjä, joka saapuu kohteeseen.
    - **Nimi englanniksi** Varaston nimi englannin kielellä.
    - **Kuvaus englanniksi** Pidempi kuvaus varastosta englanniksi.
    - **Hinta** Kohteen hinta.
    - **Matka-aikataulu** Milloin kohde tulee varata.
    - **Hinnoittelutyyppi** Miten hinta on laskettu.
    - **Varaston tyyppi** Minkä tyyppinen varasto on.
    - **Maksettava** Milloin matkustajalta veloitetaan. *Tällä hetkellä tuemme vain välitöntä maksua.*
    - **Politiikka** Sisältää peruutusehdot.
    - **Päivähintalista** Jos varattava kohde on huone, voit halutessasi sisällyttää huoneen hinnan jokaiselle yölle, jonka vieras viipyy.
    - **Edunsaajat** Sisältää, mille TripPay-tilille/tilille määrätään mikä osuus kohteen hinnasta. *Kiinteät/prosenttiosuudet ovat tuettuja.*

:::note
Yllä olevassa JSON-esimerkissä meillä on 3 edunsaajaa:

- 1,5 % osoitettiin alustalle, joka mahdollisti varauksen. *esim. Wink*
- 9,85 % osoitettiin affiliate-tilille komissiona. *esim. Kim Kardashian*
- 88,65 % osoitettiin toimittajalle huoneen varauksesta. *esim. Hilton*
