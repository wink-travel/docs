...
:::note
La columna **Riesgo** refleja la sensibilidad: `read` es el nivel más bajo y `write`/`remove` el más alto. **Payment** y **Accounting** controlan el movimiento de dinero, por lo que tienen un riesgo elevado.
:::
...
- **Los scopes desconocidos son rechazados.** Solicitar un scope para el que tu aplicación no está registrada, o que no existe, falla en la emisión del token.
...
Obtené un 'Client ID' y 'Secret Key' de una de tus apps y navegá a cualquiera de nuestras [APIs](/es-AR/api/overview/).
:::
