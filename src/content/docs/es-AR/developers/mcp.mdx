...
description: Conectá agentes de IA a la plataforma Wink mediante el Model Context Protocol. Los servidores MCP autenticados le dan a tu agente acceso directo a inventario, reservas, integraciones con channel manager y más, todo asegurado con inicio de sesión interactivo OAuth2.
sidebar:
  order: 10
---
...
:::note
El soporte MCP en Codex CLI requiere la versión **1.0** o superior. Ejecutá `codex --version` para verificar y `npm install -g @openai/codex` para actualizar.
:::
...
:::note
El soporte MCP en Windsurf requiere la versión **1.4** o superior. Verificá en **Ayuda → Acerca de Windsurf** y actualizá desde el sitio web de Windsurf si es necesario.
:::
...
Cuando aprobás la pantalla de consentimiento verás un desglose de lo que el agente está solicitando. Los clientes MCP usan el mismo vocabulario de permisos que el resto de la plataforma — consultá la referencia completa de
[alcances en la página de Autenticación](/es-AR/developers/authentication#scopes).
...
Estás conectado a una cuenta Wink que no tiene entidad gestora. Entrá a [Wink Portal](https://app.wink.travel) y asegurate de que tu cuenta esté configurada antes de usar el servidor MCP.
