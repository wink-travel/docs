...
- **Si proporcionaste un ID de Medición GA4 + secreto API** (paso 3 arriba), el evento `purchase` para reservas completadas se envía adicionalmente **del lado del servidor** vía el Protocolo de Medición GA4, para que una venta se registre incluso si el huésped nunca vuelve a la página de confirmación.
...
No confíes en que las propiedades ecommerce persistan entre eventos. Siempre léelas al momento del disparo dentro del mismo evento.
</Aside>
...
| `room_location` | string | Descriptor de ubicación de la habitación (ej. `Overwater`, `Garden`) |
| `room_view` | string | Descriptor de vista de la habitación (ej. `Ocean`, `City`) |
...
3. **Completa los detalles de la dimensión**
   ...
   - **Parámetro de evento** — el nombre exacto del parámetro tal como aparece en el dataLayer (ver tabla abajo)
...
| Cantidad de adultos | `num_adults` | Número |
| Cantidad de niños | `num_children` | Número |
...
| Cantidad de beneficios | `num_perks` | Número |
...
| `deviceAgent` | Cadena raw del user agent |
