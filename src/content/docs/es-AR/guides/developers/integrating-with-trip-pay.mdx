...
- [Payment Web Component](/es-AR/integrations/web-components/#payment).
- [Payment API](/es-AR/api). *Leé la sección sobre `Create payable contracts`*
...
Asumimos que ya completaste `Paso 1` y `Paso 2` y estás listo para informar a TripPay sobre la reserva.

## Crear contrato pagable

:::note
Ingresá a la Payment API para aprender más sobre los requisitos para crear un contrato pagable.
:::
...
- Cada entrada bajo `contractItemList` contiene:
    ...
    - **Política** Incluí reglas de política de cancelación.
    ...
    - **Beneficiarios** Incluí qué cuenta(s) de TripPay debe(n) recibir qué monto(s) del precio del ítem. *Se soportan montos fijos y porcentajes.*
