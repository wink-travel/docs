...
Puedes encontrar ejemplos para [tarjetas](/es-AR/portal/studio/cards), [grillas](/es-AR/portal/studio/grids) y [mapas](/es-AR/portal/studio/maps).

### Búsqueda (Lookup)

Lookup funciona muy bien junto con una de tus grillas clasificadas. Permite a tus usuarios buscar hoteles y destinos y la grilla se actualiza en respuesta a su búsqueda.
...
:::note
Para aprender cómo generar el `id` del contrato de reserva, lee la guía: [Integrando con TripPay](/es-AR/guides/developers/integrating-with-trip-pay).
:::
...
El inventario podría no estar disponible. Ve a [Wink Studio](/es-AR/portal/studio/cards) y verifica si el estado de la "tarjeta" está disponible.
...
El inventario podría haber sido eliminado. Ve a [Wink Studio](/es-AR/portal/studio/cards) y verifica si el estado de la "tarjeta" está disponible.
Si no lo está, aparecerá en rojo. En ese caso, puedes esperar y tal vez el proveedor lo vuelva a poner disponible o eliminarlo de tu lista.
...
Esto es solo para desarrolladores web. Si eliminaste accidentalmente la [Aplicación](/es-AR/portal/settings/applications); haciendo que el client ID ya no esté disponible, por favor crea una nueva aplicación y usa el nuevo client ID para insertar nuestros Componentes Web.
