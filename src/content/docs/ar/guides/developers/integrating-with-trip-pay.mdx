...
تحتوي هذه المقالة على مثال شامل من البداية إلى النهاية لكيفية التكامل مع TripPay.

يوصى بأن تكون قد قرأت مسبقًا:

- [الدفع > التعيين](/ar/payment/mapping).
- [الدفع > التكامل](/ar/payment/integration).
- [مكون الويب للدفع](/ar/integrations/web-components/#payment).
- [واجهة برمجة تطبيقات الدفع](/ar/api). *اقرأ القسم الخاص بـ `إنشاء عقود قابلة للدفع`*
...
:::note
انتقل إلى واجهة برمجة تطبيقات الدفع لمعرفة المزيد عن المتطلبات لإنشاء عقد قابل للدفع.
:::
...
- `redirectUrl` تخبر TripPay بأي صفحة يجب إعادة التوجيه إليها بعد اكتمال الدفع.
- `sourceUrl` هو الموقع / التطبيق الذي تم فيه الحجز.

يحتوي مصفوفة `contractList` على العنصر (العناصر) التي يريد المسافر شرائها من موردين متعددين. يحتوي كل عنصر على:

- `identifier` هو UUID صالح تقوم بإنشائه.
- المورد الذي تريد الحجز من مخزونه.
...
- كل إدخال تحت `contractItemList` يحتوي على:
    ...
    - **المستفيدون** تضمين حسابات TripPay التي يجب تخصيص مبالغ معينة لها من سعر العنصر. *يتم دعم المبالغ الثابتة / النسب المئوية.*

:::note
في مثال JSON أعلاه، لدينا 3 مستفيدين:

- 1.5% تم تخصيصها للمنصة التي سهلت الحجز. *مثل Wink*
- 9.85% تم تخصيصها للتابع كعمولة. *مثل Kim Kardashian*
- 88.65% تم تخصيصها للمورد مقابل حجز الغرفة. *مثل Hilton*

عندما يرد TripPay بعقد قابل للدفع، ستلاحظ أنه أضاف مستفيدًا خاصًا به بنسبة 4.0%.
:::
...
احفظ `id` في السطر 3. ستستخدمه لحقنه في مكون الويب الخاص بـ TripPay.
