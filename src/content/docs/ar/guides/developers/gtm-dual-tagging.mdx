...
2. **أدخله في بوابة Wink**
   ...
   سجّل الدخول إلى [app.wink.travel](https://app.wink.travel)، اختر حسابك، واذهب إلى تبويب **التخصيصات**. الصق معرف حاوية GTM الخاص بـ Sojern في حقل Sojern واحفظ. أصبح تكاملك الآن مباشرًا.
...
يعمل هذا التكوين مع أي معرف حاوية GTM مخصص. استخدمنا Sojern كمثال لأنه تكامل شائع جدًا.
</Aside>
...
- **يوجد حقل معرف حاوية GTM لـ Sojern** في تبويب التخصيصات في بوابة Wink الخاصة بك. عند إدخال معرف الحاوية، يتحقق Wink من التنسيق ويخزنه بأمان مقابل ممتلكاتك.
- **في كل صفحة تكون فيها ممتلكاتك نشطة** على Wink، يتم تضمين معرف حاوية GTM الخاص بـ Sojern في الصفحة ويقوم نظام إدارة العلامات الخاص بـ Wink بتحميل حاوية Sojern الخاصة بك ديناميكيًا إلى جانب حاويته الخاصة. يحدث هذا تلقائيًا — لا خطوات يدوية مطلوبة من فريق Wink لكل ممتلك.
...
يتم إطلاق هذه الأحداث تلقائيًا أثناء تنقل الضيف عبر مسار الحجز. قم بتكوين مشغلات GTM الخاصة بك كمشغلات **حدث مخصص** تطابق أسماء الأحداث أدناه.
...
يتم إطلاقه عند عرض شبكة أو قائمة مخزون الفنادق.
...
يتم إطلاقه عند عرض الضيف لممتلك محدد.
...
يتم إطلاقه عند إضافة الضيف لغرفة إلى عربة التسوق.
...
يتم إطلاقه عند تحميل صفحة الدفع / الخروج.
...
يتم إطلاقه عند اختيار أو تأكيد الضيف لطريقة الدفع.
...
هذه الحقول مفهومة أصلاً من GA4 وتظهر في تقارير التجارة الإلكترونية القياسية دون أي تكوين إضافي.
...
تحمل هذه المعلمات سياق الحجز الخاص بـ Wink. لا تظهر تلقائيًا في تقارير GA4 — يجب تسجيل كل منها كـ **بُعد مخصص على مستوى العنصر** في خاصية GA4 الخاصة بك قبل أن تظهر في الاستكشافات أو التقارير القياسية. راجع [الأبعاد المخصصة في GA4](#custom-dimensions-in-ga4) أدناه لتعليمات الإعداد.
...
يتجاهل GA4 معلمات العنصر المخصصة في التقارير حتى تقوم بتسجيلها صراحةً كبُعد مخصص على مستوى العنصر. تحتاج فقط إلى تسجيل المعلمات التي تنوي استخدامها في التقارير — لا تحتاج إلى تسجيل جميع العشرين.
...
3. **املأ تفاصيل البُعد**
   ...
   - **النطاق** — اختر **عنصر**
   - **معامل الحدث** — اسم المعامل الدقيق كما يظهر في dataLayer (انظر الجدول أدناه)
...
استخدم الجدول التالي كمرجع عند إنشاء الأبعاد المخصصة. يجب أن تتطابق قيمة **معامل الحدث** تمامًا — GA4 حساس لحالة الأحرف.

| اسم البُعد | معامل الحدث | النوع |
|----------------|-----------------|------|
| معرف الممتلك | `property_id` | نص |
| اسم الممتلك | `property_name` | نص |
| علامة الممتلك التجارية | `property_brand` | نص |
