...
Webhooks позволяват на вашите системи да получават известия в реално време, когато се случват събития във вашия Wink акаунт — нови резервации, анулации, актуализации на плащания и други. Това ръководство ви превежда през настройката и добрите практики.
...
- `type` — ключ на типа събитие (също изпратен като заглавка `Wink-Event-Type`). Разграничете по `type` и `schemaVersion`
...
- `object` — курирано резюме на ресурса, за който е събитието (идентификатори, статус, полетата, върху които действате)
  плюс `links.self`, каноничния REST URL от страна на доставчика на пълния ресурс. Извличайте го с вашите собствени API
...
| `Wink-Delivery-Id` | Уникален за всяка крайна точка и събитие; се променя само при повторна доставка. |
| `Wink-Event-Type` | Същото като `type` в тялото. |
| `Wink-Delivery-Attempt` | Номер на опита за тази доставка, започващ от 1. |
| `Wink-Signature` | HMAC подпис — вижте по-долу. |
...
`t + "." + rawBody`, ключиран с вашата тайна, а `rawBody` е точният байтов масив на тялото на заявката, както е получен —
не сериализирайте JSON отново преди проверката. В рамките на 24 часа след ротация на тайната заглавката носи
втори `v1=` подписан със старата тайна; приемете доставката, ако **някой** от `v1` съвпада.
...
- **Идемпотентност** — Вашият обработчик трябва да е идемпотентен; премахвайте дубликати по `Wink-Event-Id`. Wink повтаря, ако не получи `2xx`.
- **Проверявайте източника** — Проверявайте заглавката `Wink-Signature` (вижте [Проверка на подписите](#проверка-на-подписите)) преди обработка; отхвърляйте всичко, което не премине.
...
- [Референтна документация на Webhooks API](/bg/api/webhooks/) — Схеми на съдържанието за всяко събитие, заглавки и крайни точки за управление на абонаменти и доставки.
- [Webhooks](/bg/portal/settings/webhooks) — Пълна справка за управление на webhook.
- [Applications](/bg/account/applications) — Управление на вашите API идентификационни данни.
- [Developers > APIs](/bg/api/overview/) — Пълна API документация.
