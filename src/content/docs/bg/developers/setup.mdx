...
<Steps>
1. [Регистрирайте се](/bg/user/register) с вашия потребителски акаунт в Wink
2. Създайте [акаунт за туристически агент](/bg/travel-agent/what-is-agent) или [партньорски акаунт](/bg/studio/what-is-studio).
3. [Създайте приложение](/bg/developer/applications), като го свържете с акаунта, който създадохте в Стъпка 2.
...
Отидете в нашия раздел [APIs](/bg/developer/apis) и изберете API-то, с което искате да работите.
...
4. Ако сте го импортирали като нова колекция с име `Wink API`, кликнете на хоризонталните три точки до името на колекцията и изберете `Edit`.
...
:::tip[Професионален съвет]
Препоръчително е да направите домейна на сървъра, client ID и client secret като променливи на средата. Така лесно можете да превключвате между стейджинг и продукционна среда и да използвате повторно всички създадени операции.
:::
