...
Приложение ви предоставя `Client ID` и `Secret Key`, които са ви необходими, за да създадете удостоверена OAuth2 сесия, която можете да използвате за комуникация с Wink и TripPay API.
...
:::note
Колоната **Риск** отразява чувствителността: `read` е в ниския край, а `write`/`remove` в по-високия.
...
:::tip
Можете да тествате всички наши API без да напишете и ред код преди да започнете.

Вземете 'Client ID' и 'Secret Key' от някое от вашите приложения и отидете на някой от нашите [APIs](/bg/api/overview/).
:::
