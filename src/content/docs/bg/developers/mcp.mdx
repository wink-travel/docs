...
3. Добавете MCP сървърите на Wink:
...
6. Codex потвърждава връзката. Стартирайте `codex` и го помолете да изброи наличните инструменти, за да проверите дали сървърите на Wink са активни.
</Steps>
...
7. И двата сървъра се появяват в списъка с Connectors. Започнете нов чат — инструментите на Wink са налични автоматично.
</Steps>

:::note[Ограничение на уеб приложението]
MCP сървърите могат да се конфигурират само в **desktop приложението**. Уеб интерфейсът на ChatGPT на chatgpt.com не поддържа персонализирани MCP сървъри.
:::
...
2. Добавете MCP сървърите на Wink:
...
5. Отворете чат (`Ctrl+L` / `Cmd+L`) и помолете Cursor да изброи наличните инструменти на Wink, за да потвърдите връзката.
</Steps>
...
:::note
Поддръжката на Windsurf MCP изисква версия **1.4** или по-нова. Проверете чрез **Help → About Windsurf** и обновете от сайта на Windsurf, ако е необходимо.
:::
...
2. Кликнете върху **Cline иконата** в Activity Bar (лявата странична лента), за да отворите панела на Cline.
...
Когато одобрите екрана за съгласие, ще видите разбивка на заявените от агента разрешения. MCP клиентите използват същата терминология за разрешения като останалата част от платформата — вижте пълния
[списък с обхвати на страницата за автентикация](/bg/developers/authentication#scopes).
