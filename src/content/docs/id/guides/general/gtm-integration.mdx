---
title: Integrasi GTM Analytics
description: Cara menghubungkan kontainer Google Tag Manager apa pun (seperti kontainer Sojern) ke Wink agar dapat melacak perjalanan pemesanan tamu Anda dan mendukung kampanye retargeting serta iklan Anda.
sidebar:
  label: Integrasi GTM Analytics
---

import { Aside, Steps } from '@astrojs/starlight/components';

Anda dapat menghubungkan **kontainer** Google Tag Manager (GTM) **apa pun** ke Wink dengan memasukkan **GTM Container ID** di portal Wink. Setelah terhubung, kontainer Anda akan dimuat secara otomatis untuk properti Anda dan membaca event pemesanan Wink secara langsung — mencakup seluruh perjalanan tamu dari pencarian hingga pemesanan selesai. Kasus penggunaan umum adalah kontainer **Sojern** untuk retargeting dan iklan, tetapi kontainer GTM apa pun bekerja dengan cara yang sama.

<Aside type="tip">
Ini mengasumsikan Anda sudah memiliki GTM Container ID (format `GTM-XXXXXXX`). Jika kontainer Anda berasal dari mitra seperti Sojern, minta **GTM Container ID** secara spesifik kepada manajer akun Anda — ini berbeda dari pixel atau tag ID. Baru dengan Sojern? Kunjungi [sojern.com/contact](https://www.sojern.com/contact) untuk memulai.
</Aside>
...
1. **Dapatkan GTM Container ID Anda**

   Dapatkan **GTM Container ID** Anda (format `GTM-XXXXXXX`). Jika berasal dari mitra seperti Sojern, minta GTM Container ID secara spesifik — ini berbeda dari pixel atau tag ID.

2. **Masukkan di Portal Wink**

   Masuk ke [app.wink.travel](https://app.wink.travel), pilih akun Anda, dan buka pengaturan kustomisasi. Di bagian **Analytics & integrations**, tempelkan GTM Container ID Anda ke kolom **GTM Container ID** dan simpan. Integrasi Anda sekarang aktif.

3. **Konfirmasi dengan Penyedia Anda**
...
<Aside type="note">
Anda tidak perlu menempatkan kode pixel apa pun di situs web Anda untuk integrasi ini. Wink memuat kontainer GTM Anda secara otomatis di semua halaman tempat properti Anda aktif.

Ini berlaku untuk **GTM Container ID apa pun** — Sojern hanyalah contoh umum.
</Aside>
...
- **Kolom GTM Container ID** tersedia di bagian **Analytics & integrations** pada pengaturan kustomisasi Anda. Saat Anda memasukkan Container ID, Wink memvalidasi formatnya (`GTM-XXXXXXX`) dan menyimpannya dengan aman terkait properti Anda.
- **Di setiap halaman tempat properti Anda aktif** di Wink, kontainer Anda disisipkan langsung ke halaman — berbagi `window.dataLayer` milik Wink — sehingga dimuat bersamaan dengan kontainer Wink tanpa ketergantungan konfigurasi tambahan. Ini terjadi otomatis, tanpa pekerjaan per properti dari tim Wink.
- **Kontainer Anda dimuat secara asinkron**, sehingga tidak memengaruhi performa pemuatan halaman bagi tamu Anda.
- **Pelacakan Wink dan kontainer Anda berjalan paralel**, membaca dari `dataLayer` yang sama. Setiap event pemesanan yang dilacak Wink juga tersedia secara bersamaan untuk kontainer Anda.
...
- **Event pemesanan selesai (`purchase`) dipicu di browser pada halaman terima kasih.** GTM Container ID adalah kotak hitam — Wink tidak dapat mengasumsikan tag apa yang ada di dalamnya — jadi Wink tidak pernah memanggil Google atas nama Anda dari servernya. Sebaliknya, tag GA4 atau konversi apa pun yang ada di kontainer Anda menerima event `purchase` secara client-side, sama seperti event pemesanan lainnya. Karena penyedia pembayaran mengarahkan browser tamu pergi dan kembali, Wink menyambungkan kembali sesi analytics yang terputus oleh pengalihan **sebelum** memicu `purchase`, sehingga penjualan dikaitkan dengan kunjungan asli tamu dan tidak dihitung sebagai sesi baru.

---

## Referensi DataLayer

Wink mendorong event kompatibel GA4 ke `dataLayer` sepanjang perjalanan tamu. Kontainer GTM Anda membaca dari `dataLayer` bersama ini, sehingga trigger atau variabel apa pun yang Anda konfigurasi dapat mengakses semua properti yang dijelaskan di bawah.
...
Jangan mengandalkan properti ecommerce bertahan antar event. Selalu baca saat trigger dalam event yang sama.
</Aside>
...
Event ini dipicu otomatis saat tamu bergerak melalui funnel pemesanan. Konfigurasikan trigger GTM Anda sebagai trigger **Custom Event** yang cocok dengan nama event di bawah.
