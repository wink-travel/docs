...
<Steps>
1. [Daftar](/id/user/register) akun pengguna Anda di Wink
2. Buat [akun agen perjalanan](/id/travel-agent/what-is-agent) atau [akun afiliasi](/id/studio/what-is-studio).
3. [Buat Aplikasi](/id/developer/applications) di mana Anda mengaitkannya dengan akun yang Anda buat di Langkah 2.
...
Langsung saja ke bagian [API](/id/developer/apis) kami dan pilih API yang ingin Anda gunakan.
...
:::tip[Pro Tip]
Untuk kemudahan penggunaan, kami menyarankan integrator menggunakan endpoint yang kami tetapkan sebagai `OAuth2`. Endpoint tersebut sudah mengenali pemanggil API dan sudah mengetahui akun mana yang Anda gunakan sehingga Anda tidak perlu menyebutkan `companyIdentifier` di URL Anda.
:::
...
13. Atur bidang `Scope` ke `inventory.read inventory.write inventory.remove`. Itu semua cakupan yang Anda perlukan untuk Wink.
