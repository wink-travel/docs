...
2. Arahkan ke `Applications` lalu `Webhooks`. Lihat [Webhooks](/id/portal/settings/webhooks).
...
Wink menerbitkan 70 jenis acara webhook hari ini yang mencakup pemesanan, properti, akun (entitas pengelola)
...
- `object` — ringkasan terkurasi dari sumber daya yang menjadi acara (pengidentifikasi, status, bidang yang Anda proses)
...
Setiap webhook memiliki **signing secret** (`whsec_…`) yang Wink tampilkan **sekali**, saat Anda membuat webhook atau
...
```js
// Node.js (Gaya Express; pastikan Anda memiliki badan RAW, bukan objek yang diurai)
...
`POST /api/managing-entity/{id}/webhook/{webhookId}/test`. Acara ini ditandatangani dan dikirimkan persis seperti acara nyata,
sehingga Anda dapat memverifikasi endpoint, pemeriksaan tanda tangan, dan penanganan idempoten Anda sebelum berlangganan
acara langsung.
...
- [Referensi Webhooks API](/id/api/webhooks/) — Skema payload per acara, header, dan endpoint manajemen langganan/pengiriman.
...
- [Developers > APIs](/id/api/overview/) — Dokumentasi API lengkap.
