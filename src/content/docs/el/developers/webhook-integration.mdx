...
| `Wink-Delivery-Attempt` | Αριθμός προσπάθειας για αυτή την παράδοση, ξεκινώντας από 1. |
...
Επαληθεύστε σε τέσσερα βήματα: αναλύστε το `t` και κάθε `v1`; υπολογίστε ξανά το HMAC πάνω στο `t.rawBody` με το μυστικό σας; συγκρίνετε με σύγκριση σταθερού χρόνου; απορρίψτε αν `|now − t|` υπερβαίνει την ανοχή σας (συνιστώνται 5 λεπτά).
...
Αλλάξτε το μυστικό από το portal ή με `POST /api/managing-entity/{id}/webhook/{webhookId}/rotate-secret`· η απάντηση εμφανίζει το νέο μυστικό μία φορά, και το παλιό συνεχίζει να επαληθεύεται για 24 ώρες όσο το αναπτύσσετε.
...
- Κάθε γεγονός, παράδοση και προσπάθεια (κατάσταση, απόσπασμα απάντησης) είναι ορατά κάτω από **Applications > Webhooks** και μέσω του API (`…/webhook/event/grid`, `…/webhook/delivery/grid`). Μπορείτε να **επαναπαραδώσετε** οποιαδήποτε παράδοση (`POST …/webhook/delivery/{deliveryId}/redeliver`, που ξεκινά νέα σειρά επαναλήψεων), να επαναπαραδώσετε όλες τις dead παραδόσεις ενός webhook ταυτόχρονα (`POST …/webhook/{webhookId}/redeliver-dead`), ή να ακυρώσετε μία.
...
`POST /api/managing-entity/{id}/webhook/{webhookId}/test`. Υπογράφεται και παραδίδεται ακριβώς όπως ένα πραγματικό γεγονός, ώστε να μπορείτε να επαληθεύσετε το endpoint σας, τον έλεγχο υπογραφής και τη διαχείριση αδιαίρετης εκτέλεσης πριν εγγραφείτε σε ζωντανά γεγονότα.
...
- [Developers > APIs](/el/api/overview/) — Πλήρης τεκμηρίωση API.
