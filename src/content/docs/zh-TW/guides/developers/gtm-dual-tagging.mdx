---
title: Sojern 像素整合
description: 如何將您的 Sojern 帳戶連接到 Wink，使 Sojern 能追蹤您的客人訂房旅程，並推動您的再行銷與廣告活動。
sidebar:
  label: Sojern 像素整合
---
...
<Aside type="tip">
此操作假設您已擁有有效的 Sojern 帳戶，並已從您的 Sojern 客戶經理取得 GTM 容器 ID。如果您尚未擁有 Sojern 帳戶，請造訪 [sojern.com/contact](https://www.sojern.com/contact) 開始申請。
</Aside>
...
2. **在 Wink 入口網站輸入**
   ...
   登入 [app.wink.travel](https://app.wink.travel)，選擇您的帳戶，並前往 **自訂設定** 分頁。將您的 Sojern GTM 容器 ID 貼入 Sojern 欄位並儲存。您的整合即刻生效。
...
- **在 Wink 上每個您的物業啟用的頁面**，Wink 會將您的 Sojern GTM 容器 ID 載入頁面，並由 Wink 的標籤管理系統動態載入您的 Sojern 容器與自身標籤並行。這是自動完成的，Wink 團隊不需為每個物業手動操作。
- **您的 Sojern 容器以非同步方式載入**，不會影響客人的頁面載入效能。
...
Wink 在整個客人旅程中會將 GA4 相容事件推送到 `dataLayer`。您的 GTM 容器會從同一個共用的 `dataLayer` 讀取，因此您設定的任何觸發器或變數都能存取下述所有屬性。

每個事件遵循標準 GA4 強化電子商務結構：頂層有 `event` 字串與 `ecommerce` 物件。每次推送前，Wink 會清除先前的 ecommerce 載荷，以防止舊資料影響新觸發器。
...
請勿依賴 ecommerce 屬性在事件間持續存在。請在同一事件觸發時讀取它們。
</Aside>
...
`transaction_id` 與同一訂房的 `purchase` 事件相符。`coupon`、`shipping` 與 `tax` 僅在適用時出現。
...
這些欄位為 GA4 原生識別，且在標準電子商務報表中自動呈現，無需額外設定。
...
這些參數攜帶 Wink 特定的訂房上下文。GA4 不會自動在報表中呈現，您必須先在 GA4 物業中註冊為 **物品範圍自訂維度**，才能在探索或標準報表中使用。請參考下方 [GA4 自訂維度](#custom-dimensions-in-ga4) 章節的設定說明。
...
| `cancellation_policy` | string | 取消政策，可能值：`Refundable`、`Non-Refundable`、`Refundable-With-Restrictions`、`Non-Refundable-With-Restrictions` |
...
GA4 在報表中會忽略自訂物品參數，除非您明確註冊它們為物品範圍的自訂維度。您只需註冊打算在報表中使用的參數，無需全部 20 個都註冊。
...
3. **填寫維度詳細資訊**
   ...
   - **維度名稱** — GA4 報表中顯示的人類可讀標籤（請參考下表）
   - **範圍** — 選擇 **物品**
   - **事件參數** — 與 dataLayer 中參數名稱完全相符（大小寫敏感，請參考下表）
   - **說明** — 選填，但建議填寫
...
<Aside type="note">
自訂維度註冊後，GA4 報表中可能需等待最多 24 小時才會開始顯示。註冊前收集的歷史資料不會回填。

GA4 標準物業最多允許 50 個物品範圍自訂維度（360 物業可設定更多）。
</Aside>

建立自訂維度時，請參考下表。**事件參數**欄位必須完全匹配，GA4 對大小寫敏感。
...
這是 Wink 的自訂事件 — 不屬於標準 GA4 事件集。它會在每次頁面導覽時推送，攜帶裝置與會話上下文。此事件不包含 `ecommerce` 物件。
