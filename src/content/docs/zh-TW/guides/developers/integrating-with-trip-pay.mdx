...
- [付款網頁元件](/zh-TW/integrations/web-components/#payment)。
- [付款 API](/zh-TW/api)。*請閱讀 `建立應付合約` 章節*
...
### 請求
以下是一個 JSON 範例請求，包含您建立第一個應付合約所需的所有資料。
...
- 第 **8 - 9** 行是協助訂單的 TripPay 帳戶。它可以是：
    - 您的 TripPay 整合者帳戶。
    - 如果您經營聯盟網路，則可以是您的聯盟。
- 使用 `traceId` 來將多個供應商的多筆訂單分組。如此一來，您可以取消整組訂單。
...
- `contractItemList` 中的每個條目包含：
    ...
    - **政策** 包含取消政策規則。
    - **每日費率清單** 若預訂項目為房間，您可以選擇包含旅客每晚住宿的房價。
    - **受益人** 包含哪些 TripPay 帳戶應分配該項目價格的金額。*支援固定金額與百分比。*
...
此時，您已準備好將 `付款網頁元件` 嵌入您的網站並展示給使用者。
