---
title: GTM 分析整合
description: 如何將任何 Google Tag Manager 容器（例如 Sojern 容器）連接到 Wink，以追蹤您的客人訂房旅程並推動您的再行銷和廣告活動。
sidebar:
  label: GTM 分析整合
---
...
2. **在 Wink 入口網站輸入**
   ...
   登入 [app.wink.travel](https://app.wink.travel)，選擇您的帳戶，並前往 **自訂** 標籤。將您的 GTM 容器 ID 貼入 **GTM** 欄位並儲存。您的整合現在已啟用。

3. **（選擇性）新增 GA4 測量 ID + API 密鑰以進行伺服器端購買追蹤**

   GTM 容器完全在客戶端瀏覽器執行，因此如果客人在付款重導期間關閉分頁，可能會遺漏完成訂房事件。如果您的容器載入了 **GA4 屬性**，且您希望可靠記錄完成訂房，請在 GTM 區段中同時輸入該 GA4 的 **測量 ID** 和 **測量協定 API 密鑰**。Wink 將會依照 [GA4 分析整合](/zh-TW/guides/developers/ga4-dual-tagging) 指南所述，**伺服器端** 傳送該屬性的 `purchase` 事件。如果只需要瀏覽器端追蹤，請留空。
...
<Aside type="note">
此整合不需要您在自己網站上放置任何像素代碼。Wink 會自動在所有您的物業啟用頁面載入您的 GTM 容器。

此方法適用於 **任何** GTM 容器 ID — Sojern 僅為常見範例。
</Aside>
...
| `item_list_id` | string | — | 此項目所屬列表的識別碼 |
...
這些參數攜帶 Wink 特定的訂房上下文。GA4 不會自動在報告中顯示它們 — 您必須在 GA4 屬性中將每個參數註冊為 **項目範圍自訂維度**，它們才會出現在探索或標準報告中。請參閱下方 [GA4 自訂維度](#custom-dimensions-in-ga4) 了解設定說明。
...
1. **開啟自訂定義**
   ...
   在 GA4 中，前往 **管理員**（左下齒輪圖示）→ 選擇您的屬性 → **資料顯示** → **自訂定義**。
...
GA4 標準屬性每個屬性最多允許 50 個項目範圍自訂維度（360 屬性允許更多）。
</Aside>
