...
:::note
測試應始終在我們的[測試環境](/zh-TW/getting-started/environments)進行。該環境資料較少，但您可以自由測試，不會影響生產環境。
...
<Steps>
1. 在 Wink 上[註冊](/zh-TW/account/register)您的使用者帳戶。
2. [建立聯盟帳戶](/zh-TW/portal/create-account)。您需要此帳戶來綁定您的應用程式。詳情請參閱[什麼是 Studio](/zh-TW/portal/studio/what-is-studio)。
3. [建立應用程式](/zh-TW/portal/settings/applications)，並將其與您在步驟 2 建立的聯盟帳戶關聯。對於伺服器對伺服器的 API 整合，請選擇 **MACHINE_2_MACHINE** 作為客戶端類型。
4. 建立 `Application` 後，請立即複製並保存 client-id 和 secret key — secret key 僅顯示一次，無法再次取得。
</Steps>
...
請前往我們的[API](/zh-TW/api)區域，選擇您想使用的 API。

### API 文件

我們的文件網站設計成可直接在網站內執行 API 指令。

本例中，您想使用 `Search Lookups (OAuth2)` API 呼叫。

:::tip[專家提示]
為方便使用，我們建議整合者使用標示為 `OAuth2` 的端點。這些端點會自動辨識 API 呼叫者並知道您所屬的帳戶，因此無需在 URL 中指定 `companyIdentifier`。
:::
...
2. 在[Lookup API](https://developer.wink.travel/lookup)頁面頂端點擊 `Download OpenAPI spec`。
3. 將其匯入 Postman。
...
8. 將 `Token Name` 設為您想在 Postman 中儲存的令牌名稱，例如 Wink Staging。
...
13. 將 `Scope` 設為您的整合所需的空格分隔範圍（例如 `inventory.read inventory.write inventory.remove`）。完整清單請參閱[範圍參考](/zh-TW/developers/authentication#scopes)。
14. 將 `Client Authentication` 設為 `Send as Basic Auth header`。
15. 點擊 `Get New Access Token` 按鈕以取得並儲存令牌。
16. 在 Wink API 集合下的所有 API 呼叫中，確保認證標籤顯示 `Inherit auth from parent`，以便每次呼叫都包含您的令牌。
</Steps>
