...
:::note
測試應始終在我們的 [測試環境](/zh-TW/getting-started/environments) 進行。那裡的資料較少，但您可以自由測試，不會影響生產環境。
...
2. [建立聯盟帳戶](/zh-TW/portal/create-account)。您需要此帳戶來綁定您的應用程式。更多背景請參考[什麼是 Studio](/zh-TW/portal/studio/what-is-studio)。
3. [建立應用程式](/zh-TW/account/applications)，並將其與您在步驟 2 建立的聯盟帳戶關聯。若為伺服器對伺服器的 API 整合，請選擇 **MACHINE_2_MACHINE** 作為客戶端類型。
...
上方標示的行顯示 Cloudinary 唯一 ID `partners/radisson_blu_logo`。這就是您取得該圖片任意格式、品質與尺寸的全部資訊。

## 測試

請前往我們的 [API](/zh-TW/api/overview/) 區域，選擇您想使用的 API。
...
:::tip[專家提示]
為方便使用，我們建議整合者使用我們標示為 `OAuth2` 的端點。這些端點會自動辨識 API 呼叫者並知道您所屬的帳戶，因此您無需在 URL 中指定 `companyIdentifier`。
:::
...
8. 將 `Token Name` 設為您想在 Postman 中保存的令牌名稱，例如 Wink Staging。
...
13. 將 `Scope` 設為您的整合所需的空格分隔範圍（例如 `inventory.read inventory.write inventory.remove`）。完整清單請參考[範圍參考](/zh-TW/developers/authentication#scopes)。
14. 將 `Client Authentication` 設為 `Send as Basic Auth header`。
15. 點擊 `Get New Access Token` 按鈕以取得並保存令牌。
