...
<Steps>
1. 在 Wink 上[註冊](/zh-TW/my-account/register)您的使用者帳戶。
2. [建立聯盟帳戶](/zh-TW/my-account/create-account)。您需要此帳戶來綁定您的應用程式。更多背景請參考[什麼是 Studio](/zh-TW/studio/what-is-studio)。
3. [建立應用程式](/zh-TW/my-account/applications)，並將其與您在步驟 2 建立的聯盟帳戶關聯。對於伺服器對伺服器的 API 整合，請選擇 **MACHINE_2_MACHINE** 作為客戶端類型。
...
請前往我們的 [API](/zh-TW/developers/apis) 區域，選擇您想使用的 API。
...
:::tip[專家提示]
為方便起見，我們建議整合者使用我們標示為 `OAuth2` 的端點。這些端點會自動辨識 API 呼叫者並知道您所屬的帳戶，因此您無需在 URL 中指定 `companyIdentifier`。
:::
