...
一旦準備好上線，我們將認證您的帳戶和應用程式，您即可在生產環境中啟動。
:::

請依照以下步驟操作：

<Steps>
1. 在 Wink 上[註冊](/zh-TW/user/register)您的使用者帳號
2. 建立[旅行社帳號](/zh-TW/travel-agent/what-is-agent)或[聯盟帳號](/zh-TW/studio/what-is-studio)。
3. [建立應用程式](/zh-TW/developer/applications)，並將其與步驟 2 中建立的帳號關聯。
...
上方標示的行顯示 Cloudinary 的唯一 ID `partners/radisson_blu_logo`。這就是您取得該圖片任意格式、品質與尺寸的全部資訊。

## 測試

請前往我們的 [APIs](/zh-TW/developer/apis) 區域，選擇您想使用的 API。
...
:::tip[專家提示]
為方便使用，我們建議整合者使用標示為 `OAuth2` 的端點。這些端點會自動辨識 API 呼叫者並知道您所屬的帳號，無需在 URL 中指定 `companyIdentifier`。
:::
...
8. 將 `Token Name` 設為您想在 Postman 中保存的名稱，例如 Wink Staging。
...
13. 將 `Scope` 設為 `inventory.read inventory.write inventory.remove`。這些是 Wink 所需的全部權限範圍。
14. 將 `Client Authentication` 設為 `Send as Basic Auth header`。
15. 點擊 `Get New Access Token` 按鈕取得並保存您的 Token。
16. 在 Wink API 集合下的所有 API 呼叫中，確保授權標籤設定為 `Inherit auth from parent`，以便每次呼叫都帶入您的 Token。
</Steps>
