...
<Steps>
1. 在 Wink 上[註冊](/zh-TW/getting-started/sign-up)您的使用者帳戶。
2. [建立聯盟帳戶](/zh-TW/portal/create-account)。您需要此帳戶來綁定您的應用程式。詳情請參閱[什麼是 Studio](/zh-TW/portal/studio/what-is-studio)。
3. [建立應用程式](/zh-TW/account/applications)，並將其與您在步驟 2 建立的聯盟帳戶關聯。對於伺服器對伺服器的 API 整合，請選擇 **MACHINE_2_MACHINE** 作為客戶端類型。
