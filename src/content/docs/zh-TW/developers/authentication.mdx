...
您很可能會使用您所使用語言的強大 OAuth2 函式庫，該函式庫會幫您完成所有繁重的工作。
...
權限範圍命名格式為 `section.action`，其中 action 為：

- `read` — 檢視資源（安全的 `GET` 請求）
- `write` — 建立及更新資源（`POST` / `PUT` / `PATCH`）
- `remove` — 刪除資源（`DELETE`）

Wink 依功能區域分組權限。請只請求您的整合所需的權限：
...
| **整合** | `integrations.read` `integrations.write` `integrations.remove` | 通路管理、外部訂房系統、Google Hotel、通知端點、訂單同步 | 低至中 |
...
- **請求最小權限。** 僅請求您的整合實際使用的權限 — 若權杖擁有過廣的存取權限，若外洩會造成更大影響範圍。
- **未知權限會被拒絕。** 請求未註冊於您的應用程式或不存在的權限，會在發行權杖時失敗。
...
從您的應用程式取得一組 'Client ID' 和 'Secret Key'，然後前往我們的任一 [API](/zh-TW/api/overview/)。
:::
