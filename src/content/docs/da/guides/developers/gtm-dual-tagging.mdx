...
Hvis du kører reklamekampagner gennem Sojern, kan du forbinde din Sojern-konto til Wink ved at indtaste dit Sojern GTM Container ID i Wink-portalen. Når det er forbundet, aktiveres Sojerns trackingpixel automatisk for din ejendom — og dækker hele gæstens rejse fra søgning til gennemført booking.
...
2. **Indtast det i Wink-portalen**
   ...
   Log ind på [app.wink.travel](https://app.wink.travel), vælg din konto, og gå til fanen **Customizations**. Indsæt dit Sojern GTM Container ID i Sojern-feltet og gem. Din integration er nu aktiv.
...
Du må ikke regne med, at ecommerce-egenskaber bevares mellem hændelser. Læs dem altid ved trigger-tidspunktet inden for samme hændelse.
</Aside>
...
| `creative_slot` | string | — | Placering af kampagnekreativ |
...
3. **Udfyld dimensionens detaljer**
   ...
   - **Dimension name** — det menneskelige læsbare navn, der vises i GA4-rapporter (se tabel nedenfor)
   - **Scope** — vælg **Item**
...
<Aside type="note">
Brugerdefinerede dimensioner kan tage op til 24 timer at begynde at vises i GA4-rapporter efter registrering. Historiske data indsamlet før registrering bliver ikke efterregistreret.

GA4 tillader op til 50 vare-scoped custom dimensions pr. ejendom på standardejendomme (360-ejendomme tillader flere).
</Aside>

Brug følgende tabel som reference, når du opretter brugerdefinerede dimensioner. Værdien i **Event parameter** skal matche præcist — GA4 er case-sensitiv.
...
| `userId` | Identifikator for autentificeret bruger (udelades, hvis ikke logget ind) |
