...
se [`curl`](#staging) eksemplerne ovenfor og [Postman-gennemgangen](/da/developers/setup#postman).
...
| **Integrations** | `integrations.read` `integrations.write` `integrations.remove` | Channel managers, eksterne bookingsystemer, Google Hotel, notifikationsendpoints, booking-synkronisering | Lav–Mellem |
...
:::note
Kolonnen **Risiko** afspejler følsomhed: `read` er den lavere ende og `write`/`remove` den højere
ende. **Payment** og **Accounting** beskytter pengestrømme, så de har forhøjet risiko.
:::
...
Hent et 'Client ID' og 'Secret Key' fra en af dine apps og naviger til en af vores [API'er](/da/api/overview/).
:::
