...
<Steps>
1. [Registrer](/da/getting-started/sign-up) din brugerkonto på Wink.
2. [Opret en affiliate-konto](/da/portal/create-account). Du skal bruge denne til at knytte din applikation til. Se [hvad er Studio](/da/portal/studio/what-is-studio) for mere kontekst.
3. [Opret en applikation](/da/account/applications) og tilknyt den til affiliate-kontoen, du oprettede i trin 2. For server-til-server API-integrationer skal du vælge **MACHINE_2_MACHINE** som klienttype.
...
Hos Wink bruger vi Cloudinary til alle vores billede- og videoressourcer. Når du får adgang til vores rejseinventar, har du fuld adgang til alle Cloudinarys avancerede funktioner.
...
Gå til vores [API'er](/da/api/overview/) sektion og vælg den API, du vil arbejde med.
...
3. Indtast dit client-id og client secret og klik på knappen `Get TOKEN`.
...
4. Hvis du importerede den som en ny samling kaldet `Wink API`, klik på de tre prikker ved siden af samlingsnavnet og vælg `Edit`.
...
13. Sæt feltet `Scope` til de mellemrumsskilte scopes, din integration har brug for (f.eks. `inventory.read inventory.write inventory.remove`). Se [scope reference](/da/developers/authentication#scopes) for den fulde liste.
