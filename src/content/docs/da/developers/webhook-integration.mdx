...
- `object` — et kurateret resumé af ressourcen, som begivenheden handler om (identifikatorer, status, de felter du handler på) plus `links.self`, den leverandørspecifikke kanoniske REST-URL til den fulde ressource. Hent den med dine egne API-legitimationsoplysninger, når du har brug for mere end resumeet; hvis du modtager begivenheden som forhandler eller rejsebureau, brug da det tilsvarende ressource-endpoint i dit eget API for samme identifikator.
...
- **Idempotens** — Din handler bør være idempotent; deduplikér på `Wink-Event-Id`. Wink genforsøger, når den ikke modtager et `2xx` svar.
...
Du kan **deaktivere** en webhook uden at slette den. Dette pauser leveringen, så du kan fejlfinde uden at miste din konfiguration. Når du er klar, skifter du den til on igen.
...
- [Webhooks API reference](/da/api/webhooks/) — Payload-skemaer pr. begivenhed, headers og endpoints til abonnement/leveringsstyring.
...
- [Developers > APIs](/da/api/overview/) — Fuld API-dokumentation.
