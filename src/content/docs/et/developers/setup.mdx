...
:::note
Testimine peaks alati toimuma meie [testkeskkonnas](/et/getting-started/environments). Seal on andmeid palju vähem, kuid võid testida nii palju kui soovid, ilma tootmiskeskkonda koormamata.

Kui oled valmis tootmiskeskkonda minema, sertifitseerime sinu konto ja sinu rakenduse ning saad käivitada selle tootmiskeskkonnas.
:::

Järgi neid samme:

<Steps>
1. [Registreeri](/et/account/register) oma kasutajakonto Winkis.
2. [Loo sidusettevõtte konto](/et/portal/create-account). Seda vajad, et siduda oma rakendus. Vaata lisateavet, mis on [Studio](/et/portal/studio/what-is-studio).
3. [Loo rakendus](/et/portal/settings/applications) ja seo see 2. sammus loodud sidusettevõtte kontoga. Serveri-serveri API integratsioonide puhul vali klienditüübiks **MACHINE_2_MACHINE**.
...
Mine meie [API-de](/et/api) sektsiooni ja vali API, millega soovid töötada.
...
:::tip[Pro näpunäide]
Mugavuse huvides soovitame integreerijatel kasutada meie määratud `OAuth2` lõpp-punkte. Need lõpp-punktid ootavad API-kõne tegijaid ja teavad juba, millise kontoga oled, nii et sa ei pea URL-ides `companyIdentifier`-it määrama.
:::
...
12. Sea `Client Secret` väärtuseks salajane võti, mille sinu rakendus aitas genereerida.
13. Sea `Scope` väärtuseks tühikuga eraldatud õigused, mida su integratsioon vajab (nt `inventory.read inventory.write inventory.remove`). Täieliku nimekirja leiad [õiguste viitest](/et/developers/authentication#scopes).
14. Sea `Client Authentication` väärtuseks `Send as Basic Auth header`.
15. Klõpsa `Get New Access Token` nuppu, et token saada ja salvestada.
16. Kõigi Wink API kollektsiooni kõnede puhul veendu, et autentimisvahekaart ütleb `Inherit auth from parent`, et token lisatakse igale kõnele.
</Steps>

:::tip[Pro näpunäide]
Soovitame muuta serveri domeen, kliendi ID ja salajase võtme keskkonnamuutujateks. Nii saad lihtsalt vahetada test- ja tootmiskeskkondi ning taaskasutada kõiki loodud toiminguid.
:::
