...
- **Intelligente** Abilitando l’intelligenza, cerchiamo di recuperare i contenuti `ricchi` disponibili da siti come `Wink`, `Spotify`, `Instagram`, `Facebook`, `YouTube`, `X (ex Twitter)` e altri. *Ad esempio, se condividi un post IG su WinkLinks, il post verrà mostrato come lo vedi su IG, con like, commenti e un modo per interagire con il post.*
- **Visualizzazione iniziale** Controlla come il tuo pubblico interagisce inizialmente con il tuo sito e vede i tuoi contenuti. Hai a disposizione tre opzioni:
    ...
    - Design `a lista`. La maggior parte degli altri gestori di link mostra una semplice lista con un URL e contenuti minimi. Scegli questo modo di visualizzare i contenuti se ne hai molti e vuoi mostrarli il più rapidamente possibile.
...
- **Personalizzazione** Personalizza l’aspetto e la sensazione della tua pagina WinkLinks. Puoi personalizzare la tua palette di colori a piacimento per abbinarla perfettamente al tuo brand. Questa impostazione influenza anche come vengono mostrati le schede inventario Wink, le mappe e le griglie.  
  
Controlli aggiuntivi includono:  

- Comportamento di visualizzazione dei post (Modalità Banner, Banner Intelligente/Interattivo, Regolazione automatica consapevole del layout)  
...
Gli sviluppatori che vogliono gestire `WinkLinks` possono andare su [Developers > API > WinkLinks](/it/api).
