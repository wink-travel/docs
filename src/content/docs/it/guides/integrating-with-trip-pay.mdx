...
- [Payment Web Component](/it/integrations/web-components/#payment).
...
2. Lascia che il viaggiatore scelga l’inventario che desidera prenotare e preparati al pagamento.
...
- Linee: **2 - 7** è l’utente responsabile della prenotazione. Ci sono due tipi di prenotatori:
    - Viaggiatore.
    - Agenzia di viaggi.
...
- Usa un `traceId` per raggruppare più prenotazioni su più fornitori. In questo modo puoi annullare una prenotazione di gruppo.
- `redirectUrl` indica a TripPay quale pagina reindirizzare dopo il completamento del pagamento.
...
Quando TripPay risponde alla tua richiesta, sarà simile a questa:
