---
title: Server MCP
description: Collega agenti AI alla piattaforma Wink tramite il Model Context Protocol. I server MCP autenticati danno al tuo agente accesso diretto a inventario, prenotazioni, integrazioni con channel manager e altro — protetto da login OAuth2 interattivo.
sidebar:
  order: 10
---
...
Aggiungi `--scope project` a `claude mcp add` e Claude Code scriverà l'URL del server in `.mcp.json` nella radice del repository. Committa quel file — ogni membro del team completerà il proprio accesso browser la prima volta che si connette.
:::
...
2. Apri (o crea) il file di configurazione di Codex:
...
7. Entrambi i server appaiono nella lista dei Connettori. Avvia una nuova chat — gli strumenti Wink sono disponibili automaticamente.
</Steps>
...
5. **Accedi a Wink** e approva le autorizzazioni. I server diventano verdi nel pannello delle impostazioni una volta autenticati.
</Steps>

:::note
Il supporto MCP di Windsurf richiede la versione **1.4** o successiva. Controlla tramite **Aiuto → Informazioni su Windsurf** e aggiorna dal sito di Windsurf se necessario.
:::
...
Quando approvi la schermata di consenso vedrai un dettaglio di cosa l'agente sta richiedendo. I client MCP usano lo stesso vocabolario di permessi del resto della piattaforma — vedi la
[referenza completa degli scope nella pagina Autenticazione](/it/developers/authentication#scopes).
