...
:::note
I test devono sempre essere effettuati nel nostro [ambiente di staging](/getting-started/environments). Ci sono molti meno dati, ma sei libero di testare quanto vuoi senza influire sull'ambiente di produzione.
...
<Steps>
1. [Registrati](/user/register) con il tuo account utente su Wink
2. Crea un [account agente di viaggio](/travel-agent/what-is-agent) o un [account affiliato](/studio/what-is-studio).
3. [Crea un'Applicazione](/developer/applications) associandola all'account creato al Passo 2.
4. Una volta creata la tua `Application`, conserva in un luogo sicuro client-id e client secret.
</Steps>

## CDN

Su Wink utilizziamo Cloudinary per tutte le nostre risorse di immagini e video. Quando accedi al nostro inventario di viaggi, avrai pieno accesso a tutte le funzionalità avanzate di Cloudinary.
...
Vai alla nostra sezione [API](/developer/apis) e scegli l'API con cui vuoi lavorare.
...
:::tip[Consiglio]
Si consiglia di trasformare dominio server, client ID e client secret in variabili d'ambiente. In questo modo puoi facilmente passare tra ambienti di staging e produzione e riutilizzare tutte le operazioni create.
:::
