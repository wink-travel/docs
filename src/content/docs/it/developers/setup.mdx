...
3. [Crea un’Applicazione](/it/account/applications) e associa l’account affiliato creato al Passo 2. Per integrazioni API server-to-server, scegli **MACHINE_2_MACHINE** come tipo di client.
...
Vai alla nostra sezione [API](/it/api/overview/) e scegli l’API con cui vuoi lavorare.
...
2. Clicca su `Download OpenAPI spec` dalla parte superiore della pagina della [Lookup API](https://developer.wink.travel/lookup).
...
13. Imposta il campo `Scope` con gli scope separati da spazio necessari per la tua integrazione (es. `inventory.read inventory.write inventory.remove`). Consulta il [riferimento degli scope](/it/developers/authentication#scopes) per l’elenco completo.
