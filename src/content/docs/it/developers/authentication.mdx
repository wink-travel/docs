...
Un'applicazione ti fornisce un `Client ID` e una `Secret Key` che devi utilizzare per creare una sessione OAuth2 autenticata da usare per comunicare con le API di Wink e TripPay.
...
- `Wink-Version` = `2.0` Ultima - *Consulta la nostra documentazione API per altre versioni disponibili.*
- `Authorization` = `Bearer ${access_token}` *Inserisci il tuo token di accesso.*

:::tip
Puoi testare tutte le nostre API senza scrivere una singola riga di codice prima di iniziare.
