...
| **Marketing** | `marketing.read` `marketing.write` `marketing.remove` | Campagne, vantaggi, canali di vendita, WinkLinks, social gestiti, loyalty | Basso–Medio |
...
- **Richiedi il minimo indispensabile.** Chiedi solo gli scope che la tua integrazione utilizza effettivamente — un token emesso con accesso più ampio del necessario ha un raggio d'azione più ampio in caso di fuga.
...
Prendi un 'Client ID' e una 'Secret Key' da una delle tue app e naviga in una qualsiasi delle nostre [API](/it/api/overview/).
:::
