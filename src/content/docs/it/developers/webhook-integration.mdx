...
2. Quando si verifica un evento (ad esempio, una nuova prenotazione), Wink invia un HTTP POST al tuo URL.
...
8. Clicca su `Save` — la risposta mostra il tuo **segreto di firma una sola volta**; salvalo subito.
</Steps>
...
L’elenco completo e generato — con descrizione, destinatari e link alla pagina di riferimento di ogni evento — è il **Catalogo Eventi Webhook**. La pagina di riferimento per ogni evento (corpo JSON, header, politica di retry) si trova nelle [Webhooks API](/it/api/webhooks/).
...
- `type` — chiave del tipo di evento (inviata anche come header `Wink-Event-Type`). Ramifica su `type` e `schemaVersion` per interpretare `object`.
...
Ogni schema payload è documentato per evento nelle [Webhooks API](/it/api/webhooks/) di riferimento.
...
Ogni webhook ha un **segreto di firma** (`whsec_…`) che Wink mostra **una sola volta**, quando crei il webhook o ruoti il segreto. Salvalo come una password. Ogni consegna porta
...
- Un `5xx`, timeout, `408` o `429` viene ritentato con backoff: dopo 1 minuto, 5 minuti, 30 minuti, 2 ore,
  6 ore, 12 ore, poi giornalmente — 10 tentativi in circa 3 giorni — dopo di che la consegna è marcata **dead**.
...
- Ogni evento, consegna e tentativo (stato, snippet di risposta) è visibile sotto **Applications > Webhooks**
  ...
  (`POST …/webhook/delivery/{deliveryId}/redeliver`, che avvia una nuova serie di retry), ridistribuire tutte le consegne dead di un webhook in una volta (`POST …/webhook/{webhookId}/redeliver-dead`), o cancellarne una.
...
- [Riferimento API Webhooks](/it/api/webhooks/) — Schemi payload per evento, header e endpoint di gestione sottoscrizione/consegna.
...
- [Developers > APIs](/it/api/overview/) — Documentazione completa delle API.
