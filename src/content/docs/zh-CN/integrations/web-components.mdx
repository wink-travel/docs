...
*属性 `sort` 仅在 layout 为 `RANKED` 且您不想使用已有的排名网格时可用。此时请将 `id` 留空。*
...
<CloudinaryImage
      ...
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*带搜索结果的查找模态框*</figcaption>
</figure>

### 行程

行程按钮会在按钮上显示当前行程。点击后，完整的行程选择器会以模态框形式显示。
...
搜索是行程选择器的图标按钮。点击后，完整的行程选择器会以模态框形式显示。
...
在任何行程组件中更改行程，都会触发行程更新事件，更新当前页面上显示的库存。
...
在文档底部引入我们的 Javascript。（*建议放在 &lt;body&gt; 标签结束前*）
...
如果您在 WinkLinks 或您自己的网站嵌入我们的 Web 组件时遇到错误提示，可能有以下几种原因：

### 可用性

库存可能不再可用。请前往 [Wink Studio](/zh-CN/portal/studio/cards) 检查“卡片”的状态是否可用。
如果不可用，状态会显示为红色。此时，您可以等待供应商重新开放，或将其从列表中移除。

:::note
“可用性”并非指库存的每日可预订情况，而是指该库存对您和所有人完全不可用。
:::

### 已移除

库存可能已被移除。请前往 [Wink Studio](/zh-CN/portal/studio/cards) 检查“卡片”的状态是否可用。
如果不可用，状态会显示为红色。此时，您可以等待供应商重新开放，或将其从列表中移除。

### 自定义

您可能不小心删除了与库存配套定义的自定义设置。请确保自定义设置仍然存在，若缺失请重新设置。

### 应用

此项仅针对网页开发者。如果您不小心删除了 [Application](/zh-CN/portal/settings/applications) ，导致客户端 ID 不再可用，请创建一个新的应用并使用新的客户端 ID 来嵌入我们的 Web 组件。
