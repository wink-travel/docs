...
<CloudinaryImage
      ...
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*带有结果的 Lookup 模态窗口*</figcaption>
</figure>
...
该组件会准备合同执行，并向用户显示支付详情 *(图 1)*，以完成预订。
...
如果您在嵌入我们的 Web 组件时，无论是在 WinkLinks 还是您自己的网站上，遇到错误提示，可能是以下原因：
...
您可能不小心删除了与库存配套的自定义设置。请确保自定义仍然存在，若缺失请重新设置。

### 应用

此项仅针对网页开发者。如果您不小心删除了 [Application](/zh-CN/portal/settings/applications)；导致客户端 ID 不再可用，请创建一个新应用，并使用新的客户端 ID 来嵌入我们的 Web 组件。
