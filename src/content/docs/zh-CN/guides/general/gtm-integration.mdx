...
您可以通过在 Wink 门户中输入 **GTM 容器 ID**，将**任何** Google Tag Manager (GTM) 容器连接到 Wink。连接后，您的容器会自动为您的物业加载，并直接读取 Wink 的预订事件——覆盖从搜索到完成预订的完整客人旅程。一个常见的用例是用于重定向和广告的 **Sojern** 容器，但任何 GTM 容器的工作方式相同。
...
2. **在 Wink 门户中输入**
   ...
   登录 [app.wink.travel](https://app.wink.travel)，选择您的账户，进入 **自定义** 标签页。将您的 GTM 容器 ID 粘贴到 **GTM** 字段并保存。您的集成现已生效。

3. **（可选）添加 GA4 测量 ID + API 密钥以实现服务器端购买**

   GTM 容器完全在客人的浏览器中运行，因此如果客人在支付重定向期间关闭标签页，可能会错过完成的预订。如果您的容器加载了 **GA4 属性**，并且您希望可靠地记录完成的预订，请在 GTM 部分同时输入该 GA4 的 **测量 ID** 和 **测量协议 API 密钥**。Wink 将按照 [GA4 分析集成](/zh-CN/guides/developers/ga4-dual-tagging) 指南中描述的方式，**服务器端** 发送该属性的 `purchase` 事件。如果只需要浏览器内跟踪，请留空。
...
<Aside type="note">
此集成无需您在自己的网站上放置任何像素代码。Wink 会自动在所有激活您物业的页面加载您的 GTM 容器。
...
- **如果您提供了 GA4 测量 ID + API 密钥**（上述第 3 步），完成预订的 `purchase` 事件还会通过 GA4 测量协议**服务器端**发送，即使客人未返回确认页，销售也会被记录。
...
当客人通过预订漏斗时，这些事件会自动触发。请将您的 GTM 触发器配置为匹配下表事件名称的**自定义事件**触发器。
...
这些参数携带 Wink 特定的预订上下文。GA4 不会自动在报告中显示它们——您必须在 GA4 属性中将每个参数注册为**商品范围的自定义维度**，它们才会出现在探索或标准报告中。请参见下文 [GA4 中的自定义维度](#custom-dimensions-in-ga4) 了解设置说明。
...
1. **打开自定义定义**
   ...
   在 GA4 中，进入 **管理**（左下齿轮图标）→ 选择您的属性 → **数据展示** → **自定义定义**。
...
GA4 标准属性最多支持 50 个商品范围的自定义维度（360 版本支持更多）。
</Aside>
