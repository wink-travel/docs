---
title: Sojern 像素集成
description: 如何将您的 Sojern 账户连接到 Wink，以便 Sojern 能够跟踪您的客人预订旅程，并支持您的重定向和广告活动。
sidebar:
  label: Sojern 像素集成
---
...
1. **从 Sojern 获取您的 GTM 容器 ID**
   ...
   联系您的 Sojern 客户经理，索取您的 **GTM 容器 ID**。格式为 `GTM-XXXXXXX`。这与您的 Sojern 像素 ID 不同——请确保您特别要求 GTM 容器 ID。

2. **在 Wink 门户中输入**

   登录 [app.wink.travel](https://app.wink.travel)，选择您的账户，进入 **自定义** 标签页。将您的 Sojern GTM 容器 ID 粘贴到 Sojern 字段中并保存。您的集成现已生效。

3. **与您的 Sojern 客户经理确认**

   告知您的 Sojern 客户经理集成已激活。他们可以从他们的端验证像素是否在 `wink.travel` 上正确触发，并且数据是否流入您的 Sojern 账户。
...
为确保透明，以下是 Wink 为使此集成生效所做的配置：

- **在 Wink 门户的自定义标签页中提供了 Sojern GTM 容器 ID 字段**。输入容器 ID 后，Wink 会验证格式并安全地存储于您的物业信息中。
...
- **Wink 自身的跟踪与 Sojern 像素并行运行**，共享相同的事件数据。Wink 跟踪的每个预订事件都会同时提供给您的 Sojern 容器。
- **当您的物业未被查看时**——例如客人在 Wink 浏览其他酒店时——您的 Sojern 容器不会被加载，确保 Sojern 只接收与您的物业相关的数据。
...
当客人通过预订漏斗时，这些事件会自动触发。请将您的 GTM 触发器配置为匹配以下事件名称的 **自定义事件** 触发器。
...
这些参数携带 Wink 特定的预订上下文。GA4 不会自动在报告中显示它们——您必须在 GA4 资产中将每个参数注册为 **商品范围的自定义维度**，它们才会在探索或标准报告中出现。请参见下文 [GA4 中的自定义维度](#custom-dimensions-in-ga4) 了解设置说明。
...
GA4 在报告中会忽略自定义商品参数，除非您明确将它们注册为商品范围的自定义维度。您只需注册计划在报告中使用的参数——无需注册全部 20 个。

<Steps>

1. **打开自定义定义**

   在 GA4 中，进入 **管理**（左下角齿轮图标）→ 选择您的资产 → **数据展示** → **自定义定义**。
...
标准 GA4 资产最多支持 50 个商品范围的自定义维度（360 资产支持更多）。
</Aside>

创建自定义维度时，请参考下表。**事件参数** 必须完全匹配——GA4 区分大小写。
...
| 属性 | 描述 |
|----------|-------------|
| `device` | 检测到的设备类型 — 如 `Desktop`、`Android`、`iPhone`、`iPad` 等 |
| `deviceAgent` | 原始用户代理字符串 |
| `os` | 检测到的操作系统 — 如 `Windows`、`MacOS`、`Linux`、`UNIX` |
| `path` | 当前页面路径 |
| `title` | 当前页面标题 |
