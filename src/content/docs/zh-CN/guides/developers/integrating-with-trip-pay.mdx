...
- [支付 Web 组件](/zh-CN/integrations/web-components/#payment)。
- [支付 API](/zh-CN/api)。*请阅读“创建应付合同”部分*
...
3. 使用 TripPay 创建包含要预订项目的 `payable contract`。
...
### 请求
以下是一个 JSON 示例请求，包含创建第一个应付合同所需的所有内容。
...
- `contractItemList` 中的每个条目包含：
    ...
    - **定价类型** 价格的计算方式。
    - **库存类型** 该库存的类型。
    - **应付时间** 旅客应被收费的时间。*目前仅支持即时付款选项。*
    - **政策** 包含取消政策规则。
    - **每日价格列表** 如果预订的是房间，您可以选择包含客人每晚住宿的房价。
    - **受益人** 包含哪些 TripPay 账户应分配该项目价格的金额。*支持固定金额/百分比。*
...
当 TripPay 响应应付合同时，您会注意到它注入了一个 4.0% 的自身受益人。
:::

### 响应

当 TripPay 响应您的请求时，内容大致如下：
...
此时，您已准备好将 `Payment Web Component` 嵌入您的网站并展示给用户。
