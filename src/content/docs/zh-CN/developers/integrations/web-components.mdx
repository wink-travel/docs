---
title: Web 组件
description: Wink 维护了一套可以嵌入您网站的 JavaScript Web 组件库。
sidebar:
  order: 6
---


import { Steps } from '@astrojs/starlight/components';
import CloudinaryImage from '/src/components/cloudinary-image.astro';

[Web Components](https://developer.mozilla.org/en-US/docs/Web/API/Web_components) 是一项标准，允许您将完整的 Javascript 功能集成到您的网站中，而无需编写代码。
得益于这项酷炫的技术，您可以轻松嵌入 Wink 旅游库存。本文将引导您了解我们的 Web 组件集合，并展示如何使用它们。

## 嵌入 Wink

在任何想要显示我们 Web 组件的页面中，您需要包含三样东西。

1. 引入我们的样式表。
2. 引入我们的 Javascript。
3. 引入我们的应用加载器。

### 样式表

在文档的 head 中引入我们的 CSS 样式。

```html {3}
<html>
  <head>
    <link rel="stylesheet" href="https://elements.wink.travel/styles.css"></link>
  </head>
</html>
```

### Javascript

在文档底部引入我们的 Javascript。（*建议放在 &lt;/body&gt; 标签之前*）

```html {3}
<html>
  <body>
    <script src="https://elements.wink.travel/elements.js" type="module" defer></script>
  </body>
</html>
```

您的网站现在已准备好显示我们的 Web 组件。

## 组件

请阅读下面关于我们组件库的介绍。

### 应用加载器

加载器负责维护状态并管理组件间的交互。（*请将其放在 Javascript 之后*）

```html {3-6}
<html>
  <body>
    <wink-app-loader
      client-id="YOUR CLIENT ID GOES HERE"
      configuration-id="YOUR CUSTOMIZATION ID GOES HERE"
    ></wink-app-loader>
  </body>
</html>
```

### 内容加载器

内容加载器是我们 Web 组件库的核心，负责显示您的库存（卡片、网格、地图）。

可用属性：

- layout
- id
- sort

*属性 `sort` 仅在 layout 为 `RANKED` 且您不想使用现有排名网格时可用。此时请将 `id` 留空。*

可用布局类型：

- `AD_BANNER`
- `MAP`
- `HOTEL`
- `GUEST_ROOM`
- `MEETING_ROOM`
- `SPA`
- `RESTAURANT`
- `ACTIVITY`
- `ATTRACTION`
- `PLACE`
- `ADD_ON`
- `LIST`
- `SEARCH`
- `RANKED`

可用排序类型：

- `MEMBER`
- `PRICE_LOW_TO_HIGH`
- `PRICE_HIGH_TO_LOW`
- `PRICE`
- `POPULARITY`
- `ECO`
- `EXPERIENCE`
- `PERK`
- `LOYALTY`
- `PACKAGE`

```html {3-6}
<html>
  <body>
    <wink-content-loader
       layout="GUEST_ROOM"
       id="2de7ee9c-61c9-11ef-9722-42004e494300"
    ></wink-content-loader>
  </body>
</html>
```

您可以在 [cards](/zh-CN/studio/cards)、[grids](/zh-CN/studio/grids) 和 [maps](/zh-CN/studio/maps) 找到示例。

### 查找

Lookup 组件非常适合与您的排名网格配合使用。它允许用户搜索酒店和目的地，网格会根据搜索结果动态更新。

```html {3}
<html>
  <body>
    <wink-lookup></wink-lookup>
  </body>
</html>
```

<figure>
<CloudinaryImage
      width="350"
      height="57"
      resize="scale"
      quality="best"
      alt="Lookup web component"
      src="academy/lookup"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*Lookup Web 组件*</figcaption>
</figure>

点击上方组件会打开一个模态窗口，允许您输入搜索的目的地或酒店。

<figure>
<CloudinaryImage
      width="700"
      height="186"
      resize="scale"
      quality="best"
      alt="Lookup modal"
      src="academy/lookup-results"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*带有结果的 Lookup 模态窗口*</figcaption>
</figure>

### 行程

行程按钮会在按钮上显示当前行程。点击后，完整的行程选择器会以模态窗口形式显示。

```html {3}
<html>
  <body>
    <wink-itinerary></wink-itinerary>
  </body>
</html>
```

<figure>
<CloudinaryImage
      width="350"
      height="60"
      resize="scale"
      quality="best"
      alt="Itinerary web component"
      src="academy/itinerary-button"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*行程 Web 组件*</figcaption>
</figure>

点击上方按钮会打开一个模态窗口，允许您更新行程。

<figure>
<CloudinaryImage
      width="700"
      height="459"
      resize="scale"
      quality="best"
      alt="Search modal"
      src="academy/search-modal"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*作为模态窗口的行程选择器*</figcaption>
</figure>


### 搜索

搜索是行程选择器的图标按钮。点击后，完整的行程选择器会以模态窗口形式显示。

```html {3}
<html>
  <body>
    <wink-search></wink-search>
  </body>
</html>
```

<figure>
<CloudinaryImage
      width="138"
      height="116"
      resize="scale"
      quality="best"
      alt="Search web component"
      src="academy/search-button"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*搜索 Web 组件*</figcaption>
</figure>

点击上方按钮会打开一个模态窗口，允许您更新行程。

<figure>
<CloudinaryImage
      width="700"
      height="459"
      resize="scale"
      quality="best"
      alt="Search modal"
      src="academy/search-modal"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*作为模态窗口的行程选择器*</figcaption>
</figure>

### 行程表单

行程组件显示一个用户可以交互的行程表单。

```html {3}
<html>
  <body>
    <wink-itinerary-form></wink-itinerary-form>
  </body>
</html>
```

<figure>
<CloudinaryImage
      width="700"
      height="90"
      resize="scale"
      quality="best"
      alt="Itinerary formweb component"
      src="academy/itinerary-form"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*行程表单 Web 组件*</figcaption>
</figure>

在任何行程组件中更改行程，都会触发行程更新事件，更新当前页面上显示的库存。

### 账户

账户按钮允许您为网站添加 Wink 认证功能。

```html {3}
<html>
  <body>
    <wink-account></wink-account>
  </body>
</html>
```

<figure>
<CloudinaryImage
      width="92"
      height="104"
      resize="scale"
      quality="best"
      alt="Account web component"
      src="academy/account-button"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*账户按钮 Web 组件*</figcaption>
</figure>

当用户未认证时，点击按钮会引导用户进行认证。
认证后，按钮会显示用户的头像图标。

<figure>
<CloudinaryImage
      width="92"
      height="104"
      resize="scale"
      quality="best"
      alt="Account button when authenticated"
      src="academy/account-button-auth"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*账户按钮（已认证）Web 组件*</figcaption>
</figure>

点击按钮会打开用户专属的下拉菜单。

<figure>
<CloudinaryImage
      width="350"
      height="404"
      resize="scale"
      quality="best"
      alt="Account button when authenticated"
      src="academy/account-button-auth-open"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*账户按钮（展开）Web 组件*</figcaption>
</figure>

## 嵌入 TripPay

在任何想要使用 TripPay 支付 Web 组件的页面中，您需要包含两样东西。

1. 引入我们的样式表。
2. 引入我们的 Javascript。

### 样式表

在文档的 head 中引入我们的 CSS 样式。

```html {3}
<html>
  <head>
    <link rel="stylesheet" href="https://elements.trippay.io/styles.css"></link>
  </head>
</html>
```

### Javascript

在文档底部引入我们的 Javascript。（*建议放在 &lt;body&gt; 结束标签之前*）

```html {3}
<html>
  <body>
    <script src="https://elements.trippay.io/elements.js" type="module" defer></script>
  </body>
</html>
```

您的网站现在已准备好显示我们的 Web 组件。

## 组件

### 支付

支付组件让您通知 TripPay 旅客想要购买的商品及其规则和价格。

该组件有一个必需属性：

- `id` 您想执行的预订合同的标识符。

```html {3}
<html>
  <body>
    <trip-pay id="<INSERT_BOOKING_CONTRACT_ID>"></trip-pay>
  </body>
</html>
```

:::note
要了解如何生成预订合同 `id`，请阅读指南：[Integrating with TripPay](/zh-CN/guides/integrating-with-trip-pay)。
:::

该组件会准备合同执行，并向用户显示支付详情 *(图 1)*，以完成预订。

<figure>
<CloudinaryImage
      width="700"
      height="1141"
      resize="scale"
      quality="best"
      alt="Payment details"
      src="academy/payment-modal"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*图 1. 支付表单示例*</figcaption>
</figure>

## 错误

如果您在嵌入我们的 Web 组件时，无论是在 WinkLinks 还是您自己的网站上，遇到错误提示，可能是以下原因：

### 可用性

库存可能不再可用。请前往 [Wink Studio](/zh-CN/studio/cards) 检查“卡片”的状态是否可用。
如果不可用，状态会显示为红色。此时，您可以等待供应商重新开放，或将其从列表中移除。

:::note
`可用性` 并非指库存每日可预订的数量，而是指库存对您和所有人完全不可用。
:::

### 已移除

库存可能已被移除。请前往 [Wink Studio](/zh-CN/studio/cards) 检查“卡片”的状态是否可用。
如果不可用，状态会显示为红色。此时，您可以等待供应商重新开放，或将其从列表中移除。

### 自定义

您可能不小心删除了与库存配套的自定义设置。请确保自定义仍然存在，若缺失请重新设置。

### 应用

此项仅针对网页开发者。如果您不小心删除了 [Application](/zh-CN/my-account/applications)；导致客户端 ID 不再可用，请创建一个新应用，并使用新的客户端 ID 来嵌入我们的 Web 组件。
