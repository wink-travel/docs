...
在任何我们的行程组件中更改行程，都会触发行程更新事件，更新当前页面上显示的任何库存。
...
<CloudinaryImage
      ...
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*账户按钮（打开状态） Web 组件*</figcaption>
</figure>
...
:::note
`可用性` 并非指库存的每日可预订情况，而是指库存对您和所有人完全不可用。
:::
...
此项仅针对开发者。如果您不小心删除了 [Application](/zh-CN/developer/applications) ，导致客户端 ID 不再可用，请创建新的应用并使用新的客户端 ID 嵌入我们的 Web 组件。
