...
Webhook 让您的系统能够在 Wink 账户发生事件时接收实时通知——如新预订、取消、支付更新等。本指南将引导您完成设置及最佳实践。
...
Wink 支持 **64+ 种 webhook 事件类型**，涵盖账户管理、预订合同、支付、渠道管理器更新及管理实体管理等类别。常见事件包括：
...
- **幂等性** —— 处理程序应具备幂等性。若未收到 `200` 响应，Wink 可能会重试发送。
...
- [Applications](/zh-CN/portal/settings/applications) —— 管理您的 API 凭证。
- [Developers > APIs](/zh-CN/api) —— 完整 API 文档。
