...
:::note
测试应始终在我们的[预发布环境](/zh-CN/getting-started/environments)进行。那里数据较少，但您可以自由测试，无需担心影响生产环境。
...
<Steps>
1. 在 Wink 上[注册](/zh-CN/account/register)您的用户账户。
2. [创建一个分销账户](/zh-CN/portal/create-account)。您需要将您的应用绑定到该账户。详情请参见[什么是 Studio](/zh-CN/portal/studio/what-is-studio)。
3. [创建一个应用](/zh-CN/portal/settings/applications)，并将其关联到步骤 2 中创建的分销账户。对于服务器到服务器的 API 集成，选择 **MACHINE_2_MACHINE** 作为客户端类型。
4. 创建 `Application` 后，请立即复制并保存 client-id 和 secret key —— secret key 只显示一次，无法再次获取。
</Steps>

## CDN

在 Wink，我们使用 Cloudinary 管理所有图片和视频资源。当您访问我们的旅游库存时，将可完全使用 Cloudinary 的所有高级功能。
...
请前往我们的[API](/zh-CN/api)部分，选择您想使用的 API。
...
:::tip[专业提示]
为了方便使用，我们建议集成者使用标记为 `OAuth2` 的端点。这些端点会自动识别调用者账户，无需您在 URL 中指定 `companyIdentifier`。
:::
...
/>
<figcaption>*搜索 Lookups (OAuth2) 示例*</figcaption>
</figure>
...
13. 将 `Scope` 设置为您的集成所需的空格分隔权限（例如 `inventory.read inventory.write inventory.remove`）。完整列表请参见[权限参考](/zh-CN/developers/authentication#scopes)。
14. 将 `Client Authentication` 设置为 `Send as Basic Auth header`。
15. 点击 `Get New Access Token` 按钮获取并保存令牌。
16. 在 Wink API 集合下的所有 API 调用中，确保认证标签页显示 `Inherit auth from parent`，以便每次调用都包含您的令牌。
</Steps>

:::tip[专业提示]
建议将服务器域名、Client ID 和 Client Secret 设置为环境变量。这样您可以轻松切换预发布和生产环境，并复用所有已创建的操作。
:::
