---
title: 设置
description: 本文介绍如何快速作为开发者进行设置并开始在 Wink 上测试我们的 API。
sidebar:
  order: 1
---
...
<Steps>
1. 在 Wink 上[注册](/zh-CN/my-account/register)您的用户账户。
2. [创建一个分销账户](/zh-CN/my-account/create-account)。您需要绑定您的应用程序。更多背景请参见[什么是 Studio](/zh-CN/studio/what-is-studio)。
3. [创建一个应用程序](/zh-CN/my-account/applications)，并将其关联到步骤 2 中创建的分销账户。对于服务器到服务器的 API 集成，选择 **MACHINE_2_MACHINE** 作为客户端类型。
...
请前往我们的[API](/zh-CN/developers/apis)部分，选择您想使用的 API。

### API 文档

我们的文档站点支持直接在站内运行 API 命令。
...
:::tip[专业提示]
建议将服务器域名、Client ID 和 Client Secret 设置为环境变量。这样可以轻松切换预发布和生产环境，并重用所有已创建的操作。
:::
