...
:::note
测试应始终在我们的[预发布环境](/zh-CN/getting-started/environments)进行。那里数据较少，但您可以自由测试，无需影响生产环境。
...
2. [创建一个分销账户](/zh-CN/portal/create-account)。您需要绑定您的应用程序。更多背景请参见[什么是 Studio](/zh-CN/portal/studio/what-is-studio)。
3. [创建一个应用程序](/zh-CN/account/applications)，并将其关联到步骤 2 中创建的分销账户。对于服务器到服务器的 API 集成，选择 **MACHINE_2_MACHINE** 作为客户端类型。
...
请前往我们的[API](/zh-CN/api/overview/)部分，选择您想使用的 API。
...
:::tip[专业提示]
为方便使用，我们建议集成者使用标记为 `OAuth2` 的端点。这些端点会自动识别调用者账户，无需您在 URL 中指定 `companyIdentifier`。
:::
...
16. 对 Wink API 集合下的所有 API 调用，确保认证标签页显示 `Inherit auth from parent`，以便每次调用都包含您的令牌。
</Steps>

:::tip[专业提示]
建议将服务器域名、Client ID 和 Client Secret 设置为环境变量。这样您可以轻松切换预发布和生产环境，并重用所有已创建的操作。
:::
