...
<Steps>
1. 在 Wink 上[注册](/zh-CN/getting-started/sign-up)您的用户账户。
2. [创建一个分销账户](/zh-CN/portal/create-account)。您需要将您的应用绑定到该账户。详情请参见[什么是 Studio](/zh-CN/portal/studio/what-is-studio)。
3. [创建一个应用](/zh-CN/account/applications)，并将其关联到步骤 2 中创建的分销账户。对于服务器到服务器的 API 集成，选择 **MACHINE_2_MACHINE** 作为客户端类型。
