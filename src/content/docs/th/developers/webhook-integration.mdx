...
5. กรอก **URL webhook** — จุดสิ้นสุด HTTPS บนเซิร์ฟเวอร์ของคุณ
6. **เลือกเหตุการณ์** — เลือกเหตุการณ์เฉพาะที่ต้องการสมัครรับ หรือปล่อยว่างเพื่อรับทุกเหตุการณ์
7. สลับ **เปิดใช้งาน** เป็นเปิด
8. คลิก `Save` — ระบบจะแสดง **signing secret** ให้คุณเห็นเพียงครั้งเดียว; โปรดเก็บไว้ทันที
</Steps>

## ประเภทเหตุการณ์

Wink เผยแพร่เหตุการณ์ webhook จำนวน 70 ประเภทในปัจจุบัน ครอบคลุมการจอง, ทรัพย์สิน, บัญชี (หน่วยงานจัดการ) และสินค้าคงคลัง (ประเภทห้อง, แผนราคา, ราคาหลัก, ส่วนเสริม, สิ่งอำนวยความสะดวก, ช่องทางการขาย, โปรโมชั่น) ตัวอย่างที่พบบ่อย:
...
รายการทั้งหมดที่สร้างขึ้น — พร้อมคำอธิบาย, ผู้รับ และลิงก์ไปยังหน้าข้อมูลอ้างอิงของแต่ละเหตุการณ์ — คือ **Webhook Events Catalog** หน้าข้อมูลอ้างอิงสำหรับแต่ละเหตุการณ์ (โครงสร้าง JSON, เฮดเดอร์, นโยบายการลองใหม่) อยู่ใน [Webhooks API](/th/api/webhooks/)
...
| `Wink-Delivery-Attempt` | หมายเลขครั้งที่พยายามส่ง (เริ่มที่ 1) |
...
หมุนเวียน secret จากพอร์ทัลหรือด้วย `POST /api/managing-entity/{id}/webhook/{webhookId}/rotate-secret` ระบบจะแสดง secret ใหม่ให้เห็นครั้งเดียว และ secret เก่าจะยังตรวจสอบได้อีก 24 ชั่วโมงในขณะที่คุณเปลี่ยนใช้งาน

## การลองใหม่และการส่งซ้ำ

- ตอบกลับด้วยรหัสสถานะ `2xx` ภายใน 10 วินาทีเพื่อยืนยันการรับข้อมูล ทำงานหนักแบบอะซิงโครนัส
- รหัสสถานะ `5xx`, หมดเวลา, `408` หรือ `429` จะถูกลองใหม่โดยมีช่วงเวลาหน่วง: หลัง 1 นาที, 5 นาที, 30 นาที, 2 ชั่วโมง, 6 ชั่วโมง, 12 ชั่วโมง แล้วทุกวัน — รวม 10 ครั้งในประมาณ 3 วัน — หลังจากนั้นการส่งจะถูกทำเครื่องหมายว่า **ตาย**
- รหัสสถานะ `4xx` อื่นๆ จะถือว่า "คุณปฏิเสธการส่งนี้" และจะ **ไม่** ถูกลองใหม่
- เหตุการณ์ การส่ง และการพยายามแต่ละครั้ง (สถานะ, ตัวอย่างการตอบกลับ) สามารถดูได้ใน **Applications > Webhooks** และผ่าน API (`…/webhook/event/grid`, `…/webhook/delivery/grid`) คุณสามารถ **ส่งซ้ำ** การส่งใดก็ได้ (`POST …/webhook/delivery/{deliveryId}/redeliver` ซึ่งเริ่มชุดลองใหม่ใหม่), ส่งซ้ำทุกการส่งที่ตายของ webhook พร้อมกัน (`POST …/webhook/{webhookId}/redeliver-dead`), หรือยกเลิกหนึ่งรายการ
...
- **ตอบกลับอย่างรวดเร็ว** — ส่งกลับ `200 OK` ทันทีที่ได้รับ payload ทำงานหนักแบบอะซิงโครนัส
- **Idempotency** — ตัวจัดการของคุณควรทำงานแบบ idempotent; กรองซ้ำโดยใช้ `Wink-Event-Id` Wink จะลองใหม่เมื่อไม่ได้รับการตอบกลับ `2xx`
- **ตรวจสอบแหล่งที่มา** — ตรวจสอบเฮดเดอร์ `Wink-Signature` (ดู [การตรวจสอบลายเซ็น](#verifying-signatures)) ก่อนประมวลผล; ปฏิเสธทุกอย่างที่ล้มเหลว
...
- [Webhooks API reference](/th/api/webhooks/) — โครงสร้าง payload ของแต่ละเหตุการณ์, เฮดเดอร์ และ endpoint การจัดการการสมัครรับและการส่ง
...
- [Developers > APIs](/th/api/overview/) — เอกสาร API ฉบับเต็ม
