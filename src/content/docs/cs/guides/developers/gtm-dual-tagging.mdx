...
2. **Zadejte ho do portálu Wink**
   ...
   Přihlaste se na [app.wink.travel](https://app.wink.travel), vyberte svůj účet a přejděte na záložku **Customizations**. Vložte své Sojern GTM Container ID do pole Sojern a uložte. Vaše integrace je nyní aktivní.
...
- **Na každé stránce, kde je vaše nemovitost aktivní** ve Winku, je vaše Sojern GTM Container ID vloženo do stránky a Winkův systém správy tagů dynamicky načítá váš Sojern kontejner vedle svého vlastního. To probíhá automaticky — tým Wink nemusí pro každou nemovitost provádět žádné manuální kroky.
- **Váš Sojern kontejner je načítán asynchronně**, což znamená, že neovlivňuje výkon načítání stránky pro vaše hosty.
- **Winkovo vlastní sledování a váš Sojern pixel běží paralelně**, čtou ze stejného sdíleného datového zdroje událostí. Každá událost rezervace, kterou Wink sleduje, je zároveň dostupná vašemu Sojern kontejneru.
- **Když vaše nemovitost není zobrazena** — například když host prohlíží jiný hotel ve Winku — váš Sojern kontejner se nenačítá. To zajišťuje, že Sojern přijímá pouze data relevantní pro vaši nemovitost.
...
Wink resetuje objekt ecommerce před každým pushnutím události:
...
`payment_type` je vyplněn podle typu platebního akceptanta. `coupon` je přítomen pouze, pokud byl použit slevový kód.
...
| `bed_type` | string | Konfigurace postelí v pokoji (např. `King`, `Twin`) |

---

### Vlastní dimenze v GA4

GA4 ignoruje vlastní parametry položek v reportech, dokud je výslovně nezaregistrujete jako vlastní dimenze na úrovni položky. Registrovat musíte pouze ty parametry, které chcete v reportech používat — není nutné registrovat všech 20.
...
GA4 umožňuje až 50 vlastních dimenzí na úrovni položky na standardní vlastnost (360 vlastnosti umožňují více).
</Aside>
...
Toto je vlastní Wink událost — není součástí standardní sady GA4 událostí. Je pushována při každé navigaci na stránce a nese kontext zařízení a relace. Neobsahuje objekt `ecommerce`.
