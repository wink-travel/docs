...
Doporučujeme, abyste si již přečetli:

- [Payment > Mapping](/cs/payment/mapping).
- [Payment > Integration](/cs/payment/integration).
- [Payment Web Component](/cs/integrations/web-components/#payment).
- [Payment API](/cs/api). *Přečtěte si sekci o `Create payable contracts`*
...
- Použijte `traceId` pro seskupení více rezervací napříč více dodavateli. Tímto způsobem můžete zrušit skupinovou rezervaci.
- `redirectUrl` informuje TripPay, na kterou stránku přesměrovat po dokončení platby.
...
- Inventář, který chcete rezervovat od tohoto dodavatele. Vše je specifikováno v zanořeném poli `contractItemList`.
- Každý záznam v `contractItemList` obsahuje:
    ...
    - **Podmínky** Zahrňte pravidla storno podmínek.
...
V tuto chvíli jste připraveni vložit `Payment Web Component` do vašeho webu a zobrazit ji uživateli.

```html
<trip-pay id="contract-1"></trip-pay>
```

Co se týče integrace, to je vše, co musíte udělat. Veškerou náročnou práci odtud provádí TripPay.
