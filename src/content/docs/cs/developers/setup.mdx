...
:::note
Testování by mělo vždy probíhat v našem [testovacím prostředí](/getting-started/environments). Je tam mnohem méně dat, ale můžete testovat, kolik chcete, aniž byste zasahovali do produkčního prostředí.
...
<Steps>
1. [Zaregistrujte](/user/register) si uživatelský účet na Wink
2. Vytvořte si [účet cestovní kanceláře](/travel-agent/what-is-agent) nebo [affiliate účet](/studio/what-is-studio).
3. [Vytvořte aplikaci](/developer/applications), kterou spojíte s účtem vytvořeným v kroku 2.
4. Jakmile vytvoříte svou `Application`, uložte si client-id a client secret na bezpečné místo.
</Steps>
...
Přejděte do sekce našich [API](/developer/apis) a vyberte API, se kterým chcete pracovat.
...
:::tip[Užitečný tip]
Pro snadnější použití doporučujeme integrátorům používat koncové body označené jako `OAuth2`. Tyto koncové body očekávají volající API a už vědí, na kterém účtu jste, takže nemusíte uvádět `companyIdentifier` v URL.
:::

<Steps>
1. Použijte [Lookup API](https://developer.wink.travel/lookup) pro získání destinací nebo hotelů.
...
13. Nastavte pole `Scope` na `inventory.read inventory.write inventory.remove`. To jsou všechny potřebné rozsahy pro Wink.
...
:::tip[Užitečný tip]
Doporučuje se nastavit doménu serveru, client ID a client secret jako proměnné prostředí. Díky tomu můžete snadno přepínat mezi testovacím a produkčním prostředím a znovu používat všechny vytvořené operace.
:::
