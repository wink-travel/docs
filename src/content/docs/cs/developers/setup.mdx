...
<Steps>
1. [Zaregistrujte se](/cs/my-account/register) jako uživatel na Wink.
2. [Vytvořte partnerský účet](/cs/my-account/create-account). Budete ho potřebovat k navázání vaší aplikace. Více informací najdete v [co je Studio](/cs/studio/what-is-studio).
3. [Vytvořte aplikaci](/cs/my-account/applications) a přiřaďte ji k partnerskému účtu, který jste vytvořili v kroku 2. Pro server-to-server API integrace vyberte jako typ klienta **MACHINE_2_MACHINE**.
...
Ve Wink používáme Cloudinary pro všechny naše obrazové a video zdroje. Když přistupujete k našemu cestovnímu inventáři, máte plný přístup ke všem pokročilým funkcím Cloudinary.
...
Pro každý typ cestovního inventáře, který podporujeme, existuje vnořený JSON pole s názvem `multimedias`. JSON pro položku multimedia vypadá takto:
...
Přejděte do naší sekce [API](/cs/developers/apis) a vyberte API, se kterým chcete pracovat.
...
13. Nastavte pole `Scope` na `inventory.read inventory.write inventory.remove`. To jsou všechny rozsahy, které pro Wink potřebujete.
