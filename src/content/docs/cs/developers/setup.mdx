...
Jakmile budete připraveni spustit produkci, ověříme váš účet a vaši aplikaci a můžete spustit provoz v našem produkčním prostředí.
:::

Postupujte podle těchto kroků:

<Steps>
1. [Zaregistrujte](/cs/getting-started/sign-up) si uživatelský účet na Wink.
2. [Vytvořte affiliate účet](/cs/portal/create-account). Budete ho potřebovat k navázání vaší aplikace. Více informací najdete v [co je Studio](/cs/portal/studio/what-is-studio).
3. [Vytvořte aplikaci](/cs/account/applications) a přiřaďte ji k affiliate účtu, který jste vytvořili v kroku 2. Pro server-to-server API integrace vyberte jako typ klienta **MACHINE_2_MACHINE**.
4. Jakmile vytvoříte svou `Application`, ihned si zkopírujte a uložte client-id a tajný klíč — tajný klíč je zobrazen pouze jednou a nelze ho znovu získat.
</Steps>
...
Zvýrazněný řádek výše ukazuje unikátní ID Cloudinary `partners/radisson_blu_logo`. To je vše, co potřebujete k načtení obrázku v jakémkoli formátu, kvalitě a velikosti.

## Testování

Přejděte do naší sekce [API](/cs/api/overview/) a vyberte API, se kterým chcete pracovat.
...
:::tip[Pro tip]
Pro snadnější použití doporučujeme integrátorům používat endpointy označené jako `OAuth2`. Tyto endpointy očekávají volající API a už vědí, na kterém účtu jste, takže nemusíte uvádět `companyIdentifier` v URL.
:::
...
3. Importujte jej do Postmanu.
4. Pokud jste jej importovali jako novou kolekci s názvem `Wink API`, klikněte na horizontální elipsu (3 tečky) vedle názvu kolekce a zvolte `Edit`.
...
8. Nastavte pole `Token Name` na libovolný název vašeho uloženého tokenu v Postmanu, např. Wink Staging.
...
13. Nastavte pole `Scope` na mezery oddělené rozsahy, které vaše integrace potřebuje (např. `inventory.read inventory.write inventory.remove`). Kompletní seznam najdete v [referenci scope](/cs/developers/authentication#scopes).
...
:::tip[Pro tip]
Doporučujeme nastavit doménu serveru, client ID a client secret jako proměnné prostředí. Díky tomu můžete snadno přepínat mezi staging a produkčním prostředím a znovu používat všechny vytvořené operace.
:::
