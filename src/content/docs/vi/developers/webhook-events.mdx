...
Trang này liệt kê mọi sự kiện webhook mà bạn có thể đăng ký trên Wink. Để biết hướng dẫn thiết lập, xem [Hướng dẫn Tích hợp Webhook](/vi/developers/webhook-integration) hoặc [Trang quản lý Webhooks](/vi/portal/settings/webhooks).
...
Để biết các bước thiết lập đầy đủ và các thực hành tốt nhất, xem [Tích hợp Webhook](/vi/developers/webhook-integration).
...
| Tên được cập nhật | `managing_entity.name.updated` | Tên thực thể đã thay đổi. |
...
Các sự kiện trong vòng đời đặt phòng trên Wink, từ tạo đến hoàn tiền và thanh toán.
...
| Quỹ có sẵn | `booking.funds.available` | Quỹ đặt phòng có sẵn để thanh toán. |
...
Các sự kiện liên quan đến hồ sơ và nội dung bất động sản, gửi đến các người đăng ký webhook của bất động sản đó.
...
Các sự kiện liên quan đến các mục tồn kho trong một bất động sản, gửi đến các người đăng ký webhook của bất động sản đó.
...
Các sự kiện cho các kênh phân phối liên kết nhà cung cấp và chi nhánh. Gửi đến cả người đăng ký webhook của nhà cung cấp và chi nhánh.
...
| Tạo người nhận thanh toán thất bại | `payee.create.failure` | Tạo người nhận thanh toán thất bại. |
| Người nhận thanh toán được cập nhật | `payee.update` | Một người nhận thanh toán đã được cập nhật. |
| Cập nhật người nhận thanh toán thất bại | `payee.update.failure` | Cập nhật người nhận thanh toán thất bại. |
...
| Thanh toán thất bại | `internal.payment.failed` | Xử lý thanh toán đã thất bại. |

---

## Sự kiện quản lý kênh

Các sự kiện liên quan đến việc đẩy cập nhật giá và tình trạng phòng của quản lý kênh.
...
| Cập nhật chung | `channel-manager.update` | Cập nhật chung của quản lý kênh. |
...
| Sự kiện | Khóa | Mô tả |
|---|---|---|
| Thư điện tử gửi đi | `email.correspondence` | Một email gửi đi đã được gửi. |
...
| Sự kiện tồn kho (loại phòng, kế hoạch giá, giá chính, phụ kiện, tiện ích) | 24 |
...
:::tip
Bắt đầu bằng cách đăng ký tất cả các sự kiện trong quá trình phát triển. Khi bạn hiểu được các payload mà hệ thống của bạn cần, hãy thu hẹp các đăng ký để giảm thiểu nhiễu.
:::
