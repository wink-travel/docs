...
Một ứng dụng cung cấp cho bạn `Client ID` và `Secret Key` mà bạn cần để tạo phiên OAuth2 đã xác thực, bạn có thể sử dụng để giao tiếp với API của Wink và TripPay.
...
Bạn rất có thể sẽ làm việc với một thư viện OAuth2 mạnh mẽ cho ngôn ngữ của bạn, thư viện đó sẽ xử lý phần lớn công việc nặng nhọc cho bạn.
Ví dụ của chúng tôi sẽ trình bày cách sử dụng cơ bản nhất từ dòng lệnh sử dụng `curl`.
...
Một **scope** là quyền mà access token của bạn mang theo. Khi bạn yêu cầu token, bạn truyền một danh sách các scope cách nhau bằng dấu cách trong tham số `scope`, và token chỉ có thể truy cập các khu vực mà các scope đó bao phủ. Với grant `client_credentials`, bạn đặt các scope trong yêu cầu token của bạn — xem các ví dụ [`curl`](#staging) ở trên và [hướng dẫn Postman](/vi/developers/setup#postman).

Các scope được đặt tên theo định dạng `section.action`, trong đó action là một trong các loại:

- `read` — xem tài nguyên (các yêu cầu `GET` an toàn)
...
| Nhóm scope | Các scope | Mở khóa những gì | Mức rủi ro |
|-------------|--------|------------------|------|
...
:::note
Cột **Mức rủi ro** phản ánh độ nhạy cảm: `read` là mức thấp hơn và `write`/`remove` là mức cao hơn.
...
- **`mcp.*` chỉ dành cho các client tác nhân AI.** Bạn cần nó để mở MCP transport; nó không bắt buộc cho các cuộc gọi REST API thông thường. Xem [Model Context Protocol](/vi/developers/mcp).

:::tip
Bạn có thể thử tất cả API của chúng tôi mà không cần viết một dòng code nào trước khi bắt đầu.

Lấy 'Client ID' và 'Secret Key' từ một trong các ứng dụng của bạn và truy cập bất kỳ [API](/vi/api/overview/) nào của chúng tôi.
:::
