...
<Aside type="tip">
Điều này giả định bạn đã có GTM Container ID (định dạng `GTM-XXXXXXX`). Nếu container của bạn đến từ đối tác như Sojern, hãy yêu cầu quản lý tài khoản của bạn cung cấp **GTM Container ID** cụ thể — nó khác với pixel hoặc tag ID. Mới dùng Sojern? Truy cập [sojern.com/contact](https://www.sojern.com/contact) để bắt đầu.
</Aside>
...
2. **Nhập vào cổng thông tin Wink**
   ...
   Đăng nhập vào [app.wink.travel](https://app.wink.travel), chọn tài khoản của bạn và vào tab **Customizations**. Dán GTM Container ID vào trường **GTM** và lưu lại. Tích hợp của bạn giờ đã hoạt động.

3. **(Tùy chọn) Thêm GA4 Measurement ID + API Secret cho mua hàng phía máy chủ**

   Container GTM chạy hoàn toàn trong trình duyệt của khách, nên một đặt phòng hoàn tất có thể bị bỏ lỡ nếu khách đóng tab trong lúc chuyển hướng thanh toán. Nếu container của bạn tải một **thuộc tính GA4** và bạn muốn ghi nhận đặt phòng hoàn tất một cách đáng tin cậy, hãy nhập thêm **Measurement ID** và **Measurement Protocol API secret** của GA4 đó trong phần GTM. Wink sẽ gửi sự kiện `purchase` cho thuộc tính đó **phía máy chủ**, chính xác như mô tả trong hướng dẫn [Tích hợp GA4 Analytics](/vi/guides/developers/ga4-dual-tagging). Để trống nếu bạn chỉ cần theo dõi trong trình duyệt.
...
<Aside type="note">
Bạn không cần đặt bất kỳ mã pixel nào trên trang web của mình cho tích hợp này. Wink tự động tải container GTM của bạn trên tất cả các trang nơi cơ sở lưu trú của bạn hoạt động.
...
Không dựa vào việc thuộc tính ecommerce tồn tại giữa các sự kiện. Luôn đọc chúng khi trigger trong cùng một sự kiện.
</Aside>
...
<Aside type="note">
Custom dimensions có thể mất đến 24 giờ để xuất hiện trong báo cáo GA4 sau khi đăng ký. Dữ liệu lịch sử thu thập trước khi đăng ký sẽ không được bổ sung.
