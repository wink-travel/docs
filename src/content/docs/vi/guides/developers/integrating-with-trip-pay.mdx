...
- [Payment Web Component](/vi/integrations/web-components/#payment).
- [Payment API](/vi/api). *Đọc phần về `Create payable contracts`*

Dưới đây là các bước để sử dụng TripPay thành công trong việc thanh toán cho một đặt chỗ:

<Steps>
1. Đảm bảo các người thụ hưởng bạn đề cập trong hợp đồng đặt chỗ đã được ánh xạ.
...
4. Cho khách du lịch thanh toán bằng cách sử dụng TripPay Web Component.
</Steps>
...
### Yêu cầu
Dưới đây là mẫu yêu cầu JSON bao gồm tất cả những gì bạn cần để tạo hợp đồng phải thanh toán đầu tiên của mình.
...
- Mỗi mục trong `contractItemList` bao gồm:
    ...
    - **Người thụ hưởng** Bao gồm tài khoản TripPay nào sẽ được phân bổ bao nhiêu phần giá của mục. *Hỗ trợ số tiền cố định / phần trăm.*

:::note
Trong ví dụ JSON trên, chúng tôi có 3 người thụ hưởng:

- 1,5% được phân bổ cho nền tảng đã hỗ trợ đặt chỗ. *ví dụ: Wink*
...
Khi TripPay phản hồi với hợp đồng phải thanh toán, bạn sẽ thấy nó chèn thêm người thụ hưởng của riêng nó với 4,0%.
:::
...
Ở bước này, bạn đã sẵn sàng nhúng `Payment Web Component` vào trang web của mình và hiển thị cho người dùng.

```html
<trip-pay id="contract-1"></trip-pay>
```

Về phần tích hợp, đó là tất cả những gì bạn cần làm. Mọi công việc nặng nhọc sẽ do TripPay xử lý từ đây trở đi.
