...
description: Ez a cikk egy teljes példát tartalmaz arra, hogyan integrálhatod a TripPay-t, és hogyan kezelheti a fizetést helyetted.
sidebar:
  order: 4
---
...
- [Payment Web Component](/hu/integrations/web-components/#payment).
- [Payment API](/hu/api). *Olvasd el a `Create payable contracts` szakaszt*
...
4. Hagyd, hogy az utazó a TripPay Web Component segítségével fizessen.
</Steps>
...
- Sorok: **8 - 9** a TripPay fiók, amely a foglalást kezeli. Ez lehet:
    ...
    - Ha affiliate hálózatot működtetsz, lehet az affiliate-d.
...
- Minden bejegyzés a `contractItemList` alatt tartalmazza:
    ...
    - **Fizetendő** Mikor kell az utazónak fizetnie. *Jelenleg csak az azonnali fizetési opciókat támogatjuk.*
    - **Szabályzat** Tartalmazza a lemondási szabályokat.
    - **Napi árlista** Ha a foglalás egy szoba, megadhatod, mennyibe kerül az adott szoba minden éjszakára, amit a vendég eltölt.
    - **Kedvezményezettek** Megadhatod, mely TripPay fiókoknak milyen összeget kell kiosztani a tétel árából. *Fix vagy százalékos összegek támogatottak.*
...
Amikor a TripPay válaszol egy fizetendő szerződéssel, észre fogod venni, hogy hozzáadta a saját kedvezményezettjét 4,0%-kal.
:::
...
Ekkor készen állsz arra, hogy beágyazd a `Payment Web Component`-et a weboldaladba, és megjelenítsd a felhasználónak.
