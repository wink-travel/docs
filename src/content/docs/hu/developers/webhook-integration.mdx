...
6. **Válaszd ki az eseményeket** — Válassz konkrét eseményeket, vagy hagyd üresen, hogy minden eseményt megkapj.
7. Kapcsold be az **Enabled** kapcsolót.
8. Kattints a `Save` gombra — a válasz egyszer megmutatja a **signing secret-et**; most tárold el.
</Steps>
...
A teljes, generált lista — leírással, címzettjeivel és linkkel az egyes események referenciaoldalára — a **Webhook Események Katalógusa**. Minden esemény referenciaoldala (JSON törzs, fejlécek, újrapróbálkozási szabályzat) megtalálható a [Webhooks API](/hu/api/webhooks/) oldalon.
...
Minden kézbesítés egy HTTP `POST` a webhook URL-edre `Content-Type: application/json` fejléc mellett, az alábbi csomagolással:
...
- `id` — az esemény azonosítója; azonos minden végponton, amely megkapja ezt az eseményt, és minden újrapróbálkozásnál. Használd **idempotencia kulcsként**.
- `type` — az eseménytípus kulcsa (a `Wink-Event-Type` fejlécben is elküldve). A `type` és `schemaVersion` alapján dolgozd fel az `object`-et.
- `object` — az eseményhez tartozó erőforrás válogatott összefoglalója (azonosítók, státusz, a kezelt mezők) plusz `links.self`, a szolgáltató oldali kanonikus REST URL a teljes erőforráshoz. Ha többre van szükséged az összefoglalónál, a saját API hitelesítő adataiddal kérd le; ha viszonteladóként vagy utazási irodaként kapod az eseményt, a saját API felületed megfelelő erőforrás végpontját használd ugyanazzal az azonosítóval.

Minden payload séma dokumentált eseményenként a [Webhooks API](/hu/api/webhooks/) referenciaoldalon.
...
| `Wink-Delivery-Id` | Egyedi azonosító végpontonként eseményenként; csak újraküldéskor változik. |
| `Wink-Event-Type` | Ugyanaz, mint a törzsben a `type`. |
| `Wink-Delivery-Attempt` | 1-től kezdődő próbálkozás száma ezen a kézbesítésen belül. |
| `Wink-Signature` | HMAC aláírás — lásd lent. |

## Aláírások ellenőrzése

Minden webhooknak van egy **signing secret**-je (`whsec_…`), amelyet a Wink **egyszer** mutat meg, amikor létrehozod a webhookot vagy megújítod a titkot. Tárold úgy, mint egy jelszót. Minden kézbesítés tartalmazza:

```
Wink-Signature: t=1755250200,v1=5d41402abc4b2a76b9719d911017c592…
```

ahol `t` egy Unix időbélyeg (másodpercben), és `v1` a kisbetűs hex formátumú **HMAC-SHA256** a `t + "." + rawBody` sztringről, a titkoddal kulcsolva, és a `rawBody` a pontos kérés törzs bájtjai, ahogy érkeztek — ne serializáld újra a JSON-t az ellenőrzés előtt. Titok megújítás után 24 órán át a fejléc tartalmaz egy második `v1=` értéket is, amely az előző titokkal van aláírva; fogadd el a kézbesítést, ha **bármelyik** `v1` egyezik.
...
Forgasd meg a titkot a portálon vagy a `POST /api/managing-entity/{id}/webhook/{webhookId}/rotate-secret` végponton; a válasz egyszer megmutatja az új titkot, és a régi 24 órán át még érvényes, amíg bevezetésre kerül.

## Újrapróbálkozások és újraküldés
...
- Bármely más `4xx` azt jelenti, hogy "elutasítottad ezt a kézbesítést", és **nem** próbálkozik újra.
...
Küldj magadnak egy szintetikus `webhook.test` eseményt a portálról vagy a `POST /api/managing-entity/{id}/webhook/{webhookId}/test` végponton keresztül. Ez aláírt és pontosan úgy kézbesített, mint egy valódi esemény, így ellenőrizheted a végpontodat, az aláírás ellenőrzést és az idempotencia kezelést, mielőtt élő eseményekre iratkozol fel.
...
- [Webhooks API referencia](/hu/api/webhooks/) — Eseményenkénti payload sémák, fejlécek és az előfizetés/kézbesítés kezelő végpontok.
...
- [Fejlesztők > API-k](/hu/api/overview/) — Teljes API dokumentáció.
