...
**Korak 1**. Pridobite dostopni žeton na testnem ali produkcijskem okolju:

Verjetno boste uporabljali robustno OAuth2 knjižnico za vaš programski jezik, ki bo opravila večino dela za vas.
Naši primeri bodo prikazali najosnovnejšo uporabo iz ukazne vrstice z `curl`.
...
| **Inventar** | `inventory.read` `inventory.write` `inventory.remove` | Nepremičnine, tipi sob, ceniki, cene, dodatki, razpoložljivost, aktivnosti, znamenitosti | Nizko–Srednje |
...
| **Integracije** | `integrations.read` `integrations.write` `integrations.remove` | Upravljalci kanalov, zunanji rezervacijski sistemi, Google Hotel, obvestilne končne točke, sinhronizacija rezervacij | Nizko–Srednje |
...
Pridobite 'Client ID' in 'Secret Key' iz ene od vaših aplikacij in obiščite katerikoli od naših [API-jev](/sl/api/overview/).
:::
