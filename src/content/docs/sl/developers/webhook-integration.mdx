...
2. Pojdite na `Applications` in nato `Webhooks`. Oglejte si [Webhooke](/sl/portal/settings/webhooks).
...
6. **Izberite dogodke** — Izberite specifične dogodke, na katere se želite naročiti, ali pustite prazno za prejem vseh dogodkov.
7. Vklopite **Enabled**.
...
Wink danes objavlja 70 vrst webhook dogodkov, ki zajemajo rezervacije, nepremičnine, račune (upravljavske entitete) in inventar (vrste sob, ceniki, glavni ceniki, dodatki, objekti, prodajni kanali, promocije).
Pogosti:
...
Vsaka dostava je HTTP `POST` na vaš webhook URL z `Content-Type: application/json` in to ovojnico:
...
```json
{
  ...
  "schemaVersion": 2,
  "object": { "...": "dogodek-specifična vsebina, npr. BookingWebhookPayload" }
}
```

- `id` — identifikator dogodka; enak za vsak konec vaše računa, ki prejme ta dogodek, in za vsak ponovni poskus. Uporabite ga kot **ključ idempotentnosti**.
...
| `Wink-Delivery-Id` | Edinstven za konec na dogodek; se spremeni samo ob ponovni dostavi. |
...
```js
// Node.js (Express stil; zagotovite, da imate RAW telo, ne razčlenjenega objekta)
...
Zamenjajte skrivnost iz portala ali z `POST /api/managing-entity/{id}/webhook/{webhookId}/rotate-secret`;
odgovor prikaže novo skrivnost samo enkrat, stara pa ostane veljavna 24 ur, da jo lahko uvedete.
...
- Vsak drug `4xx` se obravnava kot "dostavo ste zavrnili" in se **ne** poskuša znova.
...
- **Dnevnik** — Beležite vsak webhook, ki ga prejmete. To močno olajša odpravljanje težav pri integraciji.
...
- [Webhooks API referenca](/sl/api/webhooks/) — Sheme vsebine po dogodku, glave in končne točke za upravljanje naročnin/dostav.
- [Webhooki](/sl/portal/settings/webhooks) — Celotna referenca za upravljanje webhookov.
- [Applications](/sl/account/applications) — Upravljajte svoje API poverilnice.
- [Developers > APIs](/sl/api/overview/) — Celotna API dokumentacija.
