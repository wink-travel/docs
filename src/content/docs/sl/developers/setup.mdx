...
:::note
Testiranje naj vedno poteka v našem [testnem okolju](/sl/getting-started/environments). Tam je veliko manj podatkov, vendar lahko testirate kolikor želite, ne da bi vplivali na produkcijsko okolje.
...
<Steps>
1. [Registrirajte](/sl/account/register) svoj uporabniški račun na Wink.
...
3. [Ustvarite aplikacijo](/sl/portal/settings/applications) in jo povežite s partnerskim računom, ki ste ga ustvarili v koraku 2. Za API integracije strežnik-strežnik izberite **MACHINE_2_MACHINE** kot tip odjemalca.
...
Pojdite v našo sekcijo [API-ji](/sl/api) in izberite API, s katerim želite delati.
...
:::tip[Pro nasvet]
Za lažjo uporabo priporočamo, da integratorji uporabljajo končne točke, ki jih označimo kot `OAuth2`. Te končne točke pričakujejo klice API in že vedo, na katerem računu ste, zato vam ni treba navajati `companyIdentifier` v URL-jih.
:::
...
:::tip[Pro nasvet]
Priporočamo, da strežniško domeno, client ID in client secret nastavite kot okoljske spremenljivke. Tako lahko enostavno preklapljate med testnim in produkcijskim okoljem ter ponovno uporabljate vse ustvarjene operacije.
:::
