...
2. **Vnesite ga v portal Wink**
   ...
   Prijavite se v [app.wink.travel](https://app.wink.travel), izberite svoj račun in pojdite na zavihek **Prilagoditve**. Prilepite svoj Sojern GTM Container ID v polje Sojern in shranite. Vaša integracija je zdaj aktivna.
...
- **Polje za Sojern GTM Container ID** je na voljo v zavihku Prilagoditve v vašem portalu Wink. Ko vnesete svoj Container ID, Wink preveri format in ga varno shrani za vašo nastanitev.
- **Na vsaki strani, kjer je vaša nastanitev aktivna** na Wink, se vaš Sojern GTM Container ID vključi v stran in Winkov sistem za upravljanje oznak dinamično naloži vaš Sojern container skupaj s svojim. To se zgodi samodejno — brez ročnih korakov s strani Wink za posamezno nastanitev.
...
Vsak dogodek sledi standardni strukturi GA4 Enhanced Ecommerce: vrhnji niz `event` in objekt `ecommerce`. Pred vsakim pošiljanjem Wink počisti prejšnjo vsebino ecommerce, da prepreči prelivanje zastarelih podatkov v nove sprožilce.

<Aside type="note">
**Vzorec ponastavitve dataLayer**

Wink pred vsakim pošiljanjem dogodka ponastavi ecommerce objekt:
...
Ne zanašajte se na ohranjanje lastnosti ecommerce med dogodki. Vedno jih preberite ob sprožitvi znotraj istega dogodka.
</Aside>
...
#### Po meri določeni parametri na nivoju elementa

Ti parametri nosijo Wink-specifičen kontekst rezervacije. GA4 jih ne prikaže samodejno v poročilih — morate jih registrirati kot **po meri določene dimenzije na nivoju elementa** v svoji GA4 lastnini, preden se pojavijo v analizah ali standardnih poročilih. Navodila za nastavitev so v poglavju [Custom Dimensions in GA4](#custom-dimensions-in-ga4).

| Lastnost | Tip | Opis |
|----------|-----|------|
| `property_id` | niz | Identifikator nastanitve Wink |
...
GA4 ignorira po meri določene parametre elementov v poročilih, dokler jih ne registrirate kot dimenzije na nivoju elementa. Registrirati morate le tiste parametre, ki jih želite uporabljati v poročilih — ni potrebno registrirati vseh 20.
...
3. **Izpolnite podatke o dimenziji**
   ...
   - **Description** — neobvezno, a priporočljivo
...
GA4 omogoča do 50 po meri določenih dimenzij na nivoju elementa na standardnih lastnostih (360 lastnosti omogočajo več).
</Aside>
