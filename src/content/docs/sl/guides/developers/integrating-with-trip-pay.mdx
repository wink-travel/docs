...
description: Ta članek vsebuje celovit primer, kako se integrirati s TripPay in prepustiti plačilo njim.
sidebar:
  order: 4
---
...
- [Plačilni spletni komponent](/sl/integrations/web-components/#payment).
- [Plačilni API](/sl/api). *Preberite razdelek o `Ustvarjanju plačljivih pogodb`*
...
- Vrstice: **2 - 7** so uporabnik, odgovoren za rezervacijo. Obstajata dve vrsti rezervatorjev:
    - Popotnik.
    - Potovalni agent.
- Vrstice: **8 - 9** je TripPay račun, ki omogoča rezervacijo. Lahko je:
...
- `redirectUrl` sporoči TripPay, na katero stran naj preusmeri po zaključku plačila.
...
- `identifier` je veljaven UUID, ki ga ustvarite.
...
- Vsak vnos v `contractItemList` vsebuje:
    ...
    - **Seznam dnevnih cen** Če je element, ki se rezervira, soba, lahko vključite, koliko stane ta soba za vsako noč bivanja gosta.
...
Ko TripPay odgovori s plačljivo pogodbo, boste opazili, da je dodal svojega upravičenca z 4,0 %.
:::

### Odgovor

Ko TripPay odgovori na vašo zahtevo, bo izgledal nekako takole:
...
Shranjeno vrednost `id` na vrstici 3. To boste uporabili za vnos v TripPay spletni komponent.
...
Kar zadeva integracijo, je to vse, kar morate storiti. Vso težko delo od tu naprej opravi TripPay.
