...
Эта конфигурация работает с любым пользовательским GTM Container ID. Мы использовали Sojern в качестве примера, так как это очень распространённая интеграция.
</Aside>
...
`payment_type` заполняется типом платёжного агрегатора. `coupon` присутствует только при применении кода скидки.
...
GA4 игнорирует пользовательские параметры на уровне элемента в отчётах, пока вы явно не зарегистрируете их как пользовательские измерения с областью элемента. Регистрировать нужно только те параметры, которые вы планируете использовать в отчётах — не обязательно регистрировать все 18.
...
<Aside type="note">
Появление пользовательских измерений в отчётах GA4 может занять до 24 часов после регистрации. Исторические данные, собранные до регистрации, не заполняются задним числом.

GA4 позволяет иметь до 50 пользовательских измерений с областью элемента на стандартных свойствах (для 360-свойств лимит выше).
</Aside>
...
| Имя измерения | Event parameter | Тип |
|---------------|-----------------|-----|
