...
<Steps>
1. [Зарегистрируйте](/ru/getting-started/sign-up) свой пользовательский аккаунт на Wink.
2. [Создайте партнёрский аккаунт](/ru/portal/create-account). Он понадобится для привязки вашего приложения. Подробнее смотрите в разделе [что такое Studio](/ru/portal/studio/what-is-studio).
3. [Создайте приложение](/ru/account/applications) и свяжите его с партнёрским аккаунтом, созданным на шаге 2. Для интеграций API сервер-сервер выберите тип клиента **MACHINE_2_MACHINE**.
...
Перейдите в раздел наших [API](/ru/api/overview/) и выберите API, с которым хотите работать.
...
13. Установите поле `Scope` в пробел-разделённые области, необходимые вашей интеграции (например, `inventory.read inventory.write inventory.remove`). Полный список смотрите в [справочнике по областям](/ru/developers/authentication#scopes).
...
16. Для всех вызовов API в коллекции Wink API убедитесь, что вкладка аутентификации установлена в `Inherit auth from parent`, чтобы токен автоматически добавлялся к каждому вызову.
</Steps>
