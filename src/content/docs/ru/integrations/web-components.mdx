...
Примеры можно найти для [карточек](/ru/portal/studio/cards), [сеток](/ru/portal/studio/grids) и [карт](/ru/portal/studio/maps).

### Поиск

Компонент Lookup отлично работает вместе с одной из ваших ранжированных сеток. Он позволяет пользователям искать отели и направления, а сетка обновляется в ответ на результаты поиска.
...
Кнопка маршрута отображает текущий маршрут на кнопке. При клике открывается полный выбор маршрута в модальном окне.
...
Клик по кнопке выше открывает модальное окно, в котором можно обновить маршрут.
...
Для любой страницы, на которой вы хотите использовать платежный веб-компонент TripPay, необходимо включить два элемента.
...
Платежный компонент позволяет сообщить TripPay, что путешественник хочет что-то купить, а также правила и цены для этих товаров.

У виджета есть один обязательный атрибут:
...
:::note
Чтобы узнать, как сгенерировать `id` контракта бронирования, прочитайте руководство: [Интеграция с TripPay](/ru/guides/developers/integrating-with-trip-pay).
:::
...
Если при встраивании одного из наших веб-компонентов, как на WinkLinks, так и на вашем собственном сайте, вы видите сообщение об ошибке, возможно, произошли следующие проблемы:

### Доступность

Инвентарь может быть больше недоступен. Перейдите в [Studio](/ru/portal/studio/cards) и проверьте статус "карточки".
Если статус недоступен, он будет выделен красным. В таком случае вы можете подождать, возможно, поставщик снова сделает его доступным, или удалить его из вашего списка.
...
Инвентарь мог быть удалён. Перейдите в [Studio](/ru/portal/studio/cards) и проверьте статус "карточки".
Если статус недоступен, он будет выделен красным. В таком случае вы можете подождать, возможно, поставщик снова сделает его доступным, или удалить его из вашего списка.
...
Это касается только веб-разработчиков. Если вы случайно удалили [Приложение](/ru/account/applications), и клиентский ID стал недоступен, создайте новое приложение и используйте новый client ID для встраивания наших веб-компонентов.
