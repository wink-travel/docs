...
<Steps>
1. [Daftar](/ms/account/register) akaun pengguna anda di Wink.
2. [Buat akaun affiliate](/ms/portal/create-account). Anda memerlukan ini untuk mengikat Aplikasi anda. Lihat [apa itu Studio](/ms/portal/studio/what-is-studio) untuk konteks lanjut.
3. [Buat Aplikasi](/ms/portal/settings/applications) dan kaitkan dengan akaun affiliate yang anda buat di Langkah 2. Untuk integrasi API server-ke-server, pilih **MACHINE_2_MACHINE** sebagai jenis klien.
...
Teruskan ke bahagian [API](/ms/api) kami dan pilih API yang anda ingin gunakan.
...
:::tip[Petua Pakar]
Untuk kemudahan penggunaan, kami mengesyorkan integrator menggunakan titik akhir yang kami tetapkan sebagai `OAuth2`. Titik akhir tersebut sudah tahu untuk mengharapkan pemanggil API dan sudah tahu akaun mana yang anda gunakan jadi anda tidak perlu menyatakan `companyIdentifier` dalam URL anda.
:::
