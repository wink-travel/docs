...
Ini akan mengembalikan token akses, bersama dengan data respons OAuth2 yang lain:
...
Apabila anda membuat panggilan ke mana-mana titik akhir kami, sertakan perkara berikut dalam header:
...
| **Analitik** | `analytics.read` `analytics.write` `analytics.remove` | Laporan dan analitik — pendapatan, papan pendahulu | Rendah–Sederhana |
...
- **Minta yang minimum.** Minta hanya skop yang benar-benar digunakan oleh integrasi anda — token yang dikeluarkan
  dengan akses lebih luas daripada yang diperlukan mempunyai risiko kebocoran yang lebih besar.
- **Skop yang tidak dikenali akan ditolak.** Meminta skop yang aplikasi anda tidak didaftarkan, atau yang
...
Dapatkan 'Client ID' dan 'Secret Key' dari salah satu aplikasi anda dan navigasi ke mana-mana [API kami](/ms/api/overview/).
:::
