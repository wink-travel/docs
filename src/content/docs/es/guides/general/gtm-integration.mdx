...
3. **(Opcional) Agrega un ID de Medición GA4 + Secreto API para Compra del Lado del Servidor**
   ...
   Los contenedores GTM se ejecutan completamente en el navegador del huésped, por lo que una reserva completada puede perderse si el huésped cierra la pestaña durante la redirección de pago. Si tu contenedor carga una **propiedad GA4** y quieres que las reservas completadas se registren de forma confiable, también ingresa el **ID de Medición** y el **Secreto de Protocolo de Medición API** de esa GA4 en la sección GTM. Wink enviará entonces el evento `purchase` para esa propiedad **del lado del servidor**, exactamente como se describe en la guía de [Integración de Analytics GA4](/es/guides/developers/ga4-dual-tagging). Déjalos en blanco si solo necesitas seguimiento en el navegador.
...
- **Si proporcionaste un ID de Medición GA4 + secreto API** (paso 3 arriba), el evento `purchase` para reservas completadas se envía adicionalmente **del lado del servidor** vía el Protocolo de Medición GA4, para que una venta se registre incluso si el huésped no regresa a la página de confirmación.
...
Estos campos son entendidos nativamente por GA4 y aparecen en reportes ecommerce estándar sin configuración adicional.
...
<Aside type="note">
Las dimensiones personalizadas pueden tardar hasta 24 horas en comenzar a aparecer en los reportes GA4 después de registrarlas. Los datos históricos recolectados antes del registro no se rellenan retroactivamente.
