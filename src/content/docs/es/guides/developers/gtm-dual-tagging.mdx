---
title: Integración del Pixel Sojern
description: Cómo conectar tu cuenta Sojern a Wink para que Sojern pueda rastrear el recorrido de reserva de tus huéspedes y potenciar tus campañas de retargeting y publicidad.
sidebar:
  label: Integración del Pixel Sojern
---

import { Aside, Steps } from '@astrojs/starlight/components';

Si estás ejecutando campañas publicitarias a través de Sojern, puedes conectar tu cuenta Sojern a Wink ingresando tu ID de Contenedor GTM de Sojern en el portal de Wink. Una vez conectado, el pixel de seguimiento de Sojern se activa automáticamente para tu propiedad, cubriendo todo el recorrido del huésped desde la búsqueda hasta la reserva completada.

<Aside type="tip">
Esto asume que ya tienes una cuenta activa de Sojern y has recibido tu ID de Contenedor GTM de tu gerente de cuenta de Sojern. Si aún no tienes una cuenta Sojern, visita [sojern.com/contact](https://www.sojern.com/contact) para comenzar.
</Aside>
...
1. **Obtén tu ID de Contenedor GTM de Sojern**
   ...
   Contacta a tu gerente de cuenta de Sojern y solicita tu **ID de Contenedor GTM**. Tiene el formato `GTM-XXXXXXX`. Esto es diferente de tu ID de Pixel Sojern — asegúrate de pedir específicamente el ID de Contenedor GTM.

2. **Ingresa el ID en el Portal Wink**

   Inicia sesión en [app.wink.travel](https://app.wink.travel), selecciona tu cuenta y ve a la pestaña **Personalizaciones**. Pega tu ID de Contenedor GTM de Sojern en el campo Sojern y guarda. Tu integración ya está activa.

3. **Confirma con tu Gerente de Cuenta Sojern**

   Informa a tu gerente de cuenta Sojern que la integración está activa. Ellos pueden verificar desde su lado que el pixel se está activando correctamente en `wink.travel` para tu propiedad y que los datos están fluyendo a tu cuenta Sojern.

</Steps>

<Aside type="note">
No necesitas colocar ningún código de pixel en tu propio sitio web para esta integración. Wink carga tu contenedor Sojern automáticamente en todas las páginas donde tu propiedad está activa.
...
- **En cada página donde tu propiedad está activa** en Wink, tu ID de Contenedor GTM de Sojern se inserta en la página y el sistema de gestión de etiquetas de Wink carga dinámicamente tu contenedor Sojern junto con el suyo propio. Esto ocurre automáticamente — no se requieren pasos manuales del equipo Wink por propiedad.
- **Tu contenedor Sojern se carga de forma asíncrona**, lo que significa que no afecta el rendimiento de carga de página para tus huéspedes.
- **El seguimiento propio de Wink y tu pixel Sojern funcionan en paralelo**, leyendo desde los mismos datos compartidos de eventos. Cada evento de reserva que Wink rastrea está simultáneamente disponible para tu contenedor Sojern.
- **Cuando tu propiedad no está siendo vista** — por ejemplo, cuando un huésped navega otro hotel en Wink — tu contenedor Sojern no se carga. Esto asegura que Sojern solo reciba datos relevantes para tu propiedad.
...
Cada evento sigue la estructura estándar GA4 Enhanced Ecommerce: una cadena `event` de nivel superior y un objeto `ecommerce`. Antes de cada push, Wink limpia la carga útil ecommerce previa para evitar que datos antiguos afecten nuevos disparadores.
    ...
    "items": [
      {
        ...
        "property_name": "Grand Resort Maldivas",
