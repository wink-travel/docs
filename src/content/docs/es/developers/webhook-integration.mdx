...
- `object` — resumen curado del recurso al que se refiere el evento (identificadores, estado, campos sobre los que actúas) más `links.self`, la URL REST canónica del lado del proveedor del recurso completo. Consúltala con tus propias credenciales API cuando necesites más que el resumen; si recibes el evento como revendedor o agente de viajes, usa el endpoint correspondiente de tu propia API para el mismo identificador.
...
- Un `5xx`, un timeout, `408` o `429` se reintenta con retroceso: después de 1 minuto, 5 minutos, 30 minutos, 2 horas, 6 horas, 12 horas, luego diario — 10 intentos en unos 3 días — tras lo cual la entrega se marca como **muerta**.
...
Puedes **desactivar** un webhook sin eliminarlo. Esto pausa la entrega para que puedas solucionar problemas sin perder tu configuración. Cuando estés listo, actívalo de nuevo.
...
- [Developers > APIs](/es/api/overview/) — Documentación completa de la API.
