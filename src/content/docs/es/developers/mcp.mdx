---
title: Servidor MCP
description: Conecta rápidamente agentes de IA a la documentación de Wink y a especificaciones OpenAPI en vivo mediante el servidor MCP alojado.
sidebar:
  order: 10
---
...
- Navegar por especificaciones OpenAPI en vivo y documentación de operaciones (`wink://api/...`)
- Realizar búsquedas semánticas / por palabras clave ligeras sobre la documentación y APIs en vivo
...
Dentro de Claude Code, ejecuta `/mcp` para inspeccionar el estado del servidor. Si quieres compartir el servidor con todo el repositorio, añade `--scope project` para que Claude Code escriba la configuración en `.mcp.json`.
...
- El selector de herramientas te permite activar/desactivar `api_search` y `docs_search`.
- Añade contexto: Vista de chat → Añadir contexto → Recursos MCP para adjuntar páginas de documentación.
...
| `wink://api/{apiName}/operation/{key}/request` | Vista centrada en la solicitud con entradas requeridas y carga útil de ejemplo mínima |
...
Para resultados de operaciones, `api_search` también devuelve URIs vinculados de solicitud, respuesta, esquema y receta para que los agentes puedan acceder directamente al subrecurso más útil.
