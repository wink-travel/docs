...
consulta los ejemplos de [`curl`](#staging) arriba y el [recorrido en Postman](/es/developers/setup#postman).
...
| **MCP** | `mcp.read` `mcp.write` `mcp.remove` | Abre el [transporte MCP](/es/developers/mcp) (`/mcp`) para agentes de IA | Bajo–Medio |
...
- **`mcp.*` es solo para clientes agentes de IA.** Lo necesitas para abrir el transporte MCP; no es requerido
  para llamadas REST API regulares. Consulta [Model Context Protocol](/es/developers/mcp).

:::tip
Puedes probar todas nuestras APIs sin escribir una sola línea de código antes de comenzar.

Obtén un 'Client ID' y 'Secret Key' de una de tus aplicaciones y navega a cualquiera de nuestras [APIs](/es/api/overview/).
:::
