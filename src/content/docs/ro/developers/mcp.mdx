---
title: Server MCP
description: Conectează rapid agenții AI la documentația Wink și specificațiile OpenAPI live prin serverul MCP găzduit.
sidebar:
  order: 10
---
...
- Efectueze căutări semantice / pe cuvinte cheie ușoare în documentații și API-uri live
...
Configurație minimă la distanță (utilizator sau spațiu de lucru `mcp.json`):
```json
{
  "servers": {
    "wink-docs": {
...
| `wink://api/{apiName}` | Rezumat compact API cu autentificare, etichete, antete și operațiuni exemplu |
...
| `docs_search` | Găsește documentații conceptuale / ghiduri | `authentication`, `webhooks`, `wordPress` |

Lasă `query` gol pentru a primi sugestii. Fiecare rezultat include URI-ul MCP pe care îl poți deschide.
Pentru rezultate operațiuni, `api_search` returnează și URI-uri legate de cerere, răspuns, schemă și rețetă astfel încât agenții să poată accesa direct subresursa cea mai utilă.
...
- Arborele `wink://api` este live și urmează catalogul OpenAPI upstream curent în loc de un index generat al depozitului.
