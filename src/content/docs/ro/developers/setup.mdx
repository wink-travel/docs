...
<Steps>
1. [Înregistrează-te](/ro/my-account/register) cu un cont de utilizator pe Wink.
2. [Creează un cont afiliat](/ro/my-account/create-account). Vei avea nevoie de acesta pentru a lega Aplicația ta. Vezi [ce este Studio](/ro/studio/what-is-studio) pentru mai mult context.
3. [Creează o Aplicație](/ro/my-account/applications) și asociaz-o cu contul afiliat creat la Pasul 2. Pentru integrări API server-to-server, alege **MACHINE_2_MACHINE** ca tip client.
...
Accesează secțiunea noastră de [API-uri](/ro/developers/apis) și alege API-ul cu care vrei să lucrezi.
...
2. Apasă pe `Download OpenAPI spec` din partea de sus a paginii [Lookup API](https://developer.wink.travel/lookup).
