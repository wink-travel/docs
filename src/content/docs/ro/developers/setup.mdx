---
title: Configurare
description: Acest articol îți arată cum să te configurezi rapid ca dezvoltator și să începi să testezi API-urile noastre pe Wink.
sidebar:
  order: 1
---

import { Steps } from '@astrojs/starlight/components';
import CloudinaryImage from '/src/components/cloudinary-image.astro';

Acest articol îți arată cum să te configurezi rapid ca dezvoltator și să începi să testezi API-urile noastre pe Wink.

:::note
Testarea trebuie să aibă loc întotdeauna în [mediul nostru de testare](/ro/getting-started/environments). Acolo există mult mai puține date, dar poți testa cât dorești fără a afecta mediul de producție.

Odată ce ești gata să lansezi, îți vom certifica contul și aplicația, iar tu poți lansa în mediul nostru de producție.
:::

Urmează acești pași:

<Steps>
1. [Înregistrează-te](/ro/getting-started/sign-up) cu contul tău de utilizator pe Wink.
2. [Creează un cont afiliat](/ro/portal/create-account). Vei avea nevoie de acesta pentru a asocia Aplicația ta. Vezi [ce este Studio](/ro/portal/studio/what-is-studio) pentru mai multe detalii.
3. [Creează o Aplicație](/ro/account/applications) și asociază-o cu contul afiliat creat la Pasul 2. Pentru integrări API server-to-server, alege **MACHINE_2_MACHINE** ca tip client.
4. După ce ai creat `Aplicația`, copiază și salvează imediat client-id-ul și cheia secretă — cheia secretă este afișată o singură dată și nu poate fi recuperată ulterior.
</Steps>

## CDN

La Wink, folosim Cloudinary pentru toate imaginile și videoclipurile noastre. Când accesezi inventarul nostru de călătorii, vei avea acces complet la toate funcțiile avansate ale Cloudinary.

`cloud_name` nostru este `traveliko`.

### Cloudinary

- [Începe aici](https://cloudinary.com/documentation/dev_kickstart)
- [SDK-uri](https://cloudinary.com/documentation/cloudinary_sdks)
- [Cloudinary pe GitHub](https://github.com/cloudinary)
-	[Tutoriale video](https://cloudinary.com/documentation/cloudinary_sdks_tutorials)

### Exemplu

Pentru fiecare tip de inventar de călătorii pe care îl susținem, există un array JSON imbricat numit `multimedias`. JSON-ul pentru o intrare multimedia arată astfel:

```json {3}
{
  "multimediaIdentifier": "multimedia-1",
  "identifier": "partners/radisson_blu_logo",
  "type": "IMAGE",
  "width": "1024",
  "height": "768",
  "category": "1",
  "descriptions": [
    {
      "name": "Lobby",
      "description": "Picture of the lobby",
      "language": "en"
    }
  ],
  "lifestyleType": "LIFESTYLE_BUSINESS",
  "attribution": [
    {
      "url": "https://www.hilton.com",
      "name": "Hilton"
    }
  ]
}
```

Linia evidențiată de mai sus arată ID-ul unic Cloudinary `partners/radisson_blu_logo`. Acesta este tot ce ai nevoie pentru a prelua imaginea în orice format, calitate și dimensiune dorești.

## Testare

Accesează secțiunea noastră de [API-uri](/ro/api/overview/) și alege API-ul cu care vrei să lucrezi.

### Documentație API

Site-ul nostru de documentație este configurat astfel încât poți rula comenzi API direct din site.

În acest exemplu, vrei să folosești apelul API `Search Lookups (OAuth2)`.

:::tip[Pro Tip]
Pentru ușurință în utilizare, recomandăm integratorilor să folosească endpoint-urile pe care le desemnăm ca `OAuth2`. Aceste endpoint-uri știu să aștepte apelanți API și deja știu pe ce cont ești, deci nu trebuie să specifici `companyIdentifier` în URL-uri.
:::

<Steps>
1. Folosește [Lookup API](https://developer.wink.travel/lookup) pentru a prelua destinații sau hoteluri.
2. În bara de navigare din stânga, dă click pe `Authentication`.
3. Introdu client-id-ul și cheia secretă și apasă butonul `Get TOKEN`.
4. Dacă datele tale sunt valide, vei vedea că cheia API a fost aplicată.
5. Acum poți testa endpoint-ul lookup.
</Steps>

<figure>
<CloudinaryImage
      width="700"
      height="265"
      resize="scale"
      quality="best"
      alt="Lookup API endpoint"
      src="academy/lookup-api-call"
      errorImage="noimage_opaque_nyrtl0.png"
/>
<figcaption>*Exemplu Search Lookups (OAuth2)*</figcaption>
</figure>

### Postman
Poți face același lucru și cu Postman.

<Steps>
1. Descarcă [Postman](https://www.postman.com/downloads).
2. Dă click pe `Download OpenAPI spec` din partea de sus a paginii [Lookup API](https://developer.wink.travel/lookup).
3. Importă-l în Postman.
4. Dacă l-ai importat ca o colecție nouă numită `Wink API`, dă click pe elipsa orizontală (3 puncte) de lângă numele colecției și selectează `Edit`.
5. În fila `Authorization`, setează `Auth type` la OAuth2.
6. Setează câmpul `Add auth data to` la `Request headers`.
7. Setează câmpul `Header prefix` la `Bearer`.
8. Setează câmpul `Token Name` cu orice nume dorești pentru token-ul salvat în Postman, de ex. Wink Staging.
9. Setează câmpul `Grant type` la `Client Credentials`.
10. Setează câmpul `Access token URL` la https://staging-iam.wink.travel/oauth2/token.
11. Setează câmpul `Client ID` cu Client ID-ul generat de aplicația ta.
12. Setează câmpul `Client Secret` cu cheia secretă generată de aplicația ta.
13. Setează câmpul `Scope` cu scope-urile separate prin spațiu de care are nevoie integrarea ta (ex. `inventory.read inventory.write inventory.remove`). Vezi [referința scope-urilor](/ro/developers/authentication#scopes) pentru lista completă.
14. Setează câmpul `Client Authentication` la `Send as Basic Auth header`.
15. Apasă butonul `Get New Access Token` pentru a obține token-ul și salvează-l.
16. Pentru toate apelurile API din colecția Wink API, asigură-te că fila de autentificare spune `Inherit auth from parent` pentru a include token-ul la fiecare apel.
</Steps>

:::tip[Pro Tip]
Se recomandă să transformi domeniul serverului, client ID-ul și cheia secretă în variabile de mediu. Astfel poți comuta ușor între mediile de testare și producție și poți reutiliza toate operațiunile create.
:::
