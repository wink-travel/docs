...
Webhook-urile permit sistemelor tale să primească notificări în timp real când apar evenimente în contul tău Wink — rezervări noi, anulări, actualizări de plată și altele. Acest ghid te conduce prin configurare și bune practici.
...
2. Navighează la `Applications` și apoi la `Webhooks`. Vezi [Webhooks](/ro/portal/settings/webhooks).
...
Wink publică în prezent 70 de tipuri de evenimente webhook în domeniile rezervărilor, proprietăților, conturilor (entități de management) și inventarului (tipuri de camere, planuri tarifare, tarife master, suplimente, facilități, canale de vânzare, promoții). Cele comune:
...
Verifică în patru pași: parsează `t` și fiecare `v1`; recalculează HMAC-ul peste `t.rawBody` cu secretul tău; compară cu o comparație în timp constant; respinge dacă `|now − t|` depășește toleranța ta (5 minute recomandat).
...
- Fiecare eveniment, livrare și încercare (status, fragment de răspuns) este vizibil în **Applications > Webhooks** și prin API (`…/webhook/event/grid`, `…/webhook/delivery/grid`). Poți **redelivera** orice livrare (`POST …/webhook/delivery/{deliveryId}/redeliver`, care începe o nouă serie de retry-uri), redelivera toate livrările dead ale unui webhook simultan (`POST …/webhook/{webhookId}/redeliver-dead`), sau anula una.
...
- **Idempotență** — Handler-ul tău trebuie să fie idempotent; deduplicatează după `Wink-Event-Id`. Wink reîncearcă dacă nu primește un răspuns `2xx`.
...
- **Logare** — Înregistrează fiecare payload webhook primit. Acest lucru face depanarea problemelor de integrare mult mai ușoară.

## Pauzare și ștergere

Poți **dezactiva** un webhook fără să-l ștergi. Aceasta oprește livrarea astfel încât să poți depana fără să pierzi configurația. Când ești gata, activează-l din nou.
...
- [Referința API Webhooks](/ro/api/webhooks/) — Schemele de payload per eveniment, headere și endpoint-urile de gestionare a abonamentelor/livrărilor.
...
- [Developers > APIs](/ro/api/overview/) — Documentație completă API.
