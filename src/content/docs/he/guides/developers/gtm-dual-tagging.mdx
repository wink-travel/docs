...
<Aside type="tip">
הנחה היא שכבר יש לך חשבון Sojern פעיל וקיבלת את מזהה מכולת ה-GTM שלך ממנהל החשבון של Sojern. אם עדיין אין לך חשבון Sojern, בקר ב-[sojern.com/contact](https://www.sojern.com/contact) כדי להתחיל.
</Aside>
...
2. **הזן אותו בפורטל Wink**
   ...
   היכנס ל-[app.wink.travel](https://app.wink.travel), בחר את החשבון שלך ועבור ללשונית **התאמות אישיות**. הדבק את מזהה מכולת ה-GTM של Sojern בשדה Sojern ושמור. האינטגרציה שלך כעת פעילה.

3. **אשר עם מנהל החשבון שלך ב-Sojern**

   הודע למנהל החשבון שלך ב-Sojern שהאינטגרציה פעילה. הם יוכלו לוודא מצידם שהפיקסל פועל כראוי ב-`wink.travel` עבור הנכס שלך ושהנתונים זורמים לחשבון Sojern שלך.
...
- **מכולת Sojern שלך נטענת באופן אסינכרוני**, כלומר היא לא משפיעה על ביצועי טעינת הדף עבור האורחים שלך.
...
שדות אלה מובנים באופן טבעי על ידי GA4 ומופיעים בדוחות ecommerce סטנדרטיים ללא הגדרה נוספת.
...
| `creative_name` | מחרוזת | — | שם קריאייטיב בקידום |
...
פרמטרים אלה נושאים הקשר הזמנה ספציפי ל-Wink. GA4 לא מציג אותם אוטומטית בדוחות — יש לרשום כל אחד מהם כ**ממד מותאם אישית בהיקף פריט** בנכס GA4 שלך לפני שיופיע בדוחות או בחקירות. ראה [ממדים מותאמים ב-GA4](#custom-dimensions-in-ga4) למטה להוראות הגדרה.
...
השתמש בטבלה הבאה כהפניה בעת יצירת ממדים מותאמים. ערך **Event parameter** חייב להתאים במדויק — GA4 רגיש לאותיות גדולות וקטנות.
