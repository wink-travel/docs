...
description: מאמר זה מכיל דוגמה מלאה כיצד להתממשק עם TripPay ולתת לו לטפל בתשלום עבורך.
sidebar:
  order: 4
---
...
- [Payment Web Component](/he/integrations/web-components/#payment).
- [Payment API](/he/api). *קרא את הסעיף על `Create payable contracts`*
...
### בקשה
להלן דוגמת בקשת JSON הכוללת את כל מה שצריך ליצירת חוזה לתשלום ראשון.
...
לא נסביר כאן כל נקודת נתונים כי הן כבר מכוסות בתיעוד ה-API.

- שורות: **2 - 7** הן המשתמש האחראי על ההזמנה. יש שני סוגי מזמינים:
    - מטייל.
    - סוכן נסיעות.
...
- `sourceUrl` הוא האתר/האפליקציה שבה התבצעה ההזמנה.
...
- כל רשומה תחת `contractItemList` מכילה:
    ...
    - **לתשלום** מתי המטייל יחויב. *כעת אנו תומכים רק באפשרויות תשלום מיידיות.*
    - **מדיניות** כללי מדיניות ביטול.
    - **רשימת תעריפים יומית** אם הפריט הוא חדר, ניתן לכלול כמה עלה החדר לכל לילה שהאורח שוהה.
    - **מוטבים** כלול אילו חשבונות TripPay יקבלו איזה סכום(ים) ממחיר הפריט. *נתמכים סכומים קבועים ואחוזים.*

:::note
בדוגמת ה-JSON למעלה, יש לנו 3 מוטבים:
...
כאשר TripPay מגיב עם חוזה לתשלום, תבחין שהוא הזריק מוטב משלו בשיעור 4.0%.
:::
