...
:::note
הבדיקות צריכות להתבצע תמיד ב[סביבת הביניים](/he/getting-started/environments). יש שם הרבה פחות נתונים אך אתם חופשיים לבדוק כמה שתרצו מבלי להשפיע על סביבת הייצור.
...
- [התחלת עבודה](https://cloudinary.com/documentation/dev_kickstart)
...
גשו לקטגוריית ה-[APIs](/he/api/overview/) שלנו ובחרו את ה-API שברצונכם לעבוד איתו.
...
12. הגדירו את השדה `Client Secret` למפתח הסודי שנוצר עבור האפליקציה שלכם.
13. הגדירו את השדה `Scope` לטווחים המופרדים ברווח שהאינטגרציה שלכם צריכה (למשל `inventory.read inventory.write inventory.remove`). ראו את [רשימת הטווחים](/he/developers/authentication#scopes) לקבלת הרשימה המלאה.
...
16. בכל קריאות ה-API תחת אוסף Wink API, ודאו שכרטיסיית האימות מציינת `Inherit auth from parent` כך שהטוקן שלכם ייכלל בכל קריאה.
</Steps>
