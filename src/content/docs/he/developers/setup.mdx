...
:::note
הבדיקות תמיד צריכות להתבצע ב[סביבת הביניים](/he/getting-started/environments). יש שם הרבה פחות נתונים אך אתם חופשיים לבדוק כמה שתרצו מבלי להכביד על סביבת הייצור.

כאשר תהיו מוכנים להשקה, נאשר את החשבון והאפליקציה שלכם ותוכלו להשיק בסביבת הייצור שלנו.
:::

עקבו אחר השלבים הבאים:

<Steps>
1. [הרשמו](/he/my-account/register) לחשבון המשתמש שלכם ב-Wink.
2. [צרו חשבון שותף](/he/my-account/create-account). תזדקקו לכך כדי לקשר אליו את האפליקציה שלכם. ראו [מה זה Studio](/he/studio/what-is-studio) לקבלת הקשר נוסף.
3. [צרו אפליקציה](/he/my-account/applications) וקשרו אותה לחשבון השותף שיצרתם בשלב 2. לאינטגרציות API מסוג שרת-לשרת, בחרו ב-**MACHINE_2_MACHINE** כסוג הלקוח.
...
גשו לקטגוריית ה-[APIs](/he/developers/apis) שלנו ובחרו את ה-API שברצונכם לעבוד איתו.
