...
<Steps>
1. [הרשמו](/he/account/register) לחשבון המשתמש שלכם ב-Wink.
2. [צרו חשבון שותף](/he/portal/create-account). תזדקקו לכך כדי לקשר אליו את האפליקציה שלכם. ראו [מה זה Studio](/he/portal/studio/what-is-studio) לקבלת הקשר נוסף.
3. [צרו אפליקציה](/he/portal/settings/applications) וקשרו אותה לחשבון השותף שיצרתם בשלב 2. לאינטגרציות API מסוג שרת-לשרת, בחרו ב-**MACHINE_2_MACHINE** כסוג הלקוח.
...
השורה המודגשת למעלה מציגה את מזהה ה-Cloudinary הייחודי `partners/radisson_blu_logo`. זה כל מה שאתם צריכים כדי לשלוף את התמונה בכל פורמט, איכות וגודל שתרצו.

## בדיקות

גשו לקטגוריית ה-[APIs](/he/api) שלנו ובחרו את ה-API שברצונכם לעבוד איתו.
...
:::tip[טיפ מקצועי]
לנוחות השימוש, אנו ממליצים לממשקים להשתמש בנקודות הקצה שאנו מגדירים כ-`OAuth2`. נקודות קצה אלו יודעות לצפות למתקשרים ל-API וכבר יודעות באיזה חשבון אתם נמצאים כך שאין צורך לציין את ה-`companyIdentifier` בכתובות ה-URL שלכם.
:::
...
3. הזינו את ה-client-id ואת הסוד של הלקוח ולחצו על כפתור `Get TOKEN`.
...
11. הגדירו את השדה `Client ID` ל-Client ID שהאפליקציה שלכם סייעה ליצור.
12. הגדירו את השדה `Client Secret` למפתח הסודי שהאפליקציה שלכם סייעה ליצור.
13. הגדירו את השדה `Scope` לטווחים המופרדים ברווח שהאינטגרציה שלכם זקוקה להם (למשל `inventory.read inventory.write inventory.remove`). ראו את [רשימת הטווחים](/he/developers/authentication#scopes) לקבלת הרשימה המלאה.
