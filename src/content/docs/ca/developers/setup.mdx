...
Quan estiguis llest per passar a producció, certificarem el teu compte i la teva aplicació i podràs llançar-la en el nostre entorn de producció.
:::

Segueix aquests passos:

<Steps>
1. [Registra’t](/ca/my-account/register) el teu compte d’usuari a Wink.
2. [Crea un compte d’afiliat](/ca/my-account/create-account). Necessitaràs aquest per vincular-hi la teva Aplicació. Consulta [què és Studio](/ca/studio/what-is-studio) per a més context.
3. [Crea una Aplicació](/ca/my-account/applications) i associa-la amb el compte d’afiliat que vas crear al Pas 2. Per a integracions API servidor a servidor, tria **MACHINE_2_MACHINE** com a tipus de client.
...
Ves a la nostra secció d’[APIs](/ca/developers/apis) i tria l’API amb la qual vols treballar.
