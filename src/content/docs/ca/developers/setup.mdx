...
<Steps>
1. [Registra’t](/ca/user/register) amb el teu compte d’usuari a Wink
2. Crea un [compte d’agent de viatges](/ca/travel-agent/what-is-agent) o un [compte d’afiliat](/ca/studio/what-is-studio).
3. [Crea una Aplicació](/ca/developer/applications) on l’associïs amb el compte que vas crear al Pas 2.
...
Ves a la nostra secció d’[APIs](/ca/developer/apis) i tria l’API amb la qual vols treballar.
...
16. Per a totes les crides API sota la col·lecció Wink API, assegura’t que lapestanya d’autenticació digui `Inherit auth from parent` perquè inclogui el teu token en cada crida.
</Steps>
