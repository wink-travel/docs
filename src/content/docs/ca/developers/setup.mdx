...
:::note
Les proves sempre s’han de fer en el nostre [entorn de staging](/ca/getting-started/environments). Hi ha molta menys informació, però pots provar tant com vulguis sense afectar l’entorn de producció.
...
<Steps>
1. [Registra’t](/ca/account/register) el teu compte d’usuari a Wink.
2. [Crea un compte d’afiliat](/ca/portal/create-account). Necessitaràs això per vincular la teva Aplicació. Consulta [què és Studio](/ca/portal/studio/what-is-studio) per a més context.
3. [Crea una Aplicació](/ca/portal/settings/applications) i associa-la amb el compte d’afiliat que vas crear al Pas 2. Per a integracions API servidor a servidor, tria **MACHINE_2_MACHINE** com a tipus de client.
4. Un cop hagis creat la teva `Aplicació`, copia i desa immediatament el client-id i la clau secreta — la clau secreta només es mostra una vegada i no es pot recuperar.
</Steps>
...
Ves a la nostra secció d’[APIs](/ca/api) i tria l’API amb la qual vols treballar.
...
Per facilitar l’ús, recomanem que els integradors utilitzin els endpoints que designem com a `OAuth2`. Aquests endpoints esperen crides API i ja saben a quin compte estàs, així no cal que especifiquis el `companyIdentifier` a les teves URLs.
:::

<Steps>
1. Utilitza la [Lookup API](https://developer.wink.travel/lookup) per recuperar destinacions o hotels.
2. A la barra lateral esquerra, fes clic a `Authentication`.
...
11. Configura el camp `Client ID` amb el Client ID que la teva aplicació ha generat.
12. Configura el camp `Client Secret` amb el Client Secret que la teva aplicació ha generat.
13. Configura el camp `Scope` amb els àmbits separats per espais que necessita la teva integració (per exemple, `inventory.read inventory.write inventory.remove`). Consulta la [referència d’àmbits](/ca/developers/authentication#scopes) per a la llista completa.
...
16. Per a totes les crides API dins de la col·lecció Wink API, assegura’t que la pestanya d’autenticació digui `Inherit auth from parent` perquè inclogui el teu token en cada crida.
</Steps>
