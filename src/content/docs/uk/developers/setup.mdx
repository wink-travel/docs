---
title: Налаштування
description: Ця стаття показує, як швидко налаштуватися як розробник і почати тестувати наші API на Wink.
sidebar:
  order: 1
---

import { Steps } from '@astrojs/starlight/components';
import CloudinaryImage from '/src/components/cloudinary-image.astro';

Ця стаття показує, як швидко налаштуватися як розробник і почати тестувати наші API на Wink.

:::note
Тестування завжди має відбуватися в нашому [тестовому середовищі](/uk/getting-started/environments). Там значно менше даних, але ви можете тестувати скільки завгодно, не впливаючи на продуктивне середовище.

Коли ви будете готові до запуску, ми сертифікуємо ваш обліковий запис і ваш додаток, і ви зможете запуститися в нашому продуктивному середовищі.
:::

Виконайте ці кроки:

<Steps>
1. [Зареєструйтеся](/uk/getting-started/sign-up) як користувач на Wink.
2. [Створіть афілійований обліковий запис](/uk/portal/create-account). Він потрібен для прив’язки вашого додатку. Детальніше дивіться у розділі [що таке Studio](/uk/portal/studio/what-is-studio).
3. [Створіть додаток](/uk/account/applications) і зв’яжіть його з афілійованим обліковим записом, створеним на кроці 2. Для інтеграцій API сервер-сервер оберіть тип клієнта **MACHINE_2_MACHINE**.
4. Після створення `Application` одразу скопіюйте та збережіть client-id і секретний ключ — секретний ключ показується лише один раз і не може бути отриманий повторно.
</Steps>

## CDN

У Wink ми використовуємо Cloudinary для всіх наших зображень і відео. Коли ви отримуєте доступ до нашого туристичного інвентарю, у вас буде повний доступ до всіх розширених функцій Cloudinary.

Наш `cloud_name` — `traveliko`.

### Cloudinary

- [Початок роботи](https://cloudinary.com/documentation/dev_kickstart)
- [SDK](https://cloudinary.com/documentation/cloudinary_sdks)
- [Cloudinary на GitHub](https://github.com/cloudinary)
-	[Відеоуроки](https://cloudinary.com/documentation/cloudinary_sdks_tutorials)

### Приклад

Для кожного типу туристичного інвентарю, який ми підтримуємо, існує вкладений JSON-масив під назвою `multimedias`. JSON для мультимедійного запису виглядає так:

```json {3}
{
  "multimediaIdentifier": "multimedia-1",
  "identifier": "partners/radisson_blu_logo",
  "type": "IMAGE",
  "width": "1024",
  "height": "768",
  "category": "1",
  "descriptions": [
    {
      "name": "Lobby",
      "description": "Picture of the lobby",
      "language": "en"
    }
  ],
  "lifestyleType": "LIFESTYLE_BUSINESS",
  "attribution": [
    {
      "url": "https://www.hilton.com",
      "name": "Hilton"
    }
  ]
}
```

Виділений рядок вище показує унікальний ID Cloudinary `partners/radisson_blu_logo`. Це все, що потрібно, щоб отримати зображення у будь-якому форматі, якості та розмірі.

## Тестування

Перейдіть до нашого розділу [API](/uk/api/overview/) і виберіть API, з яким хочете працювати.

### Документація API

Наш сайт документації налаштований так, що ви можете виконувати API-команди безпосередньо на сайті.

У цьому прикладі ви хочете використати виклик API `Search Lookups (OAuth2)`.

:::tip[Порада]
Для зручності ми рекомендуємо інтеграторам використовувати кінцеві точки, які ми позначаємо як `OAuth2`. Ці кінцеві точки очікують виклики API і вже знають, в якому обліковому записі ви перебуваєте, тому вам не потрібно вказувати `companyIdentifier` у URL.
:::

<Steps>
1. Використайте [Lookup API](https://developer.wink.travel/lookup) для отримання інформації про напрямки або готелі.
2. У лівому навігаційному меню натисніть на `Authentication`.
3. Введіть ваш client-id і client secret і натисніть кнопку `Get TOKEN`.
4. Якщо ваші облікові дані дійсні, ви побачите, що ваш API-ключ застосовано.
5. Тепер ви можете тестувати кінцеву точку lookup.
</Steps>

<figure>
<CloudinaryImage
      width="700"
      height="265"
      resize="scale"
      quality="best"
      alt="Lookup API endpoint"
      src="academy/lookup-api-call"
      errorImage="noimage_opaque_nyrtl0.png"
    />
    <figcaption>*Приклад Search Lookups (OAuth2)*</figcaption>
</figure>

### Postman
Те саме можна зробити за допомогою Postman.

<Steps>
1. Завантажте [Postman](https://www.postman.com/downloads).
2. Натисніть `Download OpenAPI spec` у верхній частині сторінки [Lookup API](https://developer.wink.travel/lookup).
3. Імпортуйте його в Postman.
4. Якщо ви імпортували як нову колекцію з назвою `Wink API`, натисніть на горизонтальне меню (3 крапки) поруч із назвою колекції і виберіть `Edit`.
5. На вкладці `Authorization` встановіть `Auth type` на OAuth2.
6. Встановіть поле `Add auth data to` у `Request headers`.
7. Встановіть поле `Header prefix` у `Bearer`.
8. Встановіть поле `Token Name` на будь-яку назву для вашого збереженого токена в Postman, наприклад, Wink Staging.
9. Встановіть поле `Grant type` у `Client Credentials`.
10. Встановіть поле `Access token URL` на https://staging-iam.wink.travel/oauth2/token.
11. Встановіть поле `Client ID` на Client ID, який допоміг створити ваш додаток.
12. Встановіть поле `Client Secret` на Client Secret, який допоміг створити ваш додаток.
13. Встановіть поле `Scope` на пробіл-розділені області, які потрібні вашій інтеграції (наприклад, `inventory.read inventory.write inventory.remove`). Повний список дивіться у [довідці по областях](/uk/developers/authentication#scopes).
14. Встановіть поле `Client Authentication` у `Send as Basic Auth header`.
15. Натисніть кнопку `Get New Access Token`, щоб отримати токен і зберегти його.
16. Для всіх викликів API в колекції Wink API переконайтеся, що вкладка автентифікації встановлена на `Inherit auth from parent`, щоб токен додавався до кожного виклику.
</Steps>

:::tip[Порада]
Рекомендується зробити домен сервера, client ID і client secret змінними середовища. Так ви зможете легко перемикатися між тестовим і продуктивним середовищами та повторно використовувати всі створені операції.
:::
