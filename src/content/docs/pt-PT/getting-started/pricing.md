...
- **WinkLinks** — reivindique o seu URL personalizado, construa a sua página e publique nela sempre que quiser.
...
O Wink suporta tanto o modelo de comerciante como o de agente.
...
- **A franquia gratuita cobre apenas publicações de imagem padrão.** Recebe uma dessas por conta por mês. Publicações de vídeo e media gerada por IA são faturadas desde a primeira publicação — não há franquia gratuita nestes níveis, por isso uma propriedade que publica vídeo deve esperar uma cobrança no seu primeiro mês.
- **O vídeo prevalece.** Se um post contiver qualquer vídeo, o post inteiro é faturado à taxa de vídeo. Um post que mistura imagem e vídeo é considerado um post de vídeo.
- **A proveniência IA define o nível.** Media que fornece — as suas próprias fotos e vídeos, ou qualquer coisa da sua biblioteca de conteúdos Wink — é faturada à taxa padrão. Media que geramos para si é faturada à taxa IA.
...
- É cobrado **no momento da publicação**, não por tentativa. Regenerar um rascunho até ficar satisfeito não aumenta a sua fatura — paga uma vez pelo post que realmente publica. As tentativas não são ilimitadas, no entanto: cada post permite cerca de 10 regenerações para imagens e 3 para vídeo, o que reflete o custo que temos para os produzir. Verá quantas lhe restam enquanto trabalha.
- Na API de Parceiro, um **hotel-dia** é um hotel com preço para uma noite de estadia — *não* uma chamada API. Uma pesquisa que retorna 20 hotéis para uma estadia de 3 noites são 60 hotel-dias numa única requisição. Os endpoints de pesquisa e autocompletar são gratuitos e nunca medidos.

### Como ativar

O pay-as-you-go está desativado por predefinição. Todos recebem a franquia gratuita sem fazer nada.
...
:::note[Se não ativar]
Nada deixa de funcionar e nada é cobrado. Simplesmente fica-se pela franquia gratuita desse mês: posts gerados não serão publicados e chamadas à API de Parceiro retornam um `429` até a franquia ser reposta.
:::
...
| Suspenso | Uma fatura ficou por pagar até ao fim. Ações faturáveis são bloqueadas até ser regularizada; funcionalidades gratuitas continuam normalmente. |

:::tip[Preços em tempo real]
Os preços unitários e franquias gratuitas são sempre mostrados no Portal, diretamente do nosso sistema de faturação, para que possa consultá-los antes de se comprometer. Veja [Faturação](/pt-PT/portal/plan) para ativar o pay-as-you-go, escolher as suas contas e acompanhar a utilização e faturas do mês. Veja [Social](/pt-PT/portal/social/what-is-social) para saber como o volume de posts afeta o que gasta.
:::
