...
- [Payment Web Component](/pt-PT/integrations/web-components/#payment).
- [Payment API](/pt-PT/api). *Leia a secção sobre `Create payable contracts`*
...
O array `contractList` contém o(s) item(ns) que o viajante quer comprar de vários fornecedores. Cada item contém:
...
- Cada entrada em `contractItemList` contém:
    - **Utilizador hóspede** Este é o utilizador que irá chegar às instalações.
