---
title: Configuração
description: Este artigo mostra como configurar rapidamente como desenvolvedor e começar a testar as nossas APIs na Wink.
sidebar:
  order: 1
---
...
Este artigo mostra como configurar rapidamente como desenvolvedor e começar a testar as nossas APIs na Wink.

:::note
Os testes devem ser sempre realizados no nosso [ambiente de staging](/pt-PT/getting-started/environments). Há muito menos dados lá, mas pode testar à vontade sem impactar o ambiente de produção.
...
<Steps>
1. [Registe-se](/pt-PT/getting-started/sign-up) com a sua conta de utilizador na Wink.
2. [Crie uma conta de afiliado](/pt-PT/portal/create-account). Vai precisar desta para associar à sua Aplicação. Veja [o que é o Studio](/pt-PT/portal/studio/what-is-studio) para mais contexto.
3. [Crie uma Aplicação](/pt-PT/account/applications) e associe-a à conta de afiliado que criou no Passo 2. Para integrações API servidor-a-servidor, escolha **MACHINE_2_MACHINE** como tipo de cliente.
...
A linha destacada acima mostra o ID único do Cloudinary `partners/radisson_blu_logo`. É tudo o que precisa para recuperar a imagem em qualquer formato, qualidade e tamanho que desejar.

## Testes

Dirija-se à nossa secção de [APIs](/pt-PT/api/overview/) e escolha a API com que quer trabalhar.
...
:::tip[Pro Dica]
Para facilitar, recomendamos que os integradores usem os endpoints que designamos como `OAuth2`. Esses endpoints sabem que são chamados por APIs e já sabem em que conta está, por isso não precisa de especificar o `companyIdentifier` nas suas URLs.
:::
...
5. Agora pode testar o endpoint lookup.
</Steps>
...
4. Se importou como uma nova coleção chamada `Wink API`, clique nos três pontos horizontais ao lado do nome da coleção e clique em `Edit`.
...
13. Defina o campo `Scope` para os scopes delimitados por espaço que a sua integração precisa (ex.: `inventory.read inventory.write inventory.remove`). Veja a [referência de scopes](/pt-PT/developers/authentication#scopes) para a lista completa.
