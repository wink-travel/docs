...
<Steps>
1. [Registe-se](/pt-PT/account/register) com a sua conta de utilizador na Wink.
2. [Crie uma conta afiliada](/pt-PT/portal/create-account). Vai precisar desta para associar à sua Aplicação. Veja [o que é o Studio](/pt-PT/portal/studio/what-is-studio) para mais contexto.
3. [Crie uma Aplicação](/pt-PT/portal/settings/applications) e associe-a à conta afiliada que criou no Passo 2. Para integrações API servidor-a-servidor, escolha **MACHINE_2_MACHINE** como tipo de cliente.
...
Na Wink, usamos Cloudinary para todos os nossos ativos de imagem e vídeo. Quando acede ao nosso inventário de viagens, terá acesso total a todas as funcionalidades avançadas do Cloudinary.
...
Dirija-se à nossa secção de [APIs](/pt-PT/api) e escolha a API com que quer trabalhar.
...
:::tip[Pro Dica]
Para facilitar o uso, recomendamos que os integradores usem os endpoints que designamos como `OAuth2`. Esses endpoints sabem que são chamados por APIs e já sabem em que conta está, por isso não precisa de especificar o `companyIdentifier` nas suas URLs.
:::
