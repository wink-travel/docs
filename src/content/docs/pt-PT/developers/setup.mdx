...
:::note
Os testes devem ser sempre realizados no nosso [ambiente de staging](/pt-PT/getting-started/environments). Há muito menos dados lá, mas pode testar o quanto quiser sem afetar o ambiente de produção.
...
<Steps>
1. [Registe-se](/pt-PT/user/register) com a sua conta de utilizador na Wink
2. Crie uma [conta de agente de viagens](/pt-PT/travel-agent/what-is-agent) ou uma [conta de afiliado](/pt-PT/studio/what-is-studio).
3. [Crie uma Aplicação](/pt-PT/developer/applications) onde a associe à conta que criou no Passo 2.
...
Dirija-se à nossa secção de [APIs](/pt-PT/developer/apis) e escolha a API com a qual quer trabalhar.
...
:::tip[Pro Dica]
Para facilitar o uso, recomendamos que os integradores usem os endpoints que designamos como `OAuth2`. Esses endpoints sabem que esperam chamadas API e já sabem a que conta pertence, por isso não precisa de especificar o `companyIdentifier` nas suas URLs.
:::
...
16. Para todas as chamadas API na coleção Wink API, certifique-se de que a aba de autenticação diz `Inherit auth from parent` para incluir o seu token em cada chamada.
</Steps>
