...
En applikation tillhandahåller dig ett `Client ID` och en `Secret Key` som du behöver för att skapa en autentiserad OAuth2-session som du kan använda för att kommunicera med Wink- och TripPay-API:er.

Här är stegen för att skapa en autentiserad session.

**Steg 1**. Hämta en access-token i staging eller i vår produktionsmiljö:
...
som scopes täcker. Med `client_credentials`-grant sätter du scopes i din tokenförfrågan —
...
| **Integrations** | `integrations.read` `integrations.write` `integrations.remove` | Kanalhanterare, externa bokningssystem, Google Hotel, notifieringsändpunkter, bokningssynk | Låg–Medel |
...
| **Accounting** | `accounting.read` `accounting.write` `accounting.remove` | Redovisning, uttagsbok, avstämning | Medel–Hög |
...
**Payment** och **Accounting** hanterar penningrörelser, så de har förhöjd risk.
:::

Några saker att ha i åtanke:

- **Begär minimalt.** Be endast om de scopes som din integration faktiskt använder — en token utfärdad med bredare åtkomst än nödvändigt har större skaderisk vid läckage.
...
- **`mcp.*` är endast för AI-agentklienter.** Du behöver det för att öppna MCP-transporten; det krävs inte för vanliga REST API-anrop. Se [Model Context Protocol](/sv/developers/mcp).

:::tip
Du kan testa alla våra API:er utan att skriva en enda rad kod innan du börjar.

Hämta ett 'Client ID' och 'Secret Key' från någon av dina appar och navigera till någon av våra [API:er](/sv/api/overview/).
:::
