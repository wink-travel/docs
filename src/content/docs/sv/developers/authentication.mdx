...
:::tip
Wink IAM-autentiseringsservern stödjer de vanliga OAuth2-endpointsen:
...
När du gör ett anrop till någon av våra endpoints, inkludera följande i headern:

- `Wink-Version` = `2.0` Senaste - *Se vår API-dokumentation för andra tillgängliga versioner.*
- `Authorization` = `Bearer ${access_token}` *Infoga din access token.*

:::tip
Du kan testa alla våra API:er utan att skriva en enda rad kod innan du börjar.

Hämta en 'Client ID' och 'Secret Key' från någon av dina appar och navigera till någon av våra [APIs](/sv/developers/apis).
:::
