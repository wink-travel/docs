...
<Steps>
1. [Registrera](/sv/my-account/register) ditt användarkonto på Wink.
2. [Skapa ett affiliate-konto](/sv/my-account/create-account). Du behöver detta för att koppla din applikation till. Se [vad är Studio](/sv/studio/what-is-studio) för mer kontext.
3. [Skapa en applikation](/sv/my-account/applications) och koppla den till affiliate-kontot du skapade i steg 2. För server-till-server API-integrationer, välj **MACHINE_2_MACHINE** som klienttyp.
4. När du har skapat din `Application`, kopiera och spara client-id och secret key omedelbart — secret key visas bara en gång och kan inte hämtas igen.
</Steps>
...
Gå till vår [API:er](/sv/developers/apis)-sektion och välj det API du vill arbeta med.
...
16. För alla API-anrop under Wink API-samlingen, se till att autentiseringsfliken säger `Inherit auth from parent` så att din token inkluderas vid varje anrop.
</Steps>
