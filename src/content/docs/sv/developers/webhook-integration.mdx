...
Webhooks låter dina system ta emot realtidsnotiser när händelser sker på ditt Wink-konto — nya bokningar, avbokningar, betalningsuppdateringar med mera. Denna guide leder dig genom installation och bästa praxis.
...
`t + "." + rawBody`, nycklad med din hemlighet, och `rawBody` är exakt begäranens kroppsbitar som mottagits — serialisera inte om JSON innan verifiering. Under 24 timmar efter en hemlighetsrotation bär headern ett andra `v1=` värde signerat med den tidigare hemligheten; acceptera leveransen om **någon** `v1` matchar.
...
```js
// Node.js (Express-stil; se till att du har RAW-kroppen, inte ett parsad objekt)
...
- En `5xx`, timeout, `408` eller `429` försöks igen med backoff: efter 1 minut, 5 minuter, 30 minuter, 2 timmar,
  6 timmar, 12 timmar, sedan dagligen — 10 försök under cirka 3 dagar — varefter leveransen markeras som **död**.
- Alla andra `4xx` behandlas som "du avvisade denna leverans" och försöks **inte** igen.
...
- **Idempotens** — Din hanterare bör vara idempotent; deduplicera på `Wink-Event-Id`. Wink försöker igen om den inte får ett `2xx`-svar.
...
- [Webhooks API reference](/sv/api/webhooks/) — Payload-scheman per händelse, headers och prenumerations-/leveranshanteringsendpoints.
...
- [Developers > APIs](/sv/api/overview/) — Fullständig API-dokumentation.
