...
2. **Ange det i Wink-portalen**
   ...
   Logga in på [app.wink.travel](https://app.wink.travel), välj ditt konto och gå till fliken **Anpassningar**. Klistra in ditt Sojern GTM Container ID i Sojern-fältet och spara. Din integration är nu aktiv.
...
- **På varje sida där din anläggning är aktiv** på Wink renderas ditt Sojern GTM Container ID in i sidan och Winks tagghanteringssystem laddar dynamiskt din Sojern-container parallellt med sin egen. Detta sker automatiskt — inga manuella steg krävs från Wink-teamet per anläggning.
- **Din Sojern-container laddas asynkront**, vilket innebär att det inte påverkar sidans laddningsprestanda för dina gäster.
...
Wink skickar GA4-kompatibla händelser till `dataLayer` under hela gästens resa. Din GTM-container läser från samma delade `dataLayer`, så alla triggers eller variabler du konfigurerar har tillgång till alla egenskaper som beskrivs nedan.
...
|------|----------|-------------------|
| 1 | `view_item_list` | När hotellinventariets rutnät eller lista laddas |
| 2 | `view_item` | När en gäst tittar på en specifik anläggningssida |
| 3 | `add_to_cart` | När en gäst lägger till ett rum i kundvagnen |
| 4 | `begin_checkout` | När kassan/betalningssidan laddas |
| 5 | `add_payment_info` | När en gäst väljer betalningsmetod |
| 6 | `login` | När en gäst autentiserar via SSO |
| 7 | `purchase` | När bokningen bekräftas |
| 8 | `refund` | När bokningen avbokas |
...
Aktiveras när kassan/betalningssidan laddas.
...
|----------|-----|-------------|
| `property_id` | string | Wink-anläggnings-ID |
...
1. **Öppna Anpassade definitioner**
   ...
   I GA4, gå till **Admin** (kugghjulsikonen längst ner till vänster) → välj din egendom → **Data display** → **Custom definitions**.
