...
- [Payment Web Component](/sv/integrations/web-components/#payment).
- [Payment API](/sv/api). *Läs avsnittet om `Create payable contracts`*
...
2. Låt resenären välja den inventarie de vill boka och förbered dem för att betala.
...
- Använd en `traceId` för att gruppera flera bokningar över flera leverantörer. På så sätt kan du avboka en gruppbokning.
...
Arrayen `contractList` innehåller de objekt som resenären vill köpa från flera leverantörer. Varje objekt innehåller:

- `identifier` är en giltig UUID som du genererar.
- Leverantören du vill boka inventarie från.
- Inventariet du vill boka från den leverantören. Dessa specificeras i den nästlade arrayen `contractItemList`.
- Varje post under `contractItemList` innehåller:
    - **Gäst användare** Detta är användaren som kommer att anlända till platsen.
    - **Namn på engelska** Namnet på inventariet på engelska.
    - **Beskrivning på engelska** En längre beskrivning av inventariet på engelska.
    - **Pris** Priset på objektet.
    - **Resplan** När detta objekt ska bokas.
    - **Pristyp** Hur priset beräknades.
    - **Inventarietyp** Vilken typ av inventarie detta är.
    - **Payable** När resenären ska debiteras. *Vi stödjer för närvarande endast omedelbara betalningsalternativ.*
...
När TripPay svarar på din begäran kommer det att se ut ungefär så här:
