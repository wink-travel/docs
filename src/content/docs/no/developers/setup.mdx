...
:::note
Testing bør alltid foregå i vårt [staging-miljø](/no/getting-started/environments). Det er mye mindre data der, men du står fritt til å teste så mye du vil uten å påvirke produksjonsmiljøet.
...
<Steps>
1. [Registrer](/no/account/register) brukerkontoen din på Wink.
2. [Opprett en affiliate-konto](/no/portal/create-account). Du trenger denne for å knytte applikasjonen din til. Se [hva er Studio](/no/portal/studio/what-is-studio) for mer kontekst.
3. [Opprett en applikasjon](/no/portal/settings/applications) og knytt den til affiliate-kontoen du opprettet i steg 2. For server-til-server API-integrasjoner, velg **MACHINE_2_MACHINE** som klienttype.
4. Når du har opprettet din `Application`, kopier og lagre client-id og secret key umiddelbart — secret key vises kun én gang og kan ikke hentes igjen.
</Steps>
...
Gå til vår [API-er](/no/api) seksjon og velg API-et du ønsker å jobbe med.

### API-dokumentasjon

Dokumentasjonsnettstedet vårt er satt opp slik at du kan kjøre API-kommandoer direkte fra siden.

I dette eksempelet ønsker du å bruke `Search Lookups (OAuth2)` API-kallet.

:::tip[Proff tips]
For enkel bruk anbefaler vi at integratorer bruker endepunktene vi betegner som `OAuth2`. Disse endepunktene forventer API-kallere og vet allerede hvilken konto du er på, så du slipper å spesifisere `companyIdentifier` i URL-ene dine.
:::
...
4. Hvis du importerte den som en ny samling kalt `Wink API`, klikk på de horisontale ellipsene (3 prikker) ved siden av samlingsnavnet og velg `Edit`.
...
11. Sett feltet `Client ID` til Client ID-en applikasjonen din genererte.
12. Sett feltet `Client Secret` til Client Secret-en applikasjonen din genererte.
13. Sett feltet `Scope` til de mellomromsseparerte scope-ene integrasjonen din trenger (f.eks. `inventory.read inventory.write inventory.remove`). Se [scope-referansen](/no/developers/authentication#scopes) for fullstendig liste.
...
:::tip[Proff tips]
Det anbefales å gjøre serverdomenet, client ID og client secret til miljøvariabler. På den måten kan du enkelt bytte mellom staging- og produksjonsmiljøer og gjenbruke alle operasjonene du har laget.
:::
