...
<Steps>
1. [Registrer](/no/user/register) brukerkontoen din på Wink
2. Opprett en [reisebyråkonto](/no/travel-agent/what-is-agent) eller en [affiliate-konto](/no/studio/what-is-studio).
3. [Opprett en applikasjon](/no/developer/applications) hvor du knytter den til kontoen du opprettet i steg 2.
...
Gå til vår [API-er](/no/developer/apis) seksjon og velg API-et du vil jobbe med.
...
:::tip[Proff tips]
For enkel bruk anbefaler vi at integratører bruker endepunktene vi betegner som `OAuth2`. Disse endepunktene vet at det er API-kallere og vet allerede hvilken konto du er på, så du slipper å spesifisere `companyIdentifier` i URL-ene dine.
:::
...
4. Hvis du importerte det som en ny samling kalt `Wink API`, klikk på de horisontale ellipsene (3 prikker) ved siden av samlingsnavnet og klikk `Edit`.
