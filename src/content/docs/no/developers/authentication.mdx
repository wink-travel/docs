...
Du vil mest sannsynlig bruke et robust OAuth2-bibliotek for ditt programmeringsspråk, som gjør det meste av arbeidet for deg.
...
- `read` — se ressurser (trygge `GET`-forespørsler)
...
:::note
**Risiko**-kolonnen reflekterer sensitivitet: `read` er lavest og `write`/`remove` høyest.
**Payment** og **Accounting** beskytter pengeflyt, og har derfor forhøyet risiko.
:::

Noen ting å huske på:

- **Be om minimum.** Be kun om de scopes integrasjonen din faktisk bruker — en token utstedt med bredere tilgang enn nødvendig har større skadepotensial ved lekkasje.
- **Ukjente scopes avvises.** Å be om en scope applikasjonen din ikke er registrert for, eller som ikke finnes, feiler ved token-utstedelse.
- **`mcp.*` er kun for AI-agent-klienter.** Du trenger det for å åpne MCP-transporten; det er ikke nødvendig for vanlige REST API-kall. Se [Model Context Protocol](/no/developers/mcp).

:::tip
Du kan teste alle våre API-er uten å skrive en eneste linje kode før du kommer i gang.

Hent en 'Client ID' og 'Secret Key' fra en av dine apper og naviger til noen av våre [API-er](/no/api/overview/).
:::
