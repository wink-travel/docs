...
Denne guiden er for utviklere som integrerer Wink med eksterne systemer som eiendomsforvaltningssystemer (PMS), kanaladministratorer, CRM-er eller tilpassede dashbord.
...
Hver levering er en HTTP `POST` til webhook-URL-en din med `Content-Type: application/json` og denne konvolutten:
...
- `type` — hendelsestypenøkkel (også sendt som `Wink-Event-Type` header). Bruk `type` og `schemaVersion` for å tolke `object`.
- `object` — et kuratert sammendrag av ressursen hendelsen gjelder (identifikatorer, status, feltene du handler på) pluss `links.self`, leverandørsidens kanoniske REST-URL til hele ressursen. Hent den med dine egne API-legitimasjoner når du trenger mer enn sammendraget; hvis du mottar hendelsen som forhandler eller reisebyrå, bruk tilsvarende ressursendepunkt i ditt eget API for samme identifikator.
...
hvor `t` er et Unix-tidsstempel (sekunder) og `v1` er den småbokstav-hexadezimale **HMAC-SHA256** av strengen `t + "." + rawBody`, nøkklet med din hemmelighet, og `rawBody` er de eksakte forespørselsbody-bytene som mottas — ikke serialiser JSON på nytt før verifisering. I 24 timer etter en hemmelighetsrotasjon bærer headeren en andre `v1=`-verdi signert med forrige hemmelighet; aksepter leveringen hvis **noen** `v1` matcher.

Verifiser i fire steg: parse `t` og hver `v1`; regn ut HMAC over `t.rawBody` med din hemmelighet; sammenlign med en konstant-tids sammenligning; avvis hvis `|nå − t|` overskrider toleransen din (5 minutter anbefalt).

```js
// Node.js (Express-stil; sørg for at du har RAW body, ikke et tolket objekt)
...
Roter hemmeligheten fra portalen eller med `POST /api/managing-entity/{id}/webhook/{webhookId}/rotate-secret`; responsen viser den nye hemmeligheten én gang, og den gamle verifiseres fortsatt i 24 timer mens du ruller den ut.

## Retries og ny levering

- Svar med hvilken som helst `2xx` innen 10 sekunder for å bekrefte mottak. Gjør tung behandling asynkront.
- En `5xx`, timeout, `408` eller `429` prøves på nytt med backoff: etter 1 minutt, 5 minutter, 30 minutter, 2 timer, 6 timer, 12 timer, deretter daglig — 10 forsøk over omtrent 3 dager — deretter markeres leveringen som **død**.
- Enhver annen `4xx` behandles som "du avviste denne leveringen" og prøves **ikke** på nytt.
- Hver hendelse, levering og forsøk (status, responsutdrag) er synlig under **Applications > Webhooks** og via API (`…/webhook/event/grid`, `…/webhook/delivery/grid`). Du kan **ny-levere** enhver levering (`POST …/webhook/delivery/{deliveryId}/redeliver`, som starter en ny retry-serie), ny-levere alle døde leveringer for en webhook samtidig (`POST …/webhook/{webhookId}/redeliver-dead`), eller avbryte en.
...
- **Valider kilden** — Verifiser `Wink-Signature` headeren (se [Verifisere signaturer](#verifisere-signaturer)) før behandling; avvis alt som feiler.
...
- [Webhooks API reference](/no/api/webhooks/) — Nyttelastskjemaer per hendelse, headere og endepunkter for abonnement/levering.
- [Webhooks](/no/portal/settings/webhooks) — Full referanse for webhook-administrasjon.
- [Applications](/no/account/applications) — Administrer API-legitimasjonene dine.
- [Developers > APIs](/no/api/overview/) — Full API-dokumentasjon.
