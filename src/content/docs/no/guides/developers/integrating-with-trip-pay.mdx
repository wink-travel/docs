...
- [Payment Web Component](/no/integrations/web-components/#payment).
- [Payment API](/no/api). *Les seksjonen om `Create payable contracts`*
      ...
      "contractItemList": [
        {
          ...
          "descriptionInEnglish": "Dette er den beste deluxe king som penger kan kjøpe.",
          "price": {
            "amount": 100,
            "currency": "USD"
          },
...
- `sourceUrl` er nettstedet / appen bookingen skjedde på.
...
- Hver oppføring under `contractItemList` inneholder:
    ...
    - **Payable** Når den reisende skal belastes. *Vi støtter for øyeblikket kun umiddelbare betalingsalternativer.*
    - **Policy** Inkluder avbestillingsregler.
    - **Daglig pris-liste** Hvis elementet som bookes er et rom, kan du velge å inkludere hvor mye rommet koster for hver natt gjesten bor.
...
Lagre `id` på linje 3. Du vil bruke denne til å injisere i TripPay webkomponenten.
...
På dette punktet er du klar til å inkludere `Payment Web Component` på nettstedet ditt og vise det til brukeren.

```html
<trip-pay id="contract-1"></trip-pay>
```

Når det gjelder integrasjon, er det alt du trenger å gjøre. Alt det tunge arbeidet håndteres av TripPay herfra.
