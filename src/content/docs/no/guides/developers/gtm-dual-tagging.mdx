...
description: Hvordan koble Sojern-kontoen din til Wink slik at Sojern kan spore gjestenes bookingreise og drive retargeting- og reklamekampanjene dine.
sidebar:
...
Hvis du kjører reklamekampanjer gjennom Sojern, kan du koble Sojern-kontoen din til Wink ved å legge inn din Sojern GTM Container ID i Wink-portalen. Når den er koblet til, aktiveres Sojerns sporingspixel automatisk for din eiendom — og dekker hele gjestereisen fra søk til fullført booking.
...
2. **Legg den inn i Wink-portalen**
   ...
   Logg inn på [app.wink.travel](https://app.wink.travel), velg kontoen din, og gå til fanen **Customizations**. Lim inn din Sojern GTM Container ID i Sojern-feltet og lagre. Integrasjonen er nå aktiv.
...
Denne konfigurasjonen fungerer for hvilken som helst tilpasset GTM Container ID. Vi brukte bare Sojern som et eksempel siden det er en svært vanlig integrasjon.
</Aside>
...
For å være tydelige, her er hva Wink konfigurerer for at denne integrasjonen skal fungere:
...
- **Din Sojern-container lastes asynkront**, noe som betyr at det ikke påvirker sideinnlastingshastigheten for gjestene dine.
- **Winks egen sporing og din Sojern-pixel kjører parallelt**, og leser fra samme delte hendelsesdata. Hver bookinghendelse som Wink sporer, er samtidig tilgjengelig for din Sojern-container.
...
Disse hendelsene aktiveres automatisk når en gjest beveger seg gjennom bookingtrakten. Konfigurer dine GTM-triggere som **Custom Event**-triggere som matcher hendelsesnavnene nedenfor.
...
| 4 | `begin_checkout` | Kassen / betalingsside lastes |
...
Aktiveres når kassen / betalingssiden lastes.
...
Aktiveres når en booking bekreftes.
...
Aktiveres når en booking kanselleres.
...
#### Tilpassede elementspesifikke parametere

Disse parameterne bærer Wink-spesifikk bookingkontekst. GA4 viser dem ikke automatisk i rapporter — du må registrere hver enkelt som en **elementspesifikk egendefinert dimensjon** i din GA4-eiendom før de vises i utforskninger eller standardrapporter. Se [Custom Dimensions in GA4](#custom-dimensions-in-ga4) nedenfor for oppsett.

| Egenskap | Type | Beskrivelse |
|----------|------|-------------|
| `property_id` | string | Wink-eiendom-identifikator |
...
| `room_rate_name` | string | Visningsnavn for romprisen |
...
3. **Fyll inn dimensjonsdetaljer**
   ...
   - **Dimension name** — det menneske-lesbare navnet som vises i GA4-rapporter (se tabell nedenfor)
   - **Scope** — velg **Item**
...
GA4 tillater opptil 50 elementspesifikke egendefinerte dimensjoner per eiendom på standard eiendommer (360-eiendommer tillater flere).
</Aside>
...
Følgende hendelser er tilgjengelige i Wink dataLayer, men er ikke en del av standard bookingtrakt. De kan aktiveres av andre interaksjoner på plattformen.
