...
:::note
Testovanie by malo vždy prebiehať v našom [staging prostredí](/sk/getting-started/environments). Je tam oveľa menej dát, ale môžete testovať koľko chcete bez toho, aby ste zasahovali do produkčného prostredia.
...
<Steps>
1. [Zaregistrujte sa](/sk/account/register) ako používateľ na Wink.
2. [Vytvorte si affiliate účet](/sk/portal/create-account). Tento účet budete potrebovať na prepojenie vašej aplikácie. Viac informácií nájdete v [čo je Studio](/sk/portal/studio/what-is-studio).
3. [Vytvorte aplikáciu](/sk/portal/settings/applications) a priraďte ju k affiliate účtu, ktorý ste vytvorili v kroku 2. Pre server-to-server API integrácie vyberte ako typ klienta **MACHINE_2_MACHINE**.
...
Vo Wink používame Cloudinary pre všetky naše obrazové a video súbory. Keď pristupujete k našej cestovnej ponuke, máte plný prístup ku všetkým pokročilým funkciám Cloudinary.
...
Pre každý typ cestovnej ponuky, ktorý podporujeme, bude vnorené JSON pole nazvané `multimedias`. JSON pre multimediálnu položku vyzerá takto:
...
Prejdite do sekcie našich [API](/sk/api) a vyberte API, s ktorým chcete pracovať.
...
13. Nastavte pole `Scope` na medzerou oddelené rozsahy, ktoré vaša integrácia potrebuje (napr. `inventory.read inventory.write inventory.remove`). Kompletný zoznam nájdete v [scope reference](/sk/developers/authentication#scopes).
...
16. Pre všetky API volania v kolekcii Wink API sa uistite, že záložka autorizácie hovorí `Inherit auth from parent`, aby sa token pridal ku každému volaniu.
</Steps>
