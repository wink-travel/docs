...
Najpravdepodobnejšie budete pracovať s robustnou OAuth2 knižnicou pre váš jazyk, ktorá za vás vykoná všetku ťažkú prácu.
...
ktoré tieto rozsahy pokrývajú. Pri `client_credentials` grante nastavujete rozsahy vo svojej požiadavke na token —
pozrite si príklady [`curl`](#staging) vyššie a [Postman návod](/sk/developers/setup#postman).

Rozsahy sú pomenované ako `section.action`, kde akcia je jedna z:
...
| **Účtovníctvo** | `accounting.read` `accounting.write` `accounting.remove` | Účtovníctvo, výpisy výberov, zosúladenie | Stredné–Vysoké |
...
:::tip
Všetky naše API môžete otestovať bez napísania jediného riadku kódu pred začatím.

Získajte 'Client ID' a 'Secret Key' z jednej zo svojich aplikácií a prejdite na niektoré z našich [API](/sk/api/overview/).
:::
