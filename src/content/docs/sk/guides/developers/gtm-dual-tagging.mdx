...
Ak prevádzkujete reklamné kampane cez Sojern, môžete prepojiť svoj účet Sojern s Wink zadaním svojho Sojern GTM Container ID do portálu Wink. Po prepojení sa Sojern tracking pixel automaticky spustí pre vašu nehnuteľnosť — pokrývajúc celú cestu hosťa od vyhľadávania až po dokončenú rezerváciu.
...
2. **Zadajte ho do portálu Wink**
   ...
   Prihláste sa na [app.wink.travel](https://app.wink.travel), vyberte svoj účet a prejdite na kartu **Customizations**. Vložte svoje Sojern GTM Container ID do poľa Sojern a uložte. Vaša integrácia je teraz aktívna.
...
Wink posiela udalosti kompatibilné s GA4 do `dataLayer` počas celej cesty hosťa. Váš GTM kontajner číta z toho istého zdieľaného `dataLayer`, takže každý trigger alebo premenná, ktorú nastavíte, má prístup ku všetkým nižšie popísaným vlastnostiam.
...
| `coupon` | string | — | Použitý kupón na položku |
...
GA4 umožňuje až 50 vlastných dimenzií na úrovni položky pre štandardné vlastnosti (360 vlastnosti umožňujú viac).
</Aside>
...
Toto je vlastná udalosť Wink — nie je súčasťou štandardnej sady GA4 udalostí. Posiela sa pri každej navigácii na stránke a nesie kontext zariadenia a relácie. Neobsahuje objekt `ecommerce`.
