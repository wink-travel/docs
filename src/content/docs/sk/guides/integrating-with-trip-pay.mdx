...
Tento článok obsahuje príklad integrácie s TripPay od začiatku do konca.

Odporúča sa, aby ste už prečítali:

- [Platba > Mapovanie](/sk/payment/mapping).
- [Platba > Integrácia](/sk/payment/integration).
- [Platobný webový komponent](/sk/integrations/web-components/#payment).
- [Platobné API](/sk/developers/apis/#payment-api). *Prečítajte si časť o `Vytváraní platobných zmlúv`*
...
3. Vytvorte s TripPay `platobnú zmluvu`, ktorá obsahuje položky na rezerváciu.
...
## Vytvorenie platobnej zmluvy

:::note
Pre viac informácií o požiadavkách na vytvorenie platobnej zmluvy navštívte Platobné API.
:::

### Požiadavka
Tu je príklad JSON požiadavky, ktorá obsahuje všetko potrebné na vytvorenie vašej prvej platobnej zmluvy.
...
- Inventár, ktorý chcete rezervovať od daného dodávateľa. Všetky sú špecifikované v zanořeném poli `contractItemList`.
- Každý záznam v `contractItemList` obsahuje:
    ...
    - **Platiteľné** Kedy má byť cestujúcemu účtovaná platba. *Momentálne podporujeme len možnosti okamžitej platby.*
    - **Politika** Zahrňte pravidlá storno podmienok.
    - **Zoznam denných cien** Ak je rezervovaná položka izba, môžete uviesť cenu za každú noc pobytu hosťa.
...
Keď TripPay odpovie platobnou zmluvou, všimnete si, že si pridal vlastného príjemcu s 4,0 %.
:::
