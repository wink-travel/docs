...
<Steps>
1. [Magrehistro](/tl/account/register) ng iyong user account sa Wink.
2. [Gumawa ng affiliate account](/tl/portal/create-account). Kakailanganin mo ito upang i-bind ang iyong Application. Tingnan ang [ano ang Studio](/tl/portal/studio/what-is-studio) para sa karagdagang konteksto.
3. [Gumawa ng Application](/tl/portal/settings/applications) at iugnay ito sa affiliate account na ginawa mo sa Hakbang 2. Para sa server-to-server API integrations, piliin ang **MACHINE_2_MACHINE** bilang client type.
...
Ipinapakita ng naka-highlight na linya sa itaas ang Cloudinary unique ID na `partners/radisson_blu_logo`. Iyon lang ang kailangan mo upang makuha ang imahe sa anumang format, kalidad, at sukat na gusto mo.

## Pagsubok

Pumunta sa aming [APIs](/tl/api) na seksyon at piliin ang API na gusto mong gamitin.
...
:::tip[Pro Tip]
Para sa kadalian ng paggamit, inirerekomenda naming gamitin ng mga integrator ang mga endpoint na tinukoy namin bilang `OAuth2`. Alam ng mga endpoint na ito na inaasahan ang mga API caller at alam na kung anong account ka kaya hindi mo na kailangang tukuyin ang `companyIdentifier` sa iyong mga URL.
:::

<Steps>
1. Gamitin ang [Lookup API](https://developer.wink.travel/lookup) upang kunin ang mga destinasyon o hotel.
...
13. Itakda ang field na `Scope` sa mga space-delimited scopes na kailangan ng iyong integration (hal. `inventory.read inventory.write inventory.remove`). Tingnan ang [scope reference](/tl/developers/authentication#scopes) para sa buong listahan.
...
:::tip[Pro Tip]
Inirerekomenda na gawing environment variables ang server domain, client ID, at client secret. Sa ganitong paraan, madali kang makakapagpalit sa pagitan ng staging at production environments at magagamit muli ang lahat ng mga operasyon na iyong ginawa.
:::
