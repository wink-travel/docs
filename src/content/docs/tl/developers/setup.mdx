...
:::note
Dapat palaging isagawa ang pagsubok sa aming [staging environment](/tl/getting-started/environments). Mas kaunti ang datos doon ngunit malaya kang mag-test nang marami hangga't gusto mo nang hindi naaapektuhan ang production environment.
...
2. [Gumawa ng affiliate account](/tl/portal/create-account). Kakailanganin mo ito para i-bind ang iyong Application. Tingnan ang [ano ang Studio](/tl/portal/studio/what-is-studio) para sa karagdagang konteksto.
...
Pumunta sa aming [APIs](/tl/api/overview/) na seksyon at piliin ang API na gusto mong gamitin.
...
:::tip[Pro Tip]
Para sa mas madaling paggamit, inirerekomenda naming gamitin ng mga integrator ang mga endpoint na tinukoy namin bilang `OAuth2`. Alam ng mga endpoint na ito na may mga API caller at alam na kung anong account ka kaya hindi mo na kailangang tukuyin ang `companyIdentifier` sa iyong mga URL.
:::
...
8. Itakda ang field na `Token Name` sa anumang gusto mong pangalanan ang iyong saved token sa Postman. Halimbawa, Wink Staging.
...
:::tip[Pro Tip]
Inirerekomenda na gawing environment variables ang server domain, client ID, at client secret. Sa ganitong paraan, madali kang makakapagpalit sa pagitan ng staging at production environments at magagamit muli ang lahat ng operasyon na iyong ginawa.
:::
