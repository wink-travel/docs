...
Ang gabay na ito ay para sa mga developer na nag-iintegrate ng Wink sa mga panlabas na sistema tulad ng mga property management system (PMS), channel manager, CRM, o mga custom na dashboard.
...
Naglalathala ang Wink ng 70 uri ng webhook event ngayon sa mga booking, property, account (mga managing entity) at imbentaryo (mga uri ng kuwarto, mga rate plan, master rate, mga add-on, pasilidad, mga sales channel, mga promosyon). Karaniwan:
...
Ang kumpleto, na-generate na listahan — na may paglalarawan, sino ang tumatanggap nito, at isang link sa reference page ng bawat kaganapan — ay ang **Webhook Events Catalog**. Ang reference page para sa bawat kaganapan (JSON body, headers, retry policy) ay matatagpuan sa [Webhooks API](/tl/api/webhooks/).
...
Ang bawat payload schema ay dokumentado bawat kaganapan sa [Webhooks API](/tl/api/webhooks/) reference.
...
| `Wink-Delivery-Id` | Natatangi bawat endpoint bawat kaganapan; nagbabago lamang kapag nire-deliver muli. |
| `Wink-Event-Type` | Pareho ng `type` sa body. |
| `Wink-Delivery-Attempt` | Numero ng pagtatangka para sa delivery na ito, nagsisimula sa 1. |
...
kung saan ang `t` ay isang Unix timestamp (segundo) at ang `v1` ay ang lower-case hex na **HMAC-SHA256** ng string na `t + "." + rawBody`, na naka-key gamit ang iyong secret, at ang `rawBody` ay ang eksaktong request body bytes na natanggap — huwag i-re-serialize ang JSON bago mag-verify. Sa loob ng 24 na oras pagkatapos ng rotation ng secret, ang header ay may pangalawang `v1=` na value na naka-sign gamit ang dating secret; tanggapin ang delivery kung **anumang** `v1` ay tumutugma.

I-verify sa apat na hakbang: i-parse ang `t` at bawat `v1`; i-recompute ang HMAC sa `t.rawBody` gamit ang iyong secret; ikumpara gamit ang constant-time comparison; tanggihan kung ang `|ngayon − t|` ay lumampas sa iyong tolerance (inirerekomenda ang 5 minuto).
...
I-rotate ang secret mula sa portal o gamit ang `POST /api/managing-entity/{id}/webhook/{webhookId}/rotate-secret`; ipapakita ang bagong secret isang beses, at ang luma ay patuloy na magve-verify sa loob ng 24 na oras habang inilalapat mo ito.

## Mga retry at muling paghahatid
...
- Bawat kaganapan, delivery at pagtatangka (status, snippet ng tugon) ay makikita sa ilalim ng **Applications > Webhooks** at sa pamamagitan ng API (`…/webhook/event/grid`, `…/webhook/delivery/grid`). Maaari kang **muling mag-deliver** ng anumang delivery (`POST …/webhook/delivery/{deliveryId}/redeliver`, na nagsisimula ng bagong retry series), muling mag-deliver ng lahat ng dead delivery ng isang webhook nang sabay-sabay (`POST …/webhook/{webhookId}/redeliver-dead`), o kanselahin ang isa.
- Ang mga delivery ay iniimbak ng 30 araw.

## Mga test event

Magpadala sa iyong sarili ng synthetic na `webhook.test` event mula sa portal o gamit ang `POST /api/managing-entity/{id}/webhook/{webhookId}/test`. Ito ay naka-sign at naihahatid nang eksakto tulad ng totoong kaganapan, kaya maaari mong i-verify ang iyong endpoint, ang iyong signature check at ang iyong idempotency handling bago mag-subscribe sa mga live na kaganapan.

## Mga pinakamahusay na kasanayan

- **Gumamit ng HTTPS** — Nagpapadala ang Wink ng mga payload sa HTTPS endpoints lamang.
- **Mabilis na tumugon** — Ibalik ang `200 OK` agad pagkatanggap ng payload. Gawin ang mabibigat na pagproseso nang asynchronous.
...
- **I-validate ang pinagmulan** — I-verify ang `Wink-Signature` header (tingnan ang [Pag-verify ng mga signature](#pag-verify-ng-mga-signature)) bago magproseso; tanggihan ang anumang pumalya.
...
Maaari mong **i-disable** ang isang webhook nang hindi ito tinatanggal. Pinapahinto nito ang paghahatid upang makapag-troubleshoot ka nang hindi nawawala ang iyong configuration. Kapag handa ka na, i-toggle mo lang pabalik sa on.
...
- [Webhooks API reference](/tl/api/webhooks/) — Mga schema ng payload bawat kaganapan, mga header, at mga endpoint para sa pamamahala ng subscription/delivery.
...
- [Developers > APIs](/tl/api/overview/) — Kumpletong dokumentasyon ng API.
