...
- [Payment Web Component](/tl/integrations/web-components/#payment).
- [Payment API](/tl/api). *Basahin ang seksyon tungkol sa `Create payable contracts`*

Narito ang mga hakbang upang matagumpay na magamit ang TripPay para magbayad ng booking:

<Steps>
1. Siguraduhing na-map na ang mga benepisyaryo na binanggit mo sa booking contract.
2. Hayaan ang manlalakbay na pumili ng imbentaryo na nais nilang i-book at ihanda silang magbayad.
3. Gumawa ng `payable contract` sa TripPay na naglalaman ng mga item na i-book.
4. Hayaan ang manlalakbay na magbayad gamit ang TripPay Web Component.
</Steps>
...
:::note
Pumunta sa Payment API upang matuto pa tungkol sa mga kinakailangan para gumawa ng payable contract.
:::

### Request
Narito ang isang halimbawa ng JSON request na naglalaman ng lahat ng kailangan mo para gumawa ng iyong unang payable contract.
...
Hindi namin ipapaliwanag ang bawat data point dito dahil nasaklaw na ito sa mga dokumento ng API.
...
Ang array na `contractList` ay naglalaman ng mga item na nais bilhin ng manlalakbay mula sa maraming supplier. Bawat item ay naglalaman ng:

- Ang `identifier` ay isang valid na UUID na iyong gagawin.
- Ang supplier na nais mong i-book ng imbentaryo.
- Ang imbentaryo na nais mong i-book mula sa supplier na iyon. Lahat ito ay tinukoy sa nested array na `contractItemList`.
- Bawat entry sa ilalim ng `contractItemList` ay naglalaman ng:
    - **Guest user** Ito ang user na darating sa lugar.
    - **Name in English** Ang pangalan ng imbentaryo sa wikang Ingles.
    - **Description in English** Isang mas mahabang paglalarawan ng imbentaryo sa wikang Ingles.
    - **Price** Ang presyo ng item.
    - **Itinerary** Kailan dapat i-book ang item na ito.
    - **Pricing type** Paano kinuwenta ang presyo.
    - **Inventory type** Anong uri ng imbentaryo ito.
    - **Payable** Kailan dapat singilin ang manlalakbay. *Sa kasalukuyan, sinusuportahan lang namin ang mga opsyon para sa agarang pagbabayad.*
    - **Policy** Isama ang mga patakaran sa pagkansela.
    - **Daily rate list** Kung ang item na bine-book ay isang kuwarto, maaari mong isama kung magkano ang halaga ng kuwarto para sa bawat gabi ng pananatili ng bisita.
...
:::note
Sa halimbawa ng JSON sa itaas, mayroon tayong 3 benepisyaryo:
...
Kapag sumagot ang TripPay ng payable contract, mapapansin mong nag-inject ito ng sarili nitong benepisyaryo na 4.0%.
:::
...
:::note
Siguraduhing na-load mo na ang aming mga estilo at Javascript bago ito.
:::
...
Hanggang dito na lang ang kailangan mong gawin para sa integration. Ang lahat ng mabigat na trabaho ay gagawin na ng TripPay mula dito.
