...
おそらく、言語に対応した強力なOAuth2ライブラリを使用しているでしょう。これがほとんどの処理を自動で行います。
...
**スコープ**はアクセストークンに付与される権限です。トークンをリクエストする際、`scope`パラメータにスペース区切りのスコープリストを渡し、トークンはそのスコープがカバーする範囲のみアクセス可能です。`client_credentials`グラントでは、トークンリクエスト時にスコープを設定します — 上記の[`curl`](#staging)例や[Postmanウォークスルー](/ja/developers/setup#postman)を参照してください。
...
Winkは機能別に権限をグループ化しています。統合に必要なスコープのみリクエストしてください：
...
| **支払い** | `payment.read` `payment.write` `payment.remove` | 支払い処理、Stripe、Revolut、支払い、サブスクリプション | 中〜高 |
...
:::note
**リスク**列は感度を示しています：`read`は低リスク、`write`/`remove`は高リスクです。**支払い**と**会計**は資金移動を扱うため、リスクが高くなります。
:::

いくつか注意点：

- **最小限のリクエストを。** 統合で実際に使用するスコープのみをリクエストしてください。必要以上に広いアクセス権を持つトークンは、漏洩時の影響範囲が大きくなります。
- **不明なスコープは拒否されます。** 登録されていない、または存在しないスコープをリクエストすると、トークン発行時に失敗します。
- **`mcp.*`はAIエージェントクライアント専用です。** MCPトランスポートを開くために必要で、通常のREST API呼び出しには不要です。[Model Context Protocol](/ja/developers/mcp)を参照してください。

:::tip
コードを書かずにすべてのAPIをテストできます。

アプリから「Client ID」と「Secret Key」を取得し、任意の[API](/ja/api/overview/)にアクセスしてください。
:::
