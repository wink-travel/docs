...
<Steps>
1. Winkでユーザーアカウントを[登録](/ja/my-account/register)します。
2. [アフィリエイトアカウントを作成](/ja/my-account/create-account)します。アプリケーションを紐付けるために必要です。詳細は[Studioとは何か](/ja/studio/what-is-studio)をご覧ください。
3. [アプリケーションを作成](/ja/my-account/applications)し、ステップ2で作成したアフィリエイトアカウントに関連付けます。サーバー間API連携の場合は、クライアントタイプに**MACHINE_2_MACHINE**を選択してください。
...
[APIs](/ja/developers/apis)セクションにアクセスし、使用したいAPIを選択してください。
...
:::tip[プロのヒント]
使いやすさのために、統合者には`OAuth2`として指定されたエンドポイントの使用を推奨します。これらのエンドポイントはAPI呼び出し元を認識し、URLに`companyIdentifier`を指定する必要がありません。
:::
...
16. Wink APIコレクションのすべてのAPIコールで、認証タブが`Inherit auth from parent`になっていることを確認し、すべての呼び出しにトークンが含まれるようにします。
</Steps>
