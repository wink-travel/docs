---
title: MCPサーバー
description: ホストされたMCPサーバーを介して、AIエージェントをWinkのドキュメントやライブOpenAPI仕様に素早く接続します。
sidebar:
  order: 10
---
...
対応するAIクライアント（VS Code、Cursor、その他MCP HTTPトランスポートを実装しているもの）をこのエンドポイントに向けることで、以下が可能になります：
...
### Claude Code
公式ドキュメント：[Connect Claude Code to tools via MCP](https://code.claude.com/docs/en/mcp)
...
Claude Code内で`/mcp`を実行してサーバーステータスを確認できます。リポジトリ全体でサーバーを共有したい場合は、`--scope project`を追加してClaude Codeが設定を`.mcp.json`に書き込むようにしてください。

### Codex
公式ドキュメント：[Docs MCP quickstart for Codex](https://developers.openai.com/resources/docs-mcp)
...
## 典型的な使用パターン
1. 意図を持って`api_search`を実行（例：*「create affiliate」*や*「payment receipt」*）。
2. 返された`wink://api/...` URIを開くか、関連する`request_uri`、`response_uris`、`request_schema_uris`、`response_schema_uris`、`recipe_uri`フィールドをたどる。
3. 補助的な概念（認証、Webhook、ワークフロー）には`docs_search`を使用。
