...
| ツール | 目的 | 典型的なクエリ例 |
|------|---------|-----------------------|
| `sdk_search` | どのSDK／APIクラスを使うかを探す | `予約キャンセル`, `物件一覧`, `アフィリエイト在庫` |
| `docs_search` | 概念的なドキュメント／ガイドを探す | `認証`, `ウェブフック`, `WordPress` |

`query`を空にすると提案が受け取れます。すべての結果には開けるMCP URIが含まれます。

## 典型的な使用パターン
1. 意図を持って`sdk_search`を実行（例：*"物件一覧"*）。
2. 返されたSDK READMEまたはその`docs_uri`を開き、API／モデルを調査。
3. 補助的な概念（認証、ウェブフック）には`docs_search`を使用。
