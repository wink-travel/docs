...
3. サーバーがペイロードを処理し、`200 OK`で応答します。
...
5. **Webhook URL** を入力します — サーバーのHTTPSエンドポイントです。
...
Winkはアカウント管理、予約契約、支払い、チャネルマネージャーの更新、管理エンティティの管理などのカテゴリで**64以上のWebhookイベントタイプ**をサポートしています。よく使われるイベントは以下の通りです：
...
Webhookは削除せずに**無効化**できます。これにより配信が一時停止し、設定を失うことなくトラブルシューティングが可能です。準備ができたら再度オンに切り替えます。
...
- [Developers > APIs](/ja/api) — APIの完全なドキュメント。
