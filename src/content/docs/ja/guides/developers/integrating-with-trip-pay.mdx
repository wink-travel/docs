...
- [Payment Web Component](/ja/integrations/web-components/#payment).
- [Payment API](/ja/api). *`Create payable contracts`のセクションをお読みください*
...
3. 予約するアイテムを含むTripPayの`payable contract`を作成します。
...
### リクエスト
以下は、最初のpayable contractを作成するために必要なすべてを含むJSONサンプルリクエストです。
...
ここで全てのデータポイントを説明しませんが、APIドキュメントで既にカバーされています。

- 行：**2 - 7** は予約の責任者ユーザーです。予約者には2種類あります：
    - 旅行者
    - 旅行代理店
- 行：**8 - 9** は予約を仲介するTripPayアカウントです。以下のいずれかです：
    - あなたのTripPayインテグレーターアカウント
    - アフィリエイトネットワークを運営している場合は、そのアフィリエイト
- `traceId`は複数のサプライヤーにまたがる複数の予約をグループ化するために使います。これによりグループ予約のキャンセルが可能です。
...
- 予約したい在庫のサプライヤー。
- そのサプライヤーから予約したい在庫。これはネストされた配列`contractItemList`に指定されています。
- `contractItemList`の各エントリには：
    ...
    - **価格タイプ** 価格の計算方法。
    - **在庫タイプ** 在庫の種類。
    - **支払いタイミング** 旅行者がいつ支払うか。*現在は即時支払いのみ対応しています。*
    - **ポリシー** キャンセルポリシーのルールを含めます。
    - **日別料金リスト** 予約するアイテムが部屋の場合、宿泊日ごとの料金を含めることができます。
    - **受益者** どのTripPayアカウントに価格のどの金額を割り当てるかを含めます。*固定額/割合がサポートされています。*

:::note
上記JSON例では、3つの受益者がいます：
...
TripPayがpayable contractを返す際には、4.0%の独自の受益者が注入されていることに気づくでしょう。
:::
...
3行目の`id`を保存してください。これはTripPayのWebコンポーネントに注入するために使用します。
...
ここまでで、`Payment Web Component`をあなたのウェブサイトに埋め込み、ユーザーに表示する準備が整いました。

```html
<trip-pay id="contract-1"></trip-pay>
```

統合に関しては、これで完了です。あとはTripPayがすべての重い処理を行います。
