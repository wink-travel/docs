---
title: Klanten
description: Dit artikel legt de soorten klanten uit die wij bedienen en hoe Wink kan helpen.
sidebar:
  order: 2
---

Dit artikel legt de soorten klanten uit die wij bedienen en hoe wij het beste kunnen helpen.

## Eigendommen

Wink is tien jaar geleden begonnen met het onboarden van hotels. Hotels gebruiken Wink om hun voorraad effectief te beheren over duizenden verkoopkanalen, inclusief hun eigen brand.com-site, en om meer te verdienen met elke boeking terwijl ze directe kanalen versterken met behulp van onze op hospitality gerichte tools die de merkidentiteit en winstgevendheid verbeteren​.

## Merken & Ketens

Merken en ketens hebben iets andere behoeften dan een enkele accommodatie. Op Wink kunnen zij:

- Meerdere accommodaties beheren onder één account.
- Managers toewijzen aan accommodaties
- Gebruikmaken van [WinkLinks](/nl/portal/link-manager/wink-links), [WordPress](/nl/integrations/wordpress) en onze [Web Components](/nl/integrations/web-components) om meerdere accommodaties op één plek te promoten en te verkopen.

## Beheerbedrijven

Hospitality managementbedrijven en destination managementbedrijven kunnen hun vastgoedportefeuilles beheren namens de eigenaren. Er is een duidelijk voordeel om online-georiënteerde, digitaal bewuste bureaus de verkoop aan het online segment te laten afhandelen, terwijl hotels zich kunnen richten op het beheren van klanttevredenheid zodra zij op locatie zijn.

## Influencers

Influencers gebruiken Wink om hun publiek beter te gelde te maken. Ze doen dit door samen te werken met hotels, aangepaste deals te creëren en gebruik te maken van Winks socialvriendelijke tools en krachtige analyses om te volgen welke producten het beste verkopen​.

## Talentbureaus

Talentbureaus zijn voor creators wat managementbedrijven zijn voor hotels. Als je een groep influencers, content creators of reis persoonlijkheden vertegenwoordigt, kun je al hun Wink-accounts vanaf één plek beheren — voorraad vinden, deals onderhandelen met accommodaties, content bouwen en publiceren, en prestaties volgen over je hele roster in plaats van per creator.

Je onderhandelt een bureauvergoeding met elk van je talenten en verdient die vergoeding op elke boeking die zij genereren. Deze wordt automatisch uitbetaald als onderdeel van de boeking, dus er is geen facturering aan je eigen talenten en geen achtervolgen van commissies achteraf. Je talent blijft de controle houden — zij bezitten hun account, zij passen de afgesproken vergoeding toe, en ze kunnen vervalregels toevoegen of de overeenkomst beëindigen wanneer ze willen.

Zie [Agency representation](/nl/portal/settings/agency-representation) voor hoe dit is opgezet.

## Reisagenten

Reisagenten krijgen toegang tot reisvoorraad wereldwijd tegen scherpe prijzen via ons speciale [reisagentenportaal](https://agent.wink.travel). Ze kunnen boekingen maken via ons portaal, via onze API of door hun gebruikers zelf te laten boeken op de reisagentenwebsite die onze technologieën gebruikt.

## Bedrijven

Bedrijven hebben meer nodig van een reisplatform:

- Ze hebben goed georganiseerde reisschema’s nodig die flexibiliteit en last-minute wijzigingen ondersteunen.
- Ze hebben betrouwbare communicatietools nodig, zoals internet.
- Ze hebben voorkeuren voor leveranciers met lidmaatschappen.
- Ze hebben vaak een zakelijke creditcard en stellen bestedingslimieten in.
- Ze hebben behoefte aan onkostenbeheer en rapportage-integratie.
- Ze hebben conciërgeservice nodig, evenals MICE-diensten bij evenementen.

Zakelijke reizigers kunnen hun eigen accounts beheren via ons corporate portal of gebruikmaken van onze officieel ondersteunde [MS Sharepoint](https://www.microsoft.com/en-us/microsoft-365/sharepoint/collaboration) plugin en [Concur](https://www.concur.com/) integratie.

## Ontwikkelaars

Hoewel we dit platform hebben gebouwd door en voor hoteliers... is elke regel code geschreven met het specifieke doel van herbruikbaarheid. Ons team van ontwikkelaars heeft Wink gebouwd in de hoop dat de volgende 🦄 reisapp bovenop onze codebase wordt gebouwd. Ga naar de [Developer](/nl/developers/build-on-wink) pagina om te zien hoe Wink jou kan ondersteunen in je volgende project.

## Overig

Iedereen die reisvoorraad wil vinden en verkopen kan profiteren van het gebruik van Wink. Zet je passie voor hospitality om in actie door een mooie commissie te verdienen op elke boeking.
