...
- [Payment Web Component](/nl/integrations/web-components/#payment).
- [Payment API](/nl/api). *Lees de sectie over `Create payable contracts`*

Hier zijn de stappen om TripPay succesvol te gebruiken voor het betalen van een boeking:

<Steps>
1. Zorg ervoor dat de begunstigden die je noemt in het boekingscontract zijn gemapt.
...
4. Laat de reiziger betalen met behulp van de TripPay Web Component.
</Steps>
...
We zullen hier niet elk datapunt uitleggen omdat deze al behandeld zijn in de API-documentatie.

- Regels: **2 - 7** is de gebruiker die verantwoordelijk is voor de boeking. Er zijn twee soorten boekers:
    - Reiziger.
    - Reisagent.
- Regels: **8 - 9** is het TripPay-account dat de boeking faciliteert. Dit kan zijn:
...
- `redirectUrl` laat TripPay weten naar welke pagina moet worden doorgestuurd nadat de betaling is voltooid.
...
- Elke invoer onder `contractItemList` bevat:
    ...
    - **Dagelijkse tarieflijst** Als het geboekte item een kamer is, kun je opnemen hoeveel die kamer kost voor elke nacht dat de gast verblijft.
...
:::note
Zorg ervoor dat je onze stijlen en Javascript van tevoren hebt geladen.
:::
...
Wat de integratie betreft, is dat alles wat je hoeft te doen. Al het zware werk wordt vanaf hier door TripPay gedaan.
