...
2. **Voer deze in het Wink-portaal in**
   ...
   Log in op [app.wink.travel](https://app.wink.travel), selecteer uw account en ga naar het tabblad **Aanpassingen**. Plak uw Sojern GTM Container ID in het Sojern-veld en sla op. Uw integratie is nu actief.
...
- **Op elke pagina waar uw accommodatie actief is** op Wink, wordt uw Sojern GTM Container ID in de pagina weergegeven en laadt het tagmanagementsysteem van Wink dynamisch uw Sojern-container naast de eigen container. Dit gebeurt automatisch — er zijn geen handmatige stappen nodig van het Wink-team per accommodatie.
...
- **De tracking van Wink en uw Sojern-pixel draaien parallel**, waarbij ze dezelfde gedeelde gebeurtenisgegevens lezen. Elk boekingsgebeurtenis dat Wink volgt, is tegelijkertijd beschikbaar voor uw Sojern-container.
...
Wink pusht GA4-compatibele gebeurtenissen naar de `dataLayer` gedurende de hele gastreis. Uw GTM-container leest uit dezezelfde gedeelde `dataLayer`, dus elke trigger of variabele die u configureert heeft toegang tot alle hieronder beschreven eigenschappen.

Elke gebeurtenis volgt de standaard GA4 Enhanced Ecommerce-structuur: een top-level `event` string en een `ecommerce` object. Voor elke push wist Wink de vorige ecommerce-payload om te voorkomen dat verouderde gegevens in nieuwe triggers terechtkomen.

<Aside type="note">
**dataLayer reset-patroon**

Wink reset het ecommerce-object voor elke gebeurtenis-push:

```js
dataLayer.push({ ecommerce: null });   // wis vorige gebeurtenis
dataLayer.push({ event: '...', ecommerce: { ... } });
```

Vertrouw er niet op dat ecommerce-eigenschappen tussen gebeurtenissen blijven bestaan. Lees ze altijd op het moment van trigger binnen dezelfde gebeurtenis.
</Aside>

---

### Boekingsreis-gebeurtenissen

Deze gebeurtenissen worden automatisch geactiveerd terwijl een gast door de boekingsfunnel beweegt. Configureer uw GTM-triggers als **Aangepaste gebeurtenis** triggers die overeenkomen met de onderstaande gebeurtenisnamen.

| Stap | Gebeurtenis | Wanneer het afgaat |
|------|-------------|--------------------|
...
Wordt geactiveerd wanneer een gast zich aanmeldt via single sign-on.
...
`transaction_id` komt overeen met de `purchase` gebeurtenis voor dezelfde boeking. `coupon`, `shipping` en `tax` zijn alleen aanwezig indien van toepassing.

---

### Item-eigenschappen

Alle gebeurtenissen die een `items` array bevatten, delen hetzelfde itemschema. Elke eigenschap is optioneel tenzij als verplicht gemarkeerd.
...
| `discount` | number | — | Toegepaste kortingsbedrag |
...
| `property_chain` | string | Ketting waartoe de accommodatie behoort |
...
GA4 negeert aangepaste itemparameters in rapporten totdat u ze expliciet registreert als item-gescopeerde aangepaste dimensies. U hoeft alleen de parameters te registreren die u in rapporten wilt gebruiken — niet alle 20.
...
3. **Vul de dimensiedetails in**
   ...
   - **Gebeurtenisparameter** — de exacte parameternaam zoals die in de dataLayer voorkomt (zie tabel)
...
GA4 staat maximaal 50 item-gescopeerde aangepaste dimensies toe per standaardproperty (360-properties hebben een hoger limiet).
</Aside>

Gebruik de volgende tabel als referentie bij het aanmaken van aangepaste dimensies. De waarde van **Gebeurtenisparameter** moet exact overeenkomen — GA4 is hoofdlettergevoelig.

| Dimensienaam | Gebeurtenisparameter | Type |
|--------------|---------------------|------|
...
### Extra beschikbare gebeurtenissen

De volgende gebeurtenissen zijn beschikbaar in de Wink dataLayer maar maken geen deel uit van de standaard boekingsfunnel. Ze kunnen worden geactiveerd door andere interacties op het platform.
...
Bij deze gebeurtenis wordt geen `ecommerce` payload meegeleverd.
...
Dit is een aangepaste Wink-gebeurtenis — geen onderdeel van de standaard GA4-gebeurtenissen. Het wordt gepusht bij elke paginanavigatie en bevat apparaat- en sessiecontext. Het bevat geen `ecommerce` object.
