...
Hier zijn de stappen om een geauthenticeerde sessie te maken.
...
Wanneer je een verzoek doet naar een van onze eindpunten, voeg dan het volgende toe in de header:
...
door spaties gescheiden lijst van scopes mee in de `scope` parameter, en het token kan alleen toegang krijgen tot de gebieden
...
:::note
De kolom **Risico** geeft de gevoeligheid aan: `read` is het laagste niveau en `write`/`remove` het hoogste.
**Payment** en **Accounting** bewaken geldstromen, dus die hebben een verhoogd risico.
:::

Een paar dingen om in gedachten te houden:

- **Vraag het minimum aan.** Vraag alleen de scopes aan die jouw integratie daadwerkelijk gebruikt — een token met bredere toegang dan nodig vergroot de impact bij een lek.
...
- **`mcp.*` is alleen voor AI-agent clients.** Je hebt dit nodig om de MCP transport te openen; het is niet vereist voor reguliere REST API-aanroepen. Zie [Model Context Protocol](/nl/developers/mcp).

:::tip
Je kunt al onze API's testen zonder ook maar één regel code te schrijven voordat je begint.

Pak een 'Client ID' en 'Secret Key' van een van je apps en navigeer naar een van onze [API's](/nl/api/overview/).
:::
