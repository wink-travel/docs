...
<Steps>
1. [Registreer](/nl/getting-started/sign-up) je gebruikersaccount op Wink.
2. [Maak een affiliate account aan](/nl/portal/create-account). Dit heb je nodig om je Application aan te koppelen. Zie [wat is Studio](/nl/portal/studio/what-is-studio) voor meer context.
3. [Maak een Application aan](/nl/account/applications) en koppel deze aan het affiliate account dat je in stap 2 hebt aangemaakt. Kies voor server-naar-server API-integraties **MACHINE_2_MACHINE** als clienttype.
4. Zodra je je `Application` hebt aangemaakt, kopieer en bewaar je direct de client-id en geheime sleutel — de geheime sleutel wordt slechts één keer getoond en kan niet opnieuw worden opgevraagd.
</Steps>
...
Ga naar onze [API's](/nl/api/overview/) sectie en kies de API waarmee je wilt werken.
...
2. Klik bovenaan de [Lookup API](https://developer.wink.travel/lookup) pagina op `Download OpenAPI spec`.
...
13. Stel het veld `Scope` in op de spatie-gescheiden scopes die je integratie nodig heeft (bijv. `inventory.read inventory.write inventory.remove`). Zie de [scope referentie](/nl/developers/authentication#scopes) voor de volledige lijst.
...
:::tip[Pro Tip]
Het wordt aanbevolen om het serverdomein, client ID en client secret als omgevingsvariabelen in te stellen. Zo kun je eenvoudig schakelen tussen staging- en productieomgevingen en hergebruik je alle gemaakte operaties.
:::
