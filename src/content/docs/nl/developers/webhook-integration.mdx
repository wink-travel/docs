...
2. Wanneer een gebeurtenis plaatsvindt (bijv. een nieuwe boeking), stuurt Wink een HTTP POST naar je URL.
...
- Een `5xx`, timeout, `408` of `429` wordt met backoff opnieuw geprobeerd: na 1 minuut, 5 minuten, 30 minuten, 2 uur, 6 uur, 12 uur, daarna dagelijks — 10 pogingen in ongeveer 3 dagen — waarna de levering als **dead** wordt gemarkeerd.
...
`POST /api/managing-entity/{id}/webhook/{webhookId}/test`. Deze is ondertekend en wordt geleverd zoals een echte gebeurtenis, zodat je je eindpunt, handtekeningcontrole en idempotentieafhandeling kunt verifiëren voordat je je abonneert op live gebeurtenissen.

## Beste praktijken

- **Gebruik HTTPS** — Wink stuurt payloads alleen naar HTTPS-eindpunten.
- **Reageer snel** — Geef een `200 OK` terug zodra je de payload ontvangt. Doe zware verwerking asynchroon.
...
- **Valideer de bron** — Verifieer de `Wink-Signature` header (zie [Handtekeningen verifiëren](#verifiëren-handtekeningen)) voordat je verwerkt; verwerp alles wat faalt.
...
Een webhook verwijderen verwijdert deze permanent. Elke integratie die op die webhook vertrouwt, ontvangt dan geen meldingen meer.
...
- [Developers > APIs](/nl/api/overview/) — Volledige API-documentatie.
