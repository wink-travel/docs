...
- `type` — der Ereignistyp-Schlüssel (wird auch als `Wink-Event-Type` Header gesendet). Verzweigen Sie anhand von `type` und `schemaVersion`, um `object` zu parsen.
...
Jedes Payload-Schema ist pro Ereignis in der [Webhooks API](/de/api/webhooks/) Referenz dokumentiert.
...
wobei `t` ein Unix-Timestamp (Sekunden) ist und `v1` die kleingeschriebene hexadezimale **HMAC-SHA256** des Strings
...
- **Verwenden Sie HTTPS** — Wink sendet Payloads nur an HTTPS-Endpunkte.
...
- **Quelle validieren** — Verifizieren Sie den `Wink-Signature`-Header (siehe [Signaturen verifizieren](#signaturen-verifizieren)) vor der Verarbeitung; lehnen Sie alles ab, was fehlschlägt.
...
- [Webhooks API reference](/de/api/webhooks/) — Payload-Schemas pro Ereignis, Header und Endpunkte zur Verwaltung von Abonnements und Zustellungen.
...
- [Developers > APIs](/de/api/overview/) — Vollständige API-Dokumentation.
