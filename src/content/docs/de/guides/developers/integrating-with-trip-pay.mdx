...
- [Payment Web Component](/de/integrations/web-components/#payment).
- [Payment API](/de/api). *Lesen Sie den Abschnitt zu `Create payable contracts`*
...
## Erstellen eines zahlbaren Vertrags

:::note
Besuchen Sie die Payment API, um mehr über die Anforderungen zum Erstellen eines zahlbaren Vertrags zu erfahren.
:::

### Anfrage
Hier ist ein JSON-Beispiel für eine Anfrage, die alles enthält, was Sie für Ihren ersten zahlbaren Vertrag benötigen.
...
- Zeilen: **2 - 7** sind der Benutzer, der für die Buchung verantwortlich ist. Es gibt zwei Arten von Buchern:
    - Reisender.
    - Reisebüro.
...
- Jeder Eintrag unter `contractItemList` enthält:
    ...
    - **Verfügbarkeitstyp** Um welche Art von Verfügbarkeit es sich handelt.
    ...
    - **Tagespreisliste** Wenn der zu buchende Artikel ein Zimmer ist, können Sie angeben, wie viel dieses Zimmer für jede Nacht kostet, die der Gast bleibt.
...
Wenn TripPay mit einem zahlbaren Vertrag antwortet, werden Sie feststellen, dass es seinen eigenen Begünstigten mit 4,0 % eingefügt hat.
:::
...
An diesem Punkt sind Sie bereit, die `Payment Web Component` in Ihre Website einzubetten und dem Benutzer anzuzeigen.
