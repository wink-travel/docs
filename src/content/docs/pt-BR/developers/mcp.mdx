...
Servidores MCP só podem ser configurados no **app desktop**. A interface web do ChatGPT em chatgpt.com não suporta servidores MCP personalizados.
:::
...
Quando você aprova a tela de consentimento, verá um detalhamento do que o agente está solicitando. Clientes MCP usam o mesmo vocabulário de permissões do restante da plataforma — veja a referência completa de [escopos na página de Autenticação](/pt-BR/developers/authentication#scopes).
...
Você está logado em uma conta Wink sem entidade gestora. Acesse o [Portal Wink](https://app.wink.travel) e certifique-se de que sua conta está configurada antes de usar o servidor MCP.
