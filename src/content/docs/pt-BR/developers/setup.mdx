...
<Steps>
1. [Registre-se](/pt-BR/user/register) com sua conta de usuário no Wink
2. Crie uma [conta de agente de viagens](/pt-BR/travel-agent/what-is-agent) ou uma [conta de afiliado](/pt-BR/studio/what-is-studio).
3. [Crie uma Aplicação](/pt-BR/developer/applications) associando-a à conta criada no Passo 2.
...
Para cada tipo de inventário de viagens que suportamos, haverá um array JSON aninhado chamado `multimedias`. O JSON para uma entrada multimídia é assim:
...
Acesse nossa seção de [APIs](/pt-BR/developer/apis) e escolha a API com a qual deseja trabalhar.
...
Para facilitar o uso, recomendamos que os integradores utilizem os endpoints que designamos como `OAuth2`. Esses endpoints já sabem que são chamadas de API e já identificam a conta em que você está, então não é necessário especificar o `companyIdentifier` nas URLs.
:::
