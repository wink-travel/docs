---
title: 설정
description: 이 문서에서는 개발자로서 빠르게 설정하고 Wink에서 API 테스트를 시작하는 방법을 보여줍니다.
sidebar:
  order: 1
---
...
:::note
테스트는 항상 [스테이징 환경](/ko/getting-started/environments)에서 진행해야 합니다. 데이터가 훨씬 적지만, 프로덕션 환경에 영향을 주지 않고 원하는 만큼 자유롭게 테스트할 수 있습니다.

라이브 준비가 완료되면, 계정과 애플리케이션을 인증해 드리며 프로덕션 환경에서 런칭할 수 있습니다.
:::

다음 단계를 따르세요:

<Steps>
1. Wink에서 사용자 계정을 [등록](/ko/getting-started/sign-up)하세요.
2. [제휴사 계정 생성](/ko/portal/create-account). 애플리케이션을 연결하려면 필요합니다. 자세한 내용은 [Studio란 무엇인가](/ko/portal/studio/what-is-studio)를 참조하세요.
3. [애플리케이션 생성](/ko/account/applications) 후 2단계에서 생성한 제휴사 계정과 연결하세요. 서버 간 API 통합의 경우 클라이언트 유형으로 **MACHINE_2_MACHINE**을 선택하세요.
4. `Application`을 생성한 후, 클라이언트 ID와 비밀 키를 즉시 복사하여 저장하세요 — 비밀 키는 한 번만 표시되며 다시 조회할 수 없습니다.
</Steps>

## CDN

Wink에서는 모든 이미지 및 비디오 자산에 Cloudinary를 사용합니다. 여행 인벤토리에 접근할 때 Cloudinary의 모든 고급 기능을 완전히 활용할 수 있습니다.
...
[APIs](/ko/api/overview/) 섹션으로 이동하여 작업할 API를 선택하세요.
...
:::tip[전문가 팁]
사용 편의를 위해, 통합자는 우리가 `OAuth2`로 지정한 엔드포인트를 사용하는 것을 권장합니다. 해당 엔드포인트는 API 호출자를 인식하며, 어떤 계정인지 이미 알고 있어 URL에 `companyIdentifier`를 명시할 필요가 없습니다.
:::

<Steps>
1. [Lookup API](https://developer.wink.travel/lookup)를 사용하여 목적지 또는 호텔을 조회하세요.
2. 왼쪽 네비게이션 바에서 `Authentication`을 클릭하세요.
3. 클라이언트 ID와 클라이언트 비밀 키를 입력하고 `Get TOKEN` 버튼을 클릭하세요.
...
8. `Token Name` 필드에 Postman에 저장할 토큰 이름을 입력하세요. 예: Wink Staging.
9. `Grant type` 필드를 `Client Credentials`로 설정하세요.
10. `Access token URL` 필드를 https://staging-iam.wink.travel/oauth2/token 으로 설정하세요.
11. `Client ID` 필드에 앱이 생성한 클라이언트 ID를 입력하세요.
12. `Client Secret` 필드에 앱이 생성한 클라이언트 비밀 키를 입력하세요.
13. `Scope` 필드에 통합에 필요한 공백 구분 범위를 입력하세요 (예: `inventory.read inventory.write inventory.remove`). 전체 목록은 [scope reference](/ko/developers/authentication#scopes)를 참조하세요.
14. `Client Authentication` 필드를 `Send as Basic Auth header`로 설정하세요.
15. `Get New Access Token` 버튼을 클릭하여 토큰을 받고 저장하세요.
16. Wink API 컬렉션 내 모든 API 호출에서 인증 탭이 `Inherit auth from parent`로 설정되어 있어야 모든 호출에 토큰이 포함됩니다.
</Steps>

:::tip[전문가 팁]
서버 도메인, 클라이언트 ID, 클라이언트 비밀 키를 환경 변수로 설정하는 것을 권장합니다. 이렇게 하면 스테이징과 프로덕션 환경 간에 쉽게 전환하고 생성한 모든 작업을 재사용할 수 있습니다.
:::
