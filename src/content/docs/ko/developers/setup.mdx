...
라이브로 전환할 준비가 되면, 계정과 애플리케이션을 인증해 드리며 프로덕션 환경에서 시작할 수 있습니다.
:::

다음 단계를 따르세요:

<Steps>
1. Wink에서 사용자 계정을 [등록](/ko/my-account/register)하세요.
2. [제휴사 계정 생성](/ko/my-account/create-account). 애플리케이션을 연결하려면 필요합니다. 자세한 내용은 [Studio란 무엇인가](/ko/studio/what-is-studio)를 참조하세요.
3. [애플리케이션 생성](/ko/my-account/applications) 후 2단계에서 생성한 제휴사 계정과 연결하세요. 서버 간 API 통합의 경우 클라이언트 유형으로 **MACHINE_2_MACHINE**을 선택하세요.
...
지원하는 모든 여행 인벤토리 유형마다 `multimedias`라는 중첩된 JSON 배열이 있습니다. 멀티미디어 항목의 JSON 예시는 다음과 같습니다:
...
[APIs](/ko/developers/apis) 섹션으로 이동하여 작업할 API를 선택하세요.
...
:::tip[전문가 팁]
사용 편의를 위해, 통합자는 우리가 `OAuth2`로 지정한 엔드포인트를 사용하는 것을 권장합니다. 해당 엔드포인트는 API 호출자를 인식하며, URL에 `companyIdentifier`를 명시할 필요가 없습니다.
:::
...
3. Postman에 가져오세요.
4. 새 컬렉션 `Wink API`로 가져왔다면, 컬렉션 이름 옆의 가로 점 3개(…)를 클릭하고 `Edit`을 선택하세요.
...
11. `Client ID` 필드에 애플리케이션이 생성한 클라이언트 ID를 입력하세요.
12. `Client Secret` 필드에 애플리케이션이 생성한 클라이언트 비밀 키를 입력하세요.
