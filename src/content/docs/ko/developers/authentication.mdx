...
대부분의 경우, 언어별 강력한 OAuth2 라이브러리를 사용하여 모든 복잡한 작업을 처리할 것입니다.
...
- **최소한으로 요청하세요.** 통합에서 실제 사용하는 스코프만 요청하세요 — 불필요하게 넓은 권한을 가진 토큰은 유출 시 피해 범위가 커집니다.
...
앱 중 하나에서 'Client ID'와 'Secret Key'를 받아서 [API](/ko/api/overview/) 중 원하는 곳으로 이동하세요.
:::
