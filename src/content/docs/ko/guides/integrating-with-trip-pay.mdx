...
- [결제 > 매핑](/ko/payment/mapping).
- [결제 > 통합](/ko/payment/integration).
- [결제 웹 컴포넌트](/ko/integrations/web-components/#payment).
- [결제 API](/ko/developers/apis/#payment-api). *`Create payable contracts` 섹션을 읽으세요.*
...
3. 예약할 항목이 포함된 TripPay의 `payable contract`를 생성합니다.
4. 여행자가 TripPay 웹 컴포넌트를 사용하여 결제하도록 합니다.
</Steps>

`1단계`와 `2단계`를 이미 완료하고 TripPay에 예약 정보를 알릴 준비가 되었다고 가정합니다.

## payable contract 생성하기

:::note
payable contract 생성 요건에 대해 자세히 알아보려면 결제 API를 참조하세요.
:::
...
- `traceId`는 여러 공급자에 걸친 여러 예약을 그룹화하는 데 사용합니다. 이를 통해 그룹 예약을 취소할 수 있습니다.
...
- 해당 공급자로부터 예약할 인벤토리입니다. 모두 중첩 배열 `contractItemList`에 명시됩니다.
...
- 9.85%는 제휴사 커미션으로 할당되었습니다. *예: Kim Kardashian*
