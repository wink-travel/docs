...
Jei vykdote reklamos kampanijas per Sojern, galite prijungti savo Sojern paskyrą prie Wink įvesdami savo Sojern GTM konteinerio ID Wink portale. Prisijungus, Sojern sekimo pikselis automatiškai aktyvuojamas jūsų apgyvendinimo vietai — apimant visą svečio kelionę nuo paieškos iki užsakymo užbaigimo.
...
2. **Įveskite jį Wink portale**
   ...
   Prisijunkite prie [app.wink.travel](https://app.wink.travel), pasirinkite savo paskyrą ir eikite į skirtuką **Customizations**. Įklijuokite savo Sojern GTM konteinerio ID į Sojern laukelį ir išsaugokite. Jūsų integracija dabar veikia.

3. **Patvirtinkite su savo Sojern paskyros vadybininku**

   Praneškite savo Sojern paskyros vadybininkui, kad integracija aktyvi. Jie gali patikrinti iš savo pusės, ar pikselis tinkamai veikia `wink.travel` jūsų apgyvendinimo vietai ir ar duomenys teka į jūsų Sojern paskyrą.

</Steps>

<Aside type="note">
Jums nereikia įdėti jokio pikselio kodo į savo svetainę šiai integracijai. Wink automatiškai įkelia jūsų Sojern konteinerį visuose puslapiuose, kuriuose aktyvi jūsų apgyvendinimo vieta.

Šis nustatymas veikia su bet kokiu pasirinktu GTM konteinerio ID. Sojern pateiktas tik kaip pavyzdys, nes tai labai dažna integracija.
</Aside>
...
- **Sojern GTM konteinerio ID laukas** yra prieinamas Wink portalo Customizations skirtuke. Įvedus konteinerio ID, Wink patikrina formato teisingumą ir saugiai saugo jį jūsų apgyvendinimo vietai.
- **Kiekviename puslapyje, kuriame aktyvi jūsų apgyvendinimo vieta** Wink įterpia jūsų Sojern GTM konteinerio ID į puslapį ir Wink žymių valdymo sistema dinamiškai įkelia jūsų Sojern konteinerį kartu su savo. Tai vyksta automatiškai — Wink komandai nereikia atlikti jokių rankinių veiksmų kiekvienai apgyvendinimo vietai.
- **Jūsų Sojern konteineris įkeliamas asinchroniškai**, todėl tai neturi įtakos puslapio įkėlimo greičiui jūsų svečiams.
...
- **Kai jūsų apgyvendinimo vieta nėra peržiūrima** — pavyzdžiui, kai svečias naršo kitą viešbutį Wink — jūsų Sojern konteineris neįkeliamas. Tai užtikrina, kad Sojern gauna tik jūsų apgyvendinimo vietai aktualius duomenis.
...
| 2 | `view_item` | Svečias peržiūri konkrečios apgyvendinimo vietos puslapį |
...
Aktyvuojamas, kai svečias peržiūri konkrečią apgyvendinimo vietą.
...
`transaction_id` atitinka tą patį `purchase` įvykį už tą patį užsakymą. `coupon`, `shipping` ir `tax` pateikiami tik jei taikytini.
...
| `discount` | number | — | Taikyta nuolaida |
...
| `location_id` | string | — | Apgyvendinimo vietos geografinės koordinatės |
...
Šie parametrai perteikia Wink specifinį užsakymo kontekstą. GA4 jų automatiškai nerodo ataskaitose — kiekvieną reikia užregistruoti kaip **prekės lygmens pasirinktinius matmenis** savo GA4 paskyroje, kad jie atsirastų ataskaitose ar analizėse. Žr. [Pasirinktinius matmenis GA4](#custom-dimensions-in-ga4) žemiau dėl nustatymo instrukcijų.

| Savybė | Tipas | Aprašymas |
|----------|------|-------------|
| `property_id` | string | Wink apgyvendinimo vietos identifikatorius |
| `property_name` | string | Apgyvendinimo vietos pavadinimas |
| `property_brand` | string | Prekės ženklas, kuriam priklauso apgyvendinimo vieta |
| `property_chain` | string | Tinklas, kuriam priklauso apgyvendinimo vieta |
| `property_url_name` | string | URL draugiškas apgyvendinimo vietos slapyvardis |
| `city_name` | string | Miestas, kuriame yra apgyvendinimo vieta |
...
1. **Atidarykite Custom Definitions**
   ...
   GA4 eikite į **Admin** (apatinis kairysis pavaros piktograma) → pasirinkite savo paskyrą → **Data display** → **Custom definitions**.

2. **Sukurkite naują pasirinktį matmenį**
...
<Aside type="note">
Pasirinktini matmenys gali pradėti rodyti duomenis GA4 ataskaitose per 24 valandas po registracijos. Istoriniai duomenys, surinkti prieš registraciją, nebus papildyti.

GA4 leidžia iki 50 prekės lygmens pasirinktinių matmenų standartinėse paskyrose (360 paskyrose leidžiama daugiau).
</Aside>
...
|----------------|-----------------|------|
| Apgyvendinimo vietos ID | `property_id` | Tekstas |
| Apgyvendinimo vietos pavadinimas | `property_name` | Tekstas |
| Apgyvendinimo vietos prekės ženklas | `property_brand` | Tekstas |
| Apgyvendinimo vietos tinklas | `property_chain` | Tekstas |
| Apgyvendinimo vietos URL pavadinimas | `property_url_name` | Tekstas |
...
Šie įvykiai yra prieinami Wink dataLayer, bet nėra standartinio užsakymo piltuvo dalis. Jie gali būti aktyvuojami kitų platformos sąveikų metu.
...
Tai yra pasirinktinė Wink įvykio rūšis — nėra standartinės GA4 įvykių dalis. Jis siunčiamas kiekvieną kartą keičiant puslapį ir perduoda įrenginio bei sesijos kontekstą. Jame nėra `ecommerce` objekto.
