...
:::note
Testavimas visada turėtų vykti mūsų [staging aplinkoje](/lt/getting-started/environments). Ten yra daug mažiau duomenų, tačiau galite testuoti tiek, kiek norite, neįtakojant gamybinės aplinkos.

Kai būsite pasiruošę paleisti gyvai, mes patvirtinsime jūsų paskyrą ir programą, ir galėsite paleisti ją gamybinėje aplinkoje.
:::

Sekite šiuos žingsnius:

<Steps>
1. [Užsiregistruokite](/lt/getting-started/sign-up) Wink vartotojo paskyrą.
2. [Sukurkite filialo paskyrą](/lt/portal/create-account). Ją reikės susieti su jūsų programa. Daugiau konteksto rasite skyriuje [kas yra Studio](/lt/portal/studio/what-is-studio).
3. [Sukurkite programą](/lt/account/applications) ir susiekite ją su filialo paskyra, sukurtą 2 žingsnyje. Serverio-serverio API integracijoms pasirinkite klientų tipą **MACHINE_2_MACHINE**.
...
Kiekvienam palaikomam kelionių inventoriaus tipui bus įdėtas JSON masyvas pavadinimu `multimedias`. Multimedia įrašo JSON atrodo taip:
...
Eikite į mūsų [API](/lt/api/overview/) skyrių ir pasirinkite API, su kuria norite dirbti.
...
:::tip[Pro patarimas]
Patogumui rekomenduojame integratoriams naudoti galinius taškus, pažymėtus kaip `OAuth2`. Šie galiniai taškai žino, kad API kvietėjai yra autentifikuoti ir jau žino, kurioje paskyroje esate, todėl jums nereikia nurodyti `companyIdentifier` URL.
:::
...
13. Lauke `Scope` įrašykite tarpu atskirtus reikalingus integracijos leidimus (pvz., `inventory.read inventory.write inventory.remove`). Visą sąrašą rasite [scope reference](/lt/developers/authentication#scopes).
...
:::tip[Pro patarimas]
Rekomenduojama serverio domeną, client ID ir client secret paversti aplinkos kintamaisiais. Taip lengvai galėsite keisti tarp staging ir gamybinės aplinkos bei pakartotinai naudoti sukurtas operacijas.
:::
