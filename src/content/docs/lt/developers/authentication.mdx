...
- `Authorization` = `Bearer ${access_token}` *Įdėkite savo prieigos raktą.*
...
| **Rezervavimas** | `booking.read` `booking.write` `booking.remove` | Ieškoti, peržiūrėti, kurti, atšaukti ir grąžinti rezervacijas; atsiskaitymas; atšaukimo taisyklės | Žema–Vidutinė |
| **Marketingas** | `marketing.read` `marketing.write` `marketing.remove` | Kampanijos, priedai, pardavimo kanalai, WinkLinks, valdomi socialiniai tinklai, lojalumas | Žema–Vidutinė |
| **Turinys** | `content.read` `content.write` `content.remove` | Objekto aprašymai, atsiliepimai, medija, žemėlapiai, žaliasis indeksas | Žema–Vidutinė |
...
- **Prašykite minimumo.** Prašykite tik tų leidimų, kuriuos jūsų integracija iš tiesų naudoja — raktas su platesnėmis teisėmis, nei reikia, nutekėjus gali sukelti didesnę žalą.
...
- **`mcp.*` yra tik AI agentų klientams.** Jums jo reikia atidaryti MCP transportą; jis nėra būtinas įprastiems REST API kvietimams. Žr. [Model Context Protocol](/lt/developers/mcp).

:::tip
Galite išbandyti visus mūsų API nerašydami nė vienos kodo eilutės prieš pradedant.

Paimkite 'Client ID' ir 'Secret Key' iš vienos savo programų ir eikite į bet kurį iš mūsų [API](/lt/api/overview/).
:::
